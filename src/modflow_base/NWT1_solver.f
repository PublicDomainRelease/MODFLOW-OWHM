...
!------NEWTON SOLVER VERSION NUMBER 1.1.3, 8/01/2017
      USE CONSTANTS,         ONLY:BLNK,BLN,NL
...
!3-----ALLOCATE SPACE
      ...
      ALLOCATE (Diag(3,Ncol*Nrow*Nlay), II, Hiter(Ncol, Nrow, Nlay))
      Numnonzero = 0
      II = 0
      ICELL = 0
      DIAG = 0
...
!     -----------------------------------------------------------------
      ...
      INTEGER ij, jj, ichld, irhld, ilhld, IUNSTR
      INTEGER n_iter, z, n, icfld, irfld, ilfld, ITP, ITER1U
!      CHARACTER ierr
      !SAVE FHEADTEMP,itertot - MADE PART OF GWFNWTMODULE
...
!     Set the head in dewatered cells to Hdry.
     ...
     +                 LAYHDT, IOUT  
      ...
      USE GWFNWTMODULE,ONLY:itreal, itertot
      IMPLICIT NONE
!     ------------------------------------------------------------------
!     SPECIFICATIONS:
!     -----------------------------------------------------------------
...
C seb USE HFB SPECIFIC DERIVATIVE CALCULATIONS
     ...
     +                                     Cv(ic,ir,il-1)*Dv
            END IF
            IF ( IR.GT.1 ) THEN
              IF ( hh.GT.HNEW(ic,ir-1,il)) Dc(2,ij) = 
     +                                     Cc(ic,ir-1,il)*Dh
            END IF
            IF ( IC.GT.1 ) THEN
              IF ( hh.GT.HNEW(ic-1,ir,il)) Dc(3,ij) = 
     +                                     Cr(ic-1,ir,il)*Dh
            END IF
            IF ( IC.LT.NCOL ) THEN
...
C#######################################################################
            IF ( ill.LT.il.AND.irr.EQ.ir.AND.icc.EQ.ic ) THEN           !seb Rearranged order of if statements, added .AND. 
              A(I) = (Cvm1 + Dc(6,Icell(ic,ir,il-1))*(Hvm1-H))
              BB(ij) = BB(ij) + A(I)*Hvm1
              sum = sum + A(I)
            ELSEIF ( ill.GT.il.AND.irr.EQ.ir.AND.icc.EQ.ic ) THEN
              A(I) = (Cvv + Dc(1,Icell(ic,ir,il+1))*(Hvp1-H))
              BB(ij) = BB(ij) + A(I)*Hvp1
              sum = sum + A(I)
            ELSEIF ( irr.LT.ir ) THEN
              A(I) = (Ccm1 + Dc(5,JA(I))*(Hrm1-H))                      !seb CHANGED Dc(Icell(ic,ir-1,il),5) to Dc(JA(I),5)
              BB(ij) = BB(ij) + A(I)*Hrm1
              sum = sum + A(I)
            ELSEIF ( icc.LT.ic ) THEN
C seb CHANGED Dc(Icell(ic,ir-1,il),4) to Dc(JA(I),4)
              A(I) = (Crm1 + Dc(4,JA(I))*(Hcm1-H))                      !seb CHANGED Dc(Icell(ic,ir-1,il),4) to Dc(JA(I),4)
              BB(ij) = BB(ij) + A(I)*Hcm1
              sum = sum + A(I)
            ELSEIF ( icc.GT.ic ) THEN
              A(I) = (Crr  + Dc(3,JA(I))*(Hcp1-H))                      !seb CHANGED Dc(Icell(ic,ir-1,il),3) to Dc(JA(I),3)
              BB(ij) = BB(ij) + A(I)*Hcp1
              sum = sum + A(I)
            ELSEIF ( irr.GT.ir ) THEN
              A(I) = (Ccc  + Dc(2,JA(I))*(Hrp1-H))                      !seb CHANGED Dc(Icell(ic,ir-1,il),2) to Dc(JA(I),2)
