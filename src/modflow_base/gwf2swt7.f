      MODULE GWFSWTMODULE
        ...
        REAL,    SAVE,POINTER    ::NOPCS                               !seb value PCS is set to when a cell goes dry
...
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
      USE GLOBAL,    ONLY: NCOL,NROW,NLAY,NPER,ISSFLG,NSTP,LAYCBD,NOCBC,
...
C     ------------------------------------------------------------------
     ...
     1                       NLAY,ISSFLG,IOUT,LAYHDT,HOLD
      USE GWFBASMODULE,ONLY: DELT
      USE GWFSWTMODULE,ONLY: NSYSTM,THICK,CE,CI, NOPCS,
...
C3------DETERMINE STORAGE CAPACITIES FOR CELL AT START AND END OF STEP
      ...
      BT=BOTM(J,I,K)
      !
      IF(HHNEW.LE.BT .AND. HOLD(J,I,K).LE.BT) CYCLE
C
...
C3C-----THICKNESS
        ...
        IF(TFACT.LE.0.0) CYCLE         !RHO1 and RHO2 END UP BEING ZERO ANYWAY
      ENDIF
      GLN=GL(J,I,K)
      ZCN=BT
      IF(HHNEW <= BT) THEN   ! .AND. LAYHDT(K).NE.Z  seb
          ESTN=GLN
      ELSE
          ESTN=GLN-HHNEW+ZCN
      END IF
     ...
     1   (ESTOLD(J,I,K)-(ZC(J,I,K)-BT)*(SGS(J,I)-1.)))
      RHO1=CE(J,I,KQ)*FACT
      PCTMP=PCS(J,I,K)
      !
      IF(ESTN.GT.PCTMP) THEN
          RHO2=CI(J,I,KQ)*FACT
      ELSE
          RHO2=RHO1
          PCTMP=0.0
...
C4------ADD APPROPRIATE TERMS TO RHS AND HCOF
      CRHS=-RHO2*(GLN+ZCN)+PCTMP*(RHO2-RHO1)+RHO1*ESTN1   !+RHO2*H
      RHS(J,I,K)=RHS(J,I,K)+CRHS
      !
      IF(HHNEW.LE.BT) THEN
            RHS(J,I,K)=RHS(J,I,K)+ RHO2*BOT                         !MOVE RHO2*HNEW FROM HCOF TO RHS  --> RHO2*BOT = RHO2*ZCN  WHICH WOULD CANCEL -RHO2*(GLN+ZCN)
      ELSE
...
C     ------------------------------------------------------------------
     ...
     1                 VOID,PCS,GL,ZC,EST,ESTOLD,SGM,SGS,ITHK,LNWT,NOPCS
      CHARACTER*16 TEXT
      DOUBLE PRECISION RHO1,RHO2,GLN,ZCN,PCTMP,ESTN1,ESTN
C
...
C14-----UPDATE PRECONSOLIDATION HEAD AND OLD EFFECTIVE STRESS ARRAYS
      ...
      IF(HNEW(J,I,K)>BOTM(J,I,K) .AND. IBOUND(J,I,K)>0) THEN
          !
...
C
      ...
      HHNEW=HNEW(IC,IR,K)
      BOT  =BOTM(IC,IR,K)
      IF(HHNEW > BOT) THEN                            !seb
          EST(IC,IR,K)=GL(IC,IR,K)-HHNEW+BOT
      ELSE
          EST(IC,IR,K)=GL(IC,IR,K)
      END IF
...
C  Deallocate SWT MEMORY
      ...
      DEALLOCATE(GWFSWTDAT(IGRID)%NOPCS)
C
