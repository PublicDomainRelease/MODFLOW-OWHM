MODULE PCGN
  ! ... LAST MODIFIED:  SCOTT BOYCE,  OCTOBER, 2018
  ...
  ! ... R.L. Naff
  ! ...
  ! ... VERSION 2.0.4 October, 2018
  ! ...
  ! ... 10/10: VERSION 2.1, FOR USE WITH OWHMv2
  !            IMPROVEMENTS ON OCCIALATIONS AND REFACTORING OF SUBROUTINE EXECUTION SPEED
  !            ADDED STRESS PERID BASED INPUT
  !            ADDED ADDITONIAL OUTPUT FOR IDENTIFYING BAD COEFICENTS
  ...
  IMPLICIT NONE
  !
  PRIVATE:: CYCLING_TEXT_FILE
  !
  TYPE PCGNTYPE
     INTEGER, POINTER:: DIAG_INFO_REWIND
     TYPE(CYCLING_TEXT_FILE), POINTER:: DIAG_INFO
     LOGICAL, POINTER:: DAMP_BY_SP
     INTEGER, POINTER:: CHGLIMIT_BYITER
     INTEGER,         DIMENSION(:), POINTER:: CHGLIMIT_IT
     DOUBLE PRECISION,DIMENSION(:), POINTER:: CHGLIMIT_LIM
     !
  ...
  PRIVATE; PUBLIC :: PCGN2AR, PCGN2AP, PCGN2DA, HCLOSEPCGN, PCGN2RP
  ! ... HCLOSEPCGN NEEDED IN gwf2mnw7.f
CONTAINS

  SUBROUTINE PCGN2AR(IN,FREE_FMT,ITER_MO,IGRID)
    USE UTIL_INTERFACE,    ONLY: READ_TO_DATA, PARSE_WORD_UP, GET_INTEGER, GET_NUMBER, STOP_ERROR
    USE NUM2STR_INTERFACE, ONLY: NUM2STR
    USE CONSTANTS,   ONLY: BLNK
    ! ...
    ! ... ***************************************************************
    ...
    CHARACTER(LEN=768) :: CHAR_STRING  !256
...
300 FORMAT(1X,/1X,'PCG7AR -- PCGN INTERFACE PACKAGE', &
         ', VERSION 2.0, 08/2008')
    !
    ALLOCATE(PCGNDAT(IGRID)%DIAG_INFO_REWIND, SOURCE=0)
    ALLOCATE(PCGNDAT(IGRID)%DAMP_BY_SP, SOURCE=.FALSE.)
    ALLOCATE(PCGNDAT(IGRID)%CHGLIMIT_BYITER, SOURCE=0)
    !
    CALL READ_TO_DATA(CHAR_STRING,IN,IOUT,IOUT)
    DO I=1, 2
      LLOC=1
      CALL PARSE_WORD_UP(CHAR_STRING,LLOC,ISTART,ISTOP)
      IF(CHAR_STRING(ISTART:ISTOP) =='PRINT_COEFFICIENTS' .OR. CHAR_STRING(ISTART:ISTOP) =='PRINT_COEFFICIENT') THEN
         !
         PCGNDAT(IGRID)%DIAG_INFO_REWIND = 2
         ALLOCATE(PCGNDAT(IGRID)%DIAG_INFO)
         CALL PCGNDAT(IGRID)%DIAG_INFO%OPEN(139,LLOC,CHAR_STRING,IOUT,IN, DIM=PCGNDAT(IGRID)%DIAG_INFO_REWIND )
         !
         !CALL PCGNDAT(IGRID)%DIAG_INFO%SET_FMT(     '(I5)',  5,  1 )
         !CALL PCGNDAT(IGRID)%DIAG_INFO%SET_FMT(     '(I5)',  5,  2 )
         !CALL PCGNDAT(IGRID)%DIAG_INFO%SET_FMT(     '(I5)',  5,  3 )
         !CALL PCGNDAT(IGRID)%DIAG_INFO%SET_FMT(     '(I5)',  5,  4 )
         !CALL PCGNDAT(IGRID)%DIAG_INFO%SET_FMT( '(ES14.5)', 14,  5 )
         !CALL PCGNDAT(IGRID)%DIAG_INFO%SET_FMT( '(ES14.5)', 14,  6 )
         !CALL PCGNDAT(IGRID)%DIAG_INFO%SET_FMT( '(ES14.5)', 14,  7 )
         !CALL PCGNDAT(IGRID)%DIAG_INFO%SET_FMT( '(ES14.5)', 14,  8 )
         !CALL PCGNDAT(IGRID)%DIAG_INFO%SET_FMT( '(ES14.5)', 14,  9 )
         !CALL PCGNDAT(IGRID)%DIAG_INFO%SET_FMT( '(ES14.5)', 14, 10 )
         !CALL PCGNDAT(IGRID)%DIAG_INFO%SET_FMT( '(ES14.5)', 14, 11 )
         !CALL PCGNDAT(IGRID)%DIAG_INFO%SET_FMT( '(ES14.5)', 14, 12 )
         !
      ELSEIF(CHAR_STRING(ISTART:ISTOP) =='DAMP_BY_SP') THEN
          PCGNDAT(IGRID)%DAMP_BY_SP =.TRUE.
         !
      ELSEIF(CHAR_STRING(ISTART:ISTOP) =='CHGLIMIT_SPECIFY_BY_ITERATION') THEN
          PCGNDAT(IGRID)%CHGLIMIT_BYITER = 1
          !
          CALL GET_INTEGER(CHAR_STRING,LLOC,ISTART,ISTOP,IOUT,IN,IOS, MSG="PCGN HAS KEYWORD CHGLIMIT_SPECIFY_BY_ITERATION, BUT FAILED TO LOAD THE NUMBER OF CHGLIMIT's THAT WILL BE SPECIFIED.")
          !
          IF(IOS > 0) THEN
                         ALLOCATE( PCGNDAT(IGRID)%CHGLIMIT_IT(IOS+1), PCGNDAT(IGRID)%CHGLIMIT_LIM(0:IOS) )
                         !
                         PCGNDAT(IGRID)%CHGLIMIT_IT(IOS+1) = HUGE(IOS)
                         PCGNDAT(IGRID)%CHGLIMIT_BYITER = IOS
          ELSE
              PCGNDAT(IGRID)%CHGLIMIT_BYITER = 0
          END IF
      ELSE
          EXIT
      END IF
      CALL READ_TO_DATA(CHAR_STRING,IN,IOUT)
    END DO
    !
    BACKSPACE(IN) !RESUME NORMAL PCGN
    ! ...
    ! ... STRIP OUT COMMENT CARDS (#) AND WRITE DATA TO DATA_STRING ARRAY
    ! ...
    L_COUNT=0
    DO
       CALL READ_TO_DATA(CHAR_STRING,IN,IOUT,EOF=EOF)
       !
       IF (EOF) EXIT
       !
       L_COUNT=L_COUNT+1
    ...
    IF(FREE_FMT.EQ.0) THEN
       READ (UNIT=DATA_STRING(1),FMT=400,IOSTAT=IOS) ITER_MO, ITER_MI, &
            CLOSE_R, CLOSE_H
       IF (IOS/=0) THEN
            WRITE (CHAR_STRING,800) 'ITER_MO, ITER_MI, CLOSE_R, CLOSE_H', DATA_STRING(1)
            CALL STOP_ERROR(BLNK,IN,IOUT,CHAR_STRING)
       ENDIF
400    FORMAT (2I10,2F10.0)
       READ (UNIT=DATA_STRING(2),FMT=405,IOSTAT=IOS) RELAX, IFILL, &
            UNIT_PC, UNIT_TS
       IF (IOS/=0) THEN
            WRITE (CHAR_STRING,800) 'RELAX, IFILL, UNIT_PC, UNIT_TS', DATA_STRING(2)
            CALL STOP_ERROR(BLNK,IN,IOUT,CHAR_STRING)
       ENDIF
405    FORMAT (F10.0,3I10)
       IF (ITER_MO>1) THEN
          ...
          IF (IOS/=0) THEN
               WRITE (CHAR_STRING,800) 'ADAMP, DAMP, DAMP_LB, RATE_D, CHGLIMIT', DATA_STRING(3)
              CALL STOP_ERROR(BLNK,IN,IOUT,CHAR_STRING)
          ENDIF
410       FORMAT (I10,4F10.0)
          ...
          READ (UNIT=DATA_STRING(4),FMT=415,IOSTAT=IOS) ACNVG, CNVG_LB, &
               MCNVG, RATE_C, IPUNIT
          IF (IOS/=0) THEN
               WRITE (CHAR_STRING,800) 'ACNVG, CNVG_LB, MCNVG, RATE_C, IPUNIT', DATA_STRING(4)
              CALL STOP_ERROR(BLNK,IN,IOUT,CHAR_STRING)
          ENDIF
415       FORMAT (I10,F10.0,I10,F10.0,I10)
       ELSE
          ...
          PCGNDAT(IGRID)%DAMP_BY_SP = .FALSE.
       ENDIF
    ELSE
       READ (UNIT=DATA_STRING(1),FMT=*,IOSTAT=IOS) ITER_MO,ITER_MI, &
            CLOSE_R,CLOSE_H
       IF (IOS/=0) THEN
            WRITE (CHAR_STRING,800) 'TER_MO,ITER_MI,CLOSE_R,CLOSE_H', DATA_STRING(1)
              CALL STOP_ERROR(BLNK,IN,IOUT,CHAR_STRING)
       ENDIF
       ...
       IF(IFILL.NE.1 .AND. IFILL.NE.0)THEN
              CALL STOP_ERROR(DATA_STRING(2),IN,IOUT,'PCGN ERROR IFILL NOT EQUAL TO 0 OR 1. IT WAS SET TO: '//NUM2STR(IFILL))
          !WRITE (IOUT,'(A)') 'PCGN ERROR IFILL NOT EQUAL TO 0 OR 1'
          !WRITE(*,'(A)') 'PCGN ERROR IFILL NOT EQUAL TO 0 OR 1'
          !ERROR STOP
       END IF
       IF (ITER_MO>1) THEN
          ! ... NONLINEAR DAMPING
...
450 FORMAT (1X,'TOTAL ARRAY STORAGE REQUIRED IS ',I7)
    !
    !
    ! IF REQUESTED READ CHGLIMIT_BYITER
    IF(PCGNDAT(IGRID)%CHGLIMIT_BYITER > 0) THEN
        PCGNDAT(IGRID)%CHGLIMIT_LIM(0) = CHGLIMIT
        DO I=1, PCGNDAT(IGRID)%CHGLIMIT_BYITER
            !
            CALL READ_TO_DATA(CHAR_STRING,IN,IOUT)
            !
            LLOC = 1
            CALL GET_INTEGER(CHAR_STRING,LLOC,ISTART,ISTOP,IOUT,IN,PCGNDAT(IGRID)%CHGLIMIT_IT(I), MSG="PCGN HAS KEYWORD CHGLIMIT_SPECIFY_BY_ITERATION, BUT FAILED TO LOAD THE ITERATION THAT THE CHGLIMIT IS APPLIED TOO.")
            CALL GET_NUMBER (CHAR_STRING,LLOC,ISTART,ISTOP,IOUT,IN,PCGNDAT(IGRID)%CHGLIMIT_LIM(I), MSG="PCGN HAS KEYWORD CHGLIMIT_SPECIFY_BY_ITERATION, BUT FAILED TO LOAD THE CHGLIMIT.")
        END DO
    END IF
    ! ...
...
800 FORMAT (1X,/,1X,10X,'***BAD DATA READ, SUBROUTINE PCGNRP***',/, &
         1X,8X,'VARIABLES READ WERE: ',A,/, &
         1X,8X,'DATA READ WERE: ', A)
810 FORMAT (1X,/,1X,20X,'***WARNING!!!***',/, &
  ...
  SUBROUTINE PCGN2RP(IN, KPER, IGRID, IOUT, ITER_MO)
    !
    INTEGER, INTENT(IN   ) :: IN, KPER, IGRID, IOUT
    INTEGER, INTENT(INOUT) :: ITER_MO
    INTEGER:: ITMP
    !
    IF(PCGNDAT(IGRID)%DAMP_BY_SP) THEN
      IF(KPER.NE.1) THEN
        IF(IGRID==1) THEN
            CALL PCGN2_READ_DAMPLINE(PCGNDAT(IGRID),IN, ITER_MO, IOUT, KPER)
            PCGNDAT(IGRID)%MO_ITER=ITER_MO
        ELSE
            PCGNDAT(IGRID)%MO_ITER=ITER_MO
            ITMP = ITER_MO
            CALL PCGN2_READ_DAMPLINE(PCGNDAT(IGRID),IN, ITMP, IOUT, KPER)
        END IF
      END IF
    END IF
    !
    END SUBROUTINE
    !
    SUBROUTINE PCGN2_READ_DAMPLINE(PNT,IN,ITER_MO,IOUT,KPER)
    USE CONSTANTS,          ONLY: NL, TRUE, FALSE
    USE UTIL_INTERFACE,     ONLY: READ_TO_DATA, GET_INTEGER, GET_NUMBER, WARNING_MESSAGE
    USE NUM2STR_INTERFACE,  ONLY: NUM2STR
    TYPE(PCGNTYPE), INTENT(INOUT):: PNT
    INTEGER, INTENT(IN   ) :: IN, IOUT, KPER
    INTEGER, INTENT(INOUT) :: ITER_MO
    CHARACTER(512):: LINE
    INTEGER:: LLOC, ISTART, ISTOP, MXITER
    LOGICAL:: ERR
    !
    CALL READ_TO_DATA(LINE,IN,EOF=ERR)
    !
    IF(ERR) THEN
        CALL WARNING_MESSAGE(INFILE=IN,OUTPUT=IOUT,MSG='PCGN "DAMP_BY_SP" OPTION REACHED THE END OF THE PCGN FILE FOR SP '//NUM2STR(KPER)//NL//  &
                                                         'THE PREVIOUSLY LOADED ITER_MO, ADAMP, DAMP, DAMP_LB, RATE_D, CHGLIMIT'//NL//   &
                                                         'WILL BE USED FOR THE REMAINDER OF THE SIMULATION.'//NL//                       &
                                                         'ITER_MO  = '//NUM2STR(ITER_MO)      //NL//                                     &
                                                         'ADAMP    = '//NUM2STR(PNT%DAMP_A)   //NL//                                     &
                                                         'DAMP     = '//NUM2STR(PNT%SAV_DAMP) //NL//                                     &
                                                         'DAMP_LB  = '//NUM2STR(PNT%MIN_DAMP) //NL//                                     &
                                                         'RATE_D   = '//NUM2STR(PNT%D_RATE)   //NL//                                     &
                                                         'CHGLIMIT = '//NUM2STR(PNT%LIMHDCHG) , INLINE=TRUE)
        PNT%DAMP_BY_SP = FALSE
    ELSE
       LLOC=1
       CALL GET_INTEGER(LINE,LLOC,ISTART,ISTOP,IOUT,IN,MXITER, HAS_ERROR=ERR)
       IF(ERR) THEN
          !MSG='FOUND KEYWORD DAMP_BY_SP, BUT FAILED TO LOAD ITER_MO FOR STRESS PERIOD.'
          !CALL WARNING_MESSAGE(INFILE=IN,OUTPUT=IOUT,MSG='PCGN "DAMP_BY_SP" OPTION FAILED TO LOAD ITER_MO FOR SP '//NUM2STR(KPER)//NL//'THE PREVIOUSLY LOADED ITER_MO, ADAMP, DAMP, DAMP_LB, RATE_D, CHGLIMIT'//NL//'WILL BE USED FOR THE REMAINDER OF THE SIMULATION.', INLINE=TRUE)
          PNT%DAMP_BY_SP = FALSE
       ELSE
          ITER_MO = MXITER
          CALL GET_INTEGER(LINE,LLOC,ISTART,ISTOP,IOUT,IN,PNT%DAMP_A, MSG='PCGN HAS KEYWORD DAMP_BY_SP, BUT FAILED TO LOAD ADAMP FOR STRESS PERIOD.')
          CALL GET_NUMBER(LINE,LLOC,ISTART,ISTOP,IOUT,IN,PNT%SAV_DAMP,MSG='PCGN HAS KEYWORD DAMP_BY_SP, BUT FAILED TO LOAD DAMP FOR STRESS PERIOD.')
          CALL GET_NUMBER(LINE,LLOC,ISTART,ISTOP,IOUT,IN,PNT%MIN_DAMP,MSG='PCGN HAS KEYWORD DAMP_BY_SP, BUT FAILED TO LOAD DAMP_LB FOR STRESS PERIOD.')
          CALL GET_NUMBER(LINE,LLOC,ISTART,ISTOP,IOUT,IN,PNT%D_RATE,  MSG='PCGN HAS KEYWORD DAMP_BY_SP, BUT FAILED TO LOAD RATE_D FOR STRESS PERIOD.')
          CALL GET_NUMBER(LINE,LLOC,ISTART,ISTOP,IOUT,IN,PNT%LIMHDCHG,MSG='PCGN HAS KEYWORD DAMP_BY_SP, BUT FAILED TO LOAD CHGLIMIT FOR STRESS PERIOD.')
       END IF
    END IF
    !
    END SUBROUTINE
  ! ... *****************************************************************
    ...
    ENDIF
    !
    ! SET UP VAIRABLE CHGLIMIT
    !
    IF(PT%CHGLIMIT_BYITER > 0) THEN
      !
      ASSOCIATE( POS=>PT%CHGLIMIT_BYITER, ITER => PT%CHGLIMIT_IT, CHGLIMIT=>PT%CHGLIMIT_LIM )
        IF(KITER == 1) THEN
            POS = 1
            PT%LIMHDCHG = PT%CHGLIMIT_LIM(0)
        END IF
        !
        IF(ITER(POS) <= KITER) THEN
            PT%LIMHDCHG = CHGLIMIT(POS)
            POS = POS + 1
        END IF
      END ASSOCIATE
    END IF
    ...
    !
    IF (NCOL>1) THEN
        DO CONCURRENT(I=1:NODES); PT%DX(I) = ZERO
        END DO
    ELSE
                           PT%DX(1) = ZERO
    ENDIF
    IF (NROW>1) THEN
        DO CONCURRENT(I=1:NODES); PT%DY(I) = ZERO
        END DO
    ELSE
                           PT%DY(1) = ZERO
    ENDIF
    IF (NLAY>1) THEN
        DO CONCURRENT(I=1:NODES); PT%DZ(I) = ZERO
        END DO
    ELSE
                           PT%DZ(1) = ZERO
    ENDIF
    ...
    IF(PCGNDAT(IGRID)%DIAG_INFO_REWIND > 0) THEN
        !
        CALL PRINT_MATRIX_INFO(PCGNDAT(IGRID)%DIAG_INFO, PCGNDAT(IGRID)%DIAG_INFO_REWIND, KKITER, KKSTP,KKPER, PT%DD, PT%DX, PT%DY, PT%DZ, PT%RES)
        !
    END IF
    ICNVG=0
    ...
    ELSE
       CALL NONLINEAR(PT%MO_ITER,PT%MI_ITER,PT%CNVG_A,PT%DAMP_A, &
            ...
            NITER(IGRID),ICNVG,ERR_STAT,INIT)
    ENDIF
    IF (ERR_STAT(1)/=0) THEN
       IF (ERR_STAT(1)==10) THEN
            WRITE(IOUT,500) ERR_STAT(2)
...
525   FORMAT (/,1X,' RESIDUAL L2 NORM AT CONVERGENCE: ',ES12.5)
530   FORMAT (/,1X,' RESIDUAL L2 NORM AT TERMINATION: ',ES12.5)
      ! ... -------------------------------------------------------------
  ...
  ! ... ***************************************************************
    SUBROUTINE NONLINEAR(MO_ITER,MI_ITER,CNVG_A,DAMP_A,IPUNIT,IGRID, &
         ...
         LIMHDCHG,MIN_DAMP,MIN_CLOSE,RTR,MRKR,NITER,ICNVG,ERR_STAT,INIT)
      ...
      DOUBLE PRECISION :: EPS_O, EPS_IP, L2HR, RATIO_L, HLAST, HTHIS
      ...
      NO_CNVG = .FALSE.
      !
      IF (SQRT(RTR)<REAL(RCLOSE) .AND. KITER>1) THEN
      ...
      IF (KSTP==1.AND.KITER==1) THEN
         ...
         NO_ITER=MI_ITER
         !PT%HCH=ZERO
         DO CONCURRENT(I=1:NODES); PT%HCH(I) = ZERO
         END DO
         ...
         CALL MAX_HCH_FAST(RTR, MXHCH, MHC_NODE, IB0_COUNT, L2HR, NODES, PT%HCH, IBOUND)
         CALL DAMP_ADJ(DAMP_A,KITER,KSTP,MIN_DAMP,SAV_DAMP,L2HR,MXHCH, &
              D_RATE,LIMHDCHG,DDAMP,NO_CNVG,MO_ITER)
         ! ... UPDATE SOLUTION
         IF (IPUNIT>0) HLAST=HNEW(MHC_NODE)
         CALL UPDATE_HEAD(NODES,HNEW,PT%HCH,DDAMP)
...
500   FORMAT (/,1X,' CONVERGENCE DECLARED BASED ON MAXIMUM L2 NORM &
           ...
           ES12.5,/,1X,' MAXIMUM HEAD CHANGE AT CONVERGENCE: ',ES12.5)
530   FORMAT (/,1X,I7,' TOTAL DRY CELLS BY TIME STEP',I4, &
  ...
  SUBROUTINE CDS_RES(RTR, HNOFLO, IPUNIT, DD, DX, DY, DZ, RES)
    ...
    DOUBLE PRECISION, DIMENSION(:), CONTIGUOUS, INTENT(OUT) :: DD, DX, DY, DZ, RES
    ! ... ------------------------------------------------------------------
    ! ...    LOCAL VARIABLES
    ! ... ------------------------------------------------------------------
    ...
    ENDDO
    !
    !IF(PCGNDAT(IGRID)%DIAG_INFO%IS_OPEN) THEN
    !
    !
    !END IF
500 FORMAT(1X,'PCGN ASSEMBLER HAS DISCOVERED AN ADDITIONAL INACTIVE CELL'&
  ...
  ! ... ***************************************************************
  SUBROUTINE PRINT_MATRIX_INFO(FL, IREWIND, KITER, KSTP, KPER, DD, DX, DY, DZ, RES)
    ! ... ------------------------------------------------------------------
    TYPE(CYCLING_TEXT_FILE):: FL
    INTEGER, INTENT(IN) :: IREWIND, KITER, KSTP, KPER
    DOUBLE PRECISION, DIMENSION(:), CONTIGUOUS, INTENT(IN) :: DD, DX, DY, DZ, RES
    ! ... ------------------------------------------------------------------
    ! ...    LOCAL VARIABLES
    ! ... ------------------------------------------------------------------
    INTEGER :: I, J, K, N, NRC, LEVEL_CT, ROW_CT
    ! ... ==================================================================
    !
    IF(KITER==1) THEN
        WRITE(FL%REC,'(A, I4, I3)') ' ITER  LAY  ROW  COL          HNEW          HCOF           RHS     MAIN_DIAG      ROW_DIAG      COL_DIAG      LAY_DIAG      RESIDUAL',KPER, KSTP
        CALL FL%WRITE()
    END IF
    !
    NRC=NROW*NCOL
       ...
       DO I=1,NROW
          ...
          DO J=1,NCOL
             ...
             IF (IBOUND(N).GT.0)  THEN
                 WRITE(FL%REC,'(4I5, 8ES14.5)') KITER,K,I,J,HNEW(N),HCOF(N),RHS(N),DD(N),DX(N),DY(N),DZ(N),RES(N)
                 CALL FL%WRITE()
             END IF
          ENDDO
       ENDDO
    ...
    !
    !IF(PCGNDAT(IGRID)%DIAG_INFO%IS_OPEN) THEN
    !         WRITE(96,'(3I5,*(ES14.5))') K,I,J,HNEW(N),HCOF(N),RHS(N),DD(N),DX(N),DY(N),DZ(N),RES(N)
    !
    !END IF
!500 FORMAT(1X,'PCGN ASSEMBLER HAS DISCOVERED AN ADDITIONAL INACTIVE CELL'&
!         ,/,' MESH LOCATION: J=',I4,', I=',I4,', K=',I4)
  ...
  PURE SUBROUTINE MAX_HCH_FAST(RTR, MXHCH, MHC_NODE, IB0_COUNT, L2HR, DIM, HCH, IBND)
    ! ...
    ! ... ******************************************************************
    ! ... PURPOSE: EXTRACT LARGEST ABSOLUTE HEAD CHANGE;
    ! ... FORM HEAD-CHANGE L2 NORM; PRINT MESSAGE
    ! ... ******************************************************************
    USE CONSTANTS, ONLY: Z, TWO, D40, D50
    DOUBLE PRECISION,                 INTENT(IN ):: RTR
    DOUBLE PRECISION,                 INTENT(OUT):: MXHCH, L2HR
    INTEGER,                          INTENT(OUT) :: MHC_NODE, IB0_COUNT
    INTEGER,                          INTENT(IN ):: DIM
    DOUBLE PRECISION, DIMENSION(DIM), INTENT(IN ):: HCH
    INTEGER,          DIMENSION(DIM), INTENT(IN ):: IBND
    ! ... --------------------------------------------------------------
    ! ...    LOCAL VARIABLES
    ! ... --------------------------------------------------------------
    INTEGER :: N, NODE_SAVE
    ...
    MHC_NODE=NODE_SAVE
    !
    IF( HTH >= D40 .OR. RTR >= D40) THEN
        L2HR=D50
    ELSEIF(HTH*RTR >= D40) THEN
        L2HR=D50
    ELSE
        L2HR=SQRT(HTH*RTR)
    END IF
...
500 FORMAT (/,1X,' CONVERGENCE DECLARED BASED ON MAXIMUM HEAD CHANGE &
         &CRITERION')
510 FORMAT (1X,' MAXIMUM HEAD CHANGE AT CONVERGENCE: ',ES12.5)
520 FORMAT (6X,' ***PROVISIONAL CONVERGENCE***',/,1X, &
         ' MAXIMUM HEAD CHANGE AT CONVERGENCE: ',ES12.5, &
...
530 FORMAT (1X,' RESIDUAL L2 NORM AT CONVERGENCE: ',ES12.5)
540 FORMAT (/,1X,' RESIDUAL L2 NORM AT TERMINATION: ',ES12.5)
    ! ... -----------------------------------------------------------
  ...
  SUBROUTINE DAMP_ADJ(DAMP_A,KITER,KSTP,MIN_DAMP,SAV_DAMP,EPS,MXHCH, &
       D_RATE,CHGLIMIT,DAMP,NO_CNVG,MXITER)
    ! ...
    ...
    ! ...    ARGUMENT LIST
    ! ... ---------------------------------------------------------
    USE CONSTANTS, ONLY: DECIMILLI, FIFTH, D40, FALSE, TRUE
    INTEGER, INTENT(IN) :: DAMP_A,KITER, KSTP, MXITER
    LOGICAL, INTENT(INOUT):: NO_CNVG
    ...
    ! ... ---------------------------------------------------------
    INTEGER :: COUNT=0
    DOUBLE PRECISION :: RATIO_E, RATIO_H, RATIO_L, CHG_DMP
    ...
    CASE(1)
       ...
       !
       ! HEAD CHANGE DAMP
       IF (ABS(MXHCH)>CHGLIMIT) THEN
           CHG_DMP=CHGLIMIT/ABS(MXHCH)
           IF(CHG_DMP > DAMP .AND. DAMP > MIN_DAMP) CHG_DMP = DAMP
       ELSE
           CHG_DMP = D40  !NEVER WILL BE APPLIED
       END IF
       !
       ! ... LIMIT HEAD CHANGE
       IF (CHG_DMP < MIN_DAMP) THEN
          !
          IF (DAMP<CHG_DMP) THEN
              DAMP = (CHG_DMP + DAMP)*HALF
          ELSE
              DAMP=CHG_DMP
          END IF
          !
          IF (DAMP<DECIMILLI) THEN
              DAMP = DECIMILLI  !0.0001
              IF( KITER < MXITER - (MXITER/4) ) NO_CNVG = TRUE  !TOO MUCH DAMPENING DO NOT ALLOW CONVERGENCE --ONLY APPLY WHEN NOT NEAR MXITER
          END IF
          !
       ELSEIF (EPS >= D40 .AND. DAMP<MIN_DAMP) THEN !WIlD OSSCILLATION IMPOSE STRONG DAMPENING
           !
           IF (DAMP<DECIMILLI) DAMP = DECIMILLI
           !
           NO_CNVG = TRUE                           !WILD OSCCILATION DO NOT ALLOW CONVERGENCE --NOTE ICVG SHOULD NEVER BE TRUE
           !
       ! ... LOWER BOUND CHECK
       ELSEIF (DAMP<MIN_DAMP) THEN
          DAMP=MIN_DAMP
          COUNT=COUNT+1
          IF (COUNT>10) THEN
             ...
             !
          ELSEIF(MIN_DAMP < FIFTH) THEN !PREVENT OVER DAMPENING THAT CAUSES PREMATURE CONVERGENCE PASS
              !
              IF(KITER < MXITER - (MXITER/4)) NO_CNVG = TRUE
          ENDIF
       ENDIF
    CASE(2)
...
505 FORMAT(/,1X,' WARNING: RATIO_E NEGATIVE IN SUBROUTINE NONLINEAR', &
         /,10X,' OUTER ITERATION:', I5, ' RATIO_E:',ES12.5)
    ! ... -----------------------------------------------------------
...
515 FORMAT (/,1X,' PARTITION',I3,' IS CONTAINED IN FOLLOWING REGION:', &
  ...
  SUBROUTINE RENUMBER (CK_D, NO_NODES,NEW_NO,OLD_NO)
    INTEGER, DIMENSION(:), CONTIGUOUS, INTENT(INOUT) :: CK_D
  ...
  SUBROUTINE PCGN2DA(IGRID)
    ...
    !
    IF (ASSOCIATED(PCGNDAT(IGRID)%DIAG_INFO_REWIND))  THEN
        IF (PCGNDAT(IGRID)%DIAG_INFO_REWIND > 0) THEN              ! GFORTRAN compiler error work-around for pointer data type FINAL statement
            BLOCK
                TYPE(CYCLING_TEXT_FILE), POINTER:: DIAG_INFO
                DIAG_INFO=>PCGNDAT(IGRID)%DIAG_INFO
                PCGNDAT(IGRID)%DIAG_INFO=>NULL()
                DEALLOCATE(DIAG_INFO)
                DIAG_INFO=>NULL()
            END BLOCK
        END IF
        !IF (PCGNDAT(IGRID)%DIAG_INFO_REWIND > 0) DEALLOCATE(PCGNDAT(IGRID)%DIAG_INFO)
        DEALLOCATE(PCGNDAT(IGRID)%DIAG_INFO_REWIND)
    END IF
    !
    IF (ASSOCIATED(PCGNDAT(IGRID)%DAMP_BY_SP))      DEALLOCATE(PCGNDAT(IGRID)%DAMP_BY_SP,STAT=I)
    !
    IF (ASSOCIATED(PCGNDAT(IGRID)%CHGLIMIT_BYITER)) DEALLOCATE(PCGNDAT(IGRID)%CHGLIMIT_BYITER,STAT=I)
    IF (ASSOCIATED(PCGNDAT(IGRID)%CHGLIMIT_IT))     DEALLOCATE(PCGNDAT(IGRID)%CHGLIMIT_IT,    STAT=I)
    IF (ASSOCIATED(PCGNDAT(IGRID)%CHGLIMIT_LIM))    DEALLOCATE(PCGNDAT(IGRID)%CHGLIMIT_LIM,   STAT=I)

    !CLOSE(IOUT) ! seb commented out closing IOUT since it is closed by the BAS
...
100 FORMAT(I9,', ',I9,', ',  F7.5,', ',G12.5,', ',G14.7, &
  ...
  PURE SUBROUTINE UPDATE_HEAD(DIM,Y,X,R)
    ! ... MODEL: Y=Y+R*X
    ! ... Y, X: ARRAYS
    ! ... R: SCALAR
    INTEGER, INTENT(IN):: DIM
    DOUBLE PRECISION, DIMENSION(DIM), INTENT(INOUT) :: Y
    DOUBLE PRECISION, DIMENSION(DIM), INTENT(IN   ) :: X
    DOUBLE PRECISION, INTENT(IN) :: R
    INTEGER :: II
    DO CONCURRENT (II=1:DIM)
                       Y(II)=Y(II)+R*X(II)
    END DO
    ! ...
