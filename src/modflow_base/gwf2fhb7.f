     ...
     1                           NFHBX2,IFHBSS
      ...
      SUBROUTINE GET_FHB_UNIT(LINE, IN, IOUT, LLOC, IU)
      IMPLICIT NONE
      CHARACTER(*):: LINE
      INTEGER:: IN, IOUT, LLOC, IU
      INTEGER::ISTART,ISTOP,N
      REAL:: R
      !
      LLOC=1
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,1,N,R,0,IN)
      IF( LINE(ISTART:ISTOP)=='INTERNAL') THEN
          IU=IN
      ELSE
          LLOC=1
          CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,IU,R,0,IN)
      END IF
      !
      END SUBROUTINE
...
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
      USE GLOBAL,      ONLY:IOUT,ITRSS,IBOUND,NOCBC
...
C1------READ TIMES AT WHICH SPECIFIED FLOW AND HEAD VALUES WILL BE READ
      CALL READ_TO_DATA(LINE,IN,IOUT)
      CALL GET_FHB_UNIT(LINE, IN, IOUT, LLOC, IFHBUN)
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,3,N,CNSTM,IOUT,IN)
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,IFHBPT,R,IOUT,IN)
