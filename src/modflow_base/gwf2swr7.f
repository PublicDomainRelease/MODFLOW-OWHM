...
C         SSWR_P_QMFLOW
C
C     VERSION 1.03 SWR1 for MODFLOW OWHM
C     CHANGES
...
C
C     VERSION 1.04 SWR1 for MODFLOW-2005 (1.12) AND MODFLOW OWHM
C     CHANGES
...
!      USE OMP_LIB 
     ...
     2                      NPER,PERLEN,NSTP,TSMULT,NOCBC
...
C-------WRITE REACH GROUP REACH DATA
     ...
     2        (RCHGRP(i)%REACH(j),j=1,RCHGRP(i)%NRGREACH)
      END DO
C
C-------GET TOTAL SIMULATION TIME FOR MODEL  -- SCOTT WHY ARE YOU CALL SSWR_GET_MODFLOW_TIME REPEATLY WHEN CALL SSWR_GET_MODFLOW_TIME(NPER,NSTP(NPER),rt,RTOTIM) DOES THE SAME THING
      RTOTIM = RZERO
      DO n = 1, NPER
        DO i = 1, NSTP(n)
...
C-------READ REST OF STRESS PERIOD DATA FLAGS - INPUT ITEM 5
      ...
      IF ( NAUX.GT.0 ) THEN
        CALL URWORD(line, lloc, istart, istop, 2, IRDAUX, r,0, In)
      END IF
...
C               TEST USE OF STRLEN AND STRLEN2 FOR UNCONTROLLED FLOW STRUCTURES - JDH 11/28/2012
              CALL URWORD(line,lloc,istart,istop,3,ival,r,0,iut)
              IF ( r.LE.RZERO ) THEN
...
C           ADDITIONAL BOUNDARY STRUCTURE DATA FOR COUPLING TO SFR
          IF ( istrconn.EQ.0 .AND. ISFRUNIT.NE.0 ) THEN
            CALL URWORD(line,lloc,istart,istop,2,ival,r,IOUT,iut)
...
C       IN THE CURRENT STRESS PERIOD
     ...
     2           (RONE - TSMULT(Kkper)**NSTP(Kkper))
        ...
        ELSE
          dt = dt * TSMULT(Kkper)
        END IF
        IF(SPTIM(Kkper)%SPECIFY_DELT) dt = SPTIM(Kkper)%DT(n)
        stpend = stpend + dt
...
C---------Check for and decode EXTERNAL and OPEN/CLOSE records.
        IF ( Iclose.EQ.0 ) THEN
          ...
          ELSE IF( line(istart:istop).EQ.'OPEN/CLOSE' ) THEN
             ...
             OPEN(NEWUNIT=nunopn,FILE=fname,ACTION=ACTION(1))
             Iut = nunopn
...
2005    FORMAT(1X,/1X,'OPENING FILE ON UNIT ',I4,':',/1X,A)
2010    FORMAT(1X,'LIST ADJUSTMENT FACTOR=',ES12.5)
2015    FORMAT(1X,'LIST    SCALING FACTOR=',ES12.5)
...
C2--------Check for and decode EXTERNAL and OPEN/CLOSE records.
        ...
        ELSE IF( line(istart:istop).EQ.'OPEN/CLOSE' ) THEN
           ...
           fname=line(istart:istop)
           OPEN(NEWUNIT=nunopn,FILE=fname,ACTION=ACTION(1))
           IF( IPTFLG.EQ.1 ) WRITE(Iout,2005) nunopn, fname
           in=nunopn
           iclose=1
        ELSE
          BACKSPACE(in)
...
C     + + + CODE + + +
        ...
        iut = ABS( Iu )
C---------REWIND THE FILE
        CALL UTF8_BOM_OFFSET_REWIND(iut)
        IF ( Iu.GT.0 ) THEN
          READ (iut,'(A200)') line
        ELSE
          READ (iut) itmp
...
2100  FORMAT(1X,42('-'))
2110  FORMAT(1X,ES15.6,1X,I10,1X,ES15.6)
2120  FORMAT(/1X,2X,'PERCENT DISCREPANCY =',1X,ES15.6)
C     + + + CODE + + +
C
...
C       + + + CODE + + +
        ...
        e     = SQRT( EPSILON(DZERO) )
        av    = ABS( V )
        IF ( av .NE. av ) av = DONE   !TEST FOR NaN
...
9999    RETURN
      ...
      SUBROUTINE SSWR_GET_MODFLOW_TIME(Nmaxper,Ngetstp,Rt,Rtot)
        ...
        USE GLOBAL,       ONLY: PERLEN,NSTP,TSMULT, SPTIM
        IMPLICIT NONE
C       + + + DUMMY ARGUMENTS + + +
...
C       + + + CODE + + +
     ...
     2              ( RONE - TSMULT(n)**NSTP(n) )
        ...
        END DO LPER
        !
        IF(SPTIM(Nmaxper)%SPECIFY_DELT) Rt = SPTIM(Nmaxper)%DT(Ngetstp)
C---------RETURN
9999    RETURN
