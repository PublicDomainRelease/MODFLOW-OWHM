...
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
      USE CONSTANTS,   ONLY: Z, TRUE, FALSE, SNGL_ninf
      USE GLOBAL,      ONLY:IOUT,NCOL,NROW,NLAY,ISSFLG,NPER,NSTP,HNEW,
     1                      DELR,DELC,BOTM,LBOTM,SUBLNK,LAYCBD,NOCBC !,BUFF 
     ...
     5                      LPFLNK,DVZ,DELAY_HED,DELAY_HED_ID,         !SUB-Linkage rth
     6                      DVZC,NOCOMV,SEPARTE_FLOWS                   !WSCHMID
      USE UTIL_INTERFACE, ONLY:READ_TO_DATA, PARSE_WORD_UP, GET_INTEGER,
     1                      STOP_ERROR,WARNING_MESSAGE 
...
C1------IDENTIFY PACKAGE.
        WRITE(IOUT,1)IN
    1 FORMAT(/,'SUB7 -- SUBSIDENCE PACKAGE, VERSION 8,',
     1     ' 03/28/2018',' INPUT READ FROM UNIT',I3)
C
...
C4------CRITICAL HEAD ARRAYS.
      ...
      DO
         ...
         CASE('PARAMETER')
            LLOC = 10
            CALL URWORD(BL%LN,LLOC,ISTART,ISTOP,2,NSBP,R,IOUT,IN)        !seb NSBP is the number subscidence parameters
            CALL URWORD(BL%LN,LLOC,ISTART,ISTOP,2,IPRNTFLG,R,IOUT,IN)    !seb IPRNTFLG is print flag for UPARARRSUB1
         CASE('OPTION','OPTIONS')
            CALL BL%START()
            DO I=1,BL%NLINE
                LLOC = 1
                ...
                CASE('PARAMETER')
                   LLOC = 10
                 CALL URWORD(BL%LINE,LLOC,ISTART,ISTOP,2,NSBP,R,IOUT,IN)        !seb NSBP is the number subscidence parameters
                  CALL URWORD(BL%LINE,LLOC,ISTART,ISTOP,2,IPRNTFLG,R,
     +                        IOUT,IN)    !seb IPRNTFLG is print flag for UPARARRSUB1
                CASE DEFAULT
...
!      CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,ILPFLNK,R,IOUT,IN)                !SUB-Linkage rth  seb commented out
      ...
      ENDIF
cc
      IF(IGRID.EQ.1.AND.ISUBLNK.gt.Z)then                               !SUB-Linkage rth
...
C5------IF CELL-BY-CELL TERMS TO BE SAVED THEN PRINT UNIT NUMBER.
   70 IF(IIBSCB.GT.Z) WRITE(IOUT,80) IIBSCB
 ...
 100    Continue
      ...
      DO K=1,NLAY                                                       !IF LAYER IS CONFINED UPDATE ITS CONDUCTANCE
        ...
        ELSEIF (LAYHDT(K)==0 .AND. IHUF.NE.0) THEN
                  CALL USTOP('ERROR: IF USING SUBLINK WITH HUF, '//
