...
C     ------------------------------------------------------------------
      ...
      USE UTIL_INTERFACE, ONLY: STOP_ERROR, READ_TO_DATA
      USE DE4MODULE
      CHARACTER*700 LINE
C     ------------------------------------------------------------------
...
C3------ZERO OR NEGATIVE VALUES, SUBSTITUTE THE DEFAULT VALUE.
      ...
      CALL READ_TO_DATA(LINE,INDE4,IOUT,IOUT)
      LLOC=1
...
C1------READ IFREQ, MUTD4, ACCL, HCLOSE, AND IPRD4
      ...
      CALL READ_TO_DATA(LINE,INDE4,IOUT,IOUT)
      LLOC=1
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,IFREQ,R,IOUT,INDE4)
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,MUTD4,R,IOUT,INDE4)
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,3,I,ACCLDE4,IOUT,INDE4)
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,3,I,HCLOSEDE4,IOUT,INDE4)
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,IPRD4,R,IOUT,INDE4)
   ...
   11    FORMAT(1X,/1X,'INVALID VALUE FOR IFREQ PARAMETER:',I8)
         !CALL USTOP(' ')
      CALL STOP_ERROR(INFILE=INDE4,OUTPUT=IOUT,MSG=TRIM(LINE))
      END IF
C
...
C
      ReAL(KND):: HNEW,EE,COND,RR,DDH,RHS,HCOF
      ReAL(KND):: CC, CR, CV
C
...
C2------Jour., June 1974, p. 295-308.
      ...
      !CALL USTOP(' ')
      CALL STOP_ERROR(
     +  MSG='DE4 - SDE47N - ORDER EQUATIONS USING D4 ORDERING '//
     +      'UNKNOWN ERROR WITH FLAG ID4DIR')
C
...
C
   ...
   10   FORMAT((2X,3(2X,ES10.3,' (',I3,',',I3,',',I3,')')))
      ELSE
          WRITE(IOUT,15)
   ...
   20   FORMAT((2X,2(2X,ES10.3,' (',I3,',',I5,',',I5,')')))
      ENDIF
        WRITE(IOUT,30)
   30 FORMAT(1X,/1X)
C
