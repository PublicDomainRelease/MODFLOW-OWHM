...
C seb
        ...
        if (ierr /= 0) CALL USTOP("allocate error")
        tmp(1:nold) = a(1:nold)
        deallocate (a)
...
c
     ...
     [           iblck, id, kk, nred, ierror
      ...
      if (ierror /= 0) CALL USTOP( "== not enough memory (xmdsfacl) ==")
...
c
     ...
     [        maxint, iblck, id, kk, idk, idkk
      ...
      if (ierror /= 0) CALL USTOP( "== not enough memory (xmdsfacd) ==")
...
c
      ...
      if (ierror /= 0) CALL USTOP( "== not enough memory (xmdordng) ==")
...
c
      ...
      if(ierror/= 0) CALL USTOP("== not enough memory (xmdRedBlack) ==")
...
C seb
     ...
     [          sb(nblack), stat = ierror )
      if (ierror /= 0) CALL USTOP("== not enough memory (xmdcgstb) ==")
...
c      total      5*nblack
     ...
     [          v(nblack), stat = ierror )
      if (ierror /= 0) CALL USTOP( "== not enough memory (xmdcnjgd) ==")
...
c
      ...
      if (ierror /= 0) CALL USTOP( "== not enough memory (xmdnfctr) ==")
...
C seb
      ...
      if (ierror /= 0) CALL USTOP( "== not enough memory (xmdnfac) ==")
...
c       total    2*nblack + 2*(north+1)*nblack + north
     ...
     [          res(nblack), stat = ierror )
      if(ierror /= 0) CALL USTOP( "== not enough memory (xmdorthmn) ==")
...
C seb
      ...
      if (ierror /= 0) CALL USTOP( "== not enough memory (xmdsolv) ==")
...
c
     ...
     +         stat = ierror )
      END IF
      if (ierror /= 0) CALL USTOP( "== not enough memory (xmdprpc) ==")
...
c
     ...
     [               iblackend, nn, nja, nblack, ierr, redsys)
      ...
      if(ierror/= 0)CALL USTOP("== not enough memory (xmdprpc - af) ==")
