...
c        specifications:
c     ------------------------------------------------------------------
      USE GLOBAL,            ONLY: IOUT,NCOL,NROW,NLAY,NOCBC
      USE GWFMNW1MODULE
...
c1------identify package and initialize nwell2
        WRITE (IOUT, 9001) NUM2STR(In)
 9001 FORMAT (/, ' MNW1 -- MULTI-NODE WELL 1 PACKAGE, VERSION 7,', 
     +        ' 04/05/2012.', /, '    INPUT READ FROM UNIT', A)
      NWELL2 = 0
c
...
c
        ...
        IF ( IWL2CB.GT.0 ) WRITE (IOUT, 9003) NUM2STR(IWL2CB)
        IF ( IWL2CB.LT.0 ) WRITE (IOUT, 9004)
        WRITE (IOUT, 9005) NUM2STR(KSPREF)
        WRITE (IOUT, 9006) NUM2STR(NOMOITER)
 9002 FORMAT (' MAXIMUM OF', i7, ' WELLS')
 9003 FORMAT (' CELL-BY-CELL FLOWS WILL BE RECORDED ON UNIT', A)
 9004 FORMAT (' CELL-BY-CELL FLOWS WILL BE PRINTED WHEN ICBCFL NOT 0')
 9005 FORMAT ('  The heads at the beginning of SP:', A, 
     +        ' will be the default reference elevations.', /)
 9006 FORMAT (' Flow rates will not be estimated after the', A, 
     +        'th iteration')
c
...
c   Test auxillary output files for cleaning data sets
      ...
      IF ( iobynd.GT.0 ) THEN
        ...
        CALL UTF8_BOM_OFFSET_REWIND(iobynd)
        READ (iobynd, '(a)')
      ELSE
        RETURN
      ENDIF
c
      ...
      IF ( ioqsum.GT.0 ) THEN
        ...
        CALL UTF8_BOM_OFFSET_REWIND(ioqsum)
        READ (ioqsum, '(a)')
      ELSE
        RETURN
      ENDIF
