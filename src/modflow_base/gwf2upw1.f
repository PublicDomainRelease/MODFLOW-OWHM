...
!
     ...
     2                     LBOTM,HNEW,NOCBC
      ...
      USE GWFUPWMODULE
      USE UTIL_INTERFACE, ONLY: GET_INTEGER
      IMPLICIT NONE
!     ------------------------------------------------------------------
!     SPECIFICATIONS:
!     ------------------------------------------------------------------
...
!     CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,IPHDRY,R,0,IN)
      CALL GET_INTEGER(LINE,LLOC,ISTART,ISTOP,IOUT,IN,IPHDRY,
     +                 HAS_ERROR=CHK)
      IF(CHK) THEN
          IPHDRY = 0
          LLOC = ISTART
...
C3B-----GET OPTIONS.
   ...
   21    FORMAT(1X,'STORAGECOEFFICIENT OPTION:',/,
     ...
     2     1X,'Option not supported when using NWT Solver')
   ...
   29    FORMAT(1X,'NOVFC OPTION:',/,1X,
     1   'vertical flow correction not supported when using NWT Solver')
...
C seb added output of final array
      IF(PROPPRINT.NE.'') CALL PRINTARRAY(IGRID,2,PROPPRINT,IOUT)
...
C3A1----LOGARITHMIC MEAN INTERBLOCK TRANSMISSIVITY
               T2=HKUPW(J+1,I,K)
               RATIO=T2/T1
               IF(RATIO == ZERO) THEN  !seb
                   T=ZERO
               ELSEIF(RATIO.GT.FRAC1 .OR. RATIO.LT.FRAC2) THEN
                  T=(T2-T1)/LOG(RATIO)
               ELSE
                  T=HALF*(T1+T2)
               END IF
...
C3------CALCULATE VERTICAL HYDRAULIC CONDUCTIVITY FOR CELL BELOW.
            IF(LAYVKAUPW(K+1).EQ.0) THEN
               HYC2=VKAUPW(J,I,K+1)
            ELSEIF(VKAUPW(J,I,K+1)==ZERO) THEN
               HYC2=ZERO
            ELSE
...
C------Set old saturation to new saturation.
      CALL SET_ARRAY(Numactive, Sn, So)
      !DO ij = 1, Numactive
      !  So(ij) = Sn(ij)
      !END DO
      RETURN
...
!     SUBROUTINE GWF2UPWUPDATE. UPDATE VALUES AFTER OUTER ITERATION.
      ...
      USE GLOBAL, ONLY: Ncol, Nrow, Nlay, NBOTM, Ibound,HNEW,LBOTM,BOTM
...
!     -----------------------------------------------------------------
      ...
      SUBROUTINE Sn_update(NROW,NCOL,NLAY,NBOTM,Numactive,
     +                     HNEW,BOTM,LBOTM,Diag,Sn)
      USE GLOBAL,       ONLY: KND
      IMPLICIT NONE
        INTEGER, INTENT(IN) :: NROW,NCOL,NLAY,NBOTM,Numactive
        !
        REAL(KND), DIMENSION(NCOL,NROW,NLAY),    INTENT(IN):: HNEW
        REAL,      DIMENSION(NCOL,NROW,0:NBOTM), INTENT(IN):: BOTM
        INTEGER,   DIMENSION(NBOTM),             INTENT(IN):: LBOTM
        INTEGER,   DIMENSION(3,Ncol*Nrow*Nlay),  INTENT(IN):: Diag
        DOUBLE PRECISION, DIMENSION(Numactive),  INTENT(OUT):: Sn
      END SUBROUTINE
...
C------SET POINTERS FOR THE CURRENT GRID.
      ...
      CALL SGWF2BAS7PNT(IGRID)   !seb lgr
      !
      IF (Itest==1) CALL SET_ARRAY(NCOL, NROW, NLAY, HITER, HNEW)
      !
      DO ij = 1, Numactive
        !il = Diag(1, ij)
        !ir = Diag(2, ij)
        ...
        END DO
      END DO
      CALL Sn_update(NROW,NCOL,NLAY,NBOTM,Numactive,
     +                     HNEW,BOTM,LBOTM,Diag,Sn)
      END SUBROUTINE
!     -----------------------------------------------------------------
