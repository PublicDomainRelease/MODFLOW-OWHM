...
C WILL NOW WORK WITH MF-OWHM AND MT3D-USGS TRANSPORT SIMULATIONS THAT 
...
C [1.00] OR LATER.
!rgn------REVISION NUMBER CHANGED TO INDICATE MODIFICATIONS FOR OWHM 
...
C      
     ...
     &                   NLAKCON,IGWET
      ...
      USE UTIL_INTERFACE,ONLY: GET_INTEGER, WARNING_MESSAGE
...
C--CHECK for OPTIONS/PACKAGES USED IN CURRENT SIMULATION
      IUMT3D=0
      DO IU=1,NIUNIT
        IF(CUNIT(IU).EQ.'LMT6'     .OR. CUNIT(IU).EQ.'LMT') THEN
          INLMT=IUNIT(IU)
        ELSEIF(CUNIT(IU).EQ.'BCF6' .OR. CUNIT(IU).EQ.'BCF') THEN
          MTBCF=IUNIT(IU)
...
C--CHECK FOR "OUTPUT_FILE_UNIT" KEYWORD AND GET UNIT NUMBER
      ELSEIF(LINE(ITYP1:ITYP2).EQ.'OUTPUT_FILE_UNIT') THEN
        !CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,IUMT3D,R,IOUT,INLMT)
        CALL GET_INTEGER(LINE,LLOC,ISTART,ISTOP,IOUT,INLMT,IUMT3D,   !seb sets Unit to 333 if fails to load
     +                   ERROR_VAL=333)
C
...
C
   ...
   30 FORMAT(//1X,'***Link-MT3DMS Package v8***',
...
C--COLLECT AND SAVE ALL RELEVANT FLOW MODEL INFORMATION
     ...
     &   KKSTP,KKPER,IUNIT(47),IGRID)
        IF(IUNIT(66) .NE.0) ! swm added call to original WEL package
...
C
     ...
     &                      LBOTM,DELR,DELC,BUFF,IOUT,CR,CC,CV,KND
      ...
      CHARACTER*16 TEXT
      REAL(KND)::  HD,DFL,DFR,DFT,DFB,HN,CHCOF,STRG
C    
C--SET POINTERS FOR THE CURRENT GRID   
...
C--UPDATE LAKE WITH LAST REACH NUMBER
      IF(IUNIT(22).NE.0) THEN  !IUNIT(22): LAK
        DO L=1,NSFRLAK
          ISTSG=ILKSEG(L)
...
C     IDIVAR(1,nseg): segment number to which flow is diverted
            ELSEIF((IUNIT(22).NE.0).AND.(IDIVAR(1,ISTSG).LT.0)) THEN
...
C
C***********************************************************************
!      SUBROUTINE LMT8FMP7(ILMTFMT,IUMT3D,KSTP,KPER,IGRID)
!C *********************************************************************
...
!      END
C***********************************************************************
!      SUBROUTINE LMT8RIP7(ILMTFMT,IUMT3D,KSTP,KPER,IGRID)
!C *********************************************************************
!C SAVE FLOW THROUGH SOIL ZONE IN VERTICAL DIRECTION.
...
!      END
C***********************************************************************
!      SUBROUTINE LMT8SWI7(ILMTFMT,IUMT3D,KSTP,KPER,IGRID)
!C *********************************************************************
...
C
      ...
      ISFRLAKCONNECT => LMTDAT(IGRID)%ISFRLAKCONNECT
      ISFRUZFCONNECT => LMTDAT(IGRID)%ISFRUZFCONNECT
      ILAKUZFCONNECT => LMTDAT(IGRID)%ILAKUZFCONNECT
      ISNKUZFCONNECT => LMTDAT(IGRID)%ISNKUZFCONNECT
      NPCKGTXT       => LMTDAT(IGRID)%NPCKGTXT      
      IUZFFLOWS      => LMTDAT(IGRID)%IUZFFLOWS     
      ISFRFLOWS      => LMTDAT(IGRID)%ISFRFLOWS     
      ILAKFLOWS      => LMTDAT(IGRID)%ILAKFLOWS      
      NLAKCON        => LMTDAT(IGRID)%NLAKCON       
      RETURN
      END
C***********************************************************************
...
C
      ...
      LMTDAT(IGRID)%ISFRLAKCONNECT => ISFRLAKCONNECT
      LMTDAT(IGRID)%ISFRUZFCONNECT => ISFRUZFCONNECT
      LMTDAT(IGRID)%ILAKUZFCONNECT => ILAKUZFCONNECT
      LMTDAT(IGRID)%ISNKUZFCONNECT => ISNKUZFCONNECT
      LMTDAT(IGRID)%NPCKGTXT       => NPCKGTXT      
      LMTDAT(IGRID)%IUZFFLOWS      => IUZFFLOWS     
      LMTDAT(IGRID)%ISFRFLOWS      => ISFRFLOWS     
      LMTDAT(IGRID)%ILAKFLOWS      => ILAKFLOWS     
      LMTDAT(IGRID)%NLAKCON        => NLAKCON      
C
      RETURN
      END
C***********************************************************************
...
C     ------------------------------------------------------------------
C
      DEALLOCATE(LMTDAT(IGRID)%ISSMT3D       , STAT=IERR)
      DEALLOCATE(LMTDAT(IGRID)%IUMT3D        , STAT=IERR)
      DEALLOCATE(LMTDAT(IGRID)%ILMTFMT       , STAT=IERR)
      DEALLOCATE(LMTDAT(IGRID)%ISFRLAKCONNECT, STAT=IERR)
      DEALLOCATE(LMTDAT(IGRID)%ISFRUZFCONNECT, STAT=IERR)
      DEALLOCATE(LMTDAT(IGRID)%ILAKUZFCONNECT, STAT=IERR)
      DEALLOCATE(LMTDAT(IGRID)%ISNKUZFCONNECT, STAT=IERR)
      DEALLOCATE(LMTDAT(IGRID)%NPCKGTXT      , STAT=IERR)
      DEALLOCATE(LMTDAT(IGRID)%IUZFFLOWS     , STAT=IERR)
      DEALLOCATE(LMTDAT(IGRID)%ISFRFLOWS     , STAT=IERR)
      DEALLOCATE(LMTDAT(IGRID)%ILAKFLOWS     , STAT=IERR)
      DEALLOCATE(LMTDAT(IGRID)%NLAKCON       , STAT=IERR)
C
C
