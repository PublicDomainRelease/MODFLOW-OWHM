...
C
Cx------Allocate memory
      ALLOCATE(SKIP_OBS(NQTRV), SOURCE=.FALSE.)                         !seb
...
Cx------FOR THE CURRENT TIME STEP.
        DO 600 NT = NT1, NT2
          IF(SKIP_OBS(NT)) THEN
              WRITE (IOUT,80) NT, OBSNAM(NT)
  80          FORMAT (/,' RIV  OBS#',I5,', ID ',A,' IS BEING SKIPPED',
     &          ' DUE TO NOT BEING WITHIN SIMULATED TIME (OBS2RIV7SE)')
              CYCLE
          END IF
...
C
      IF(FN_PRN_ALL%IS_OPEN) THEN
         CALL UOBSSV(FN_PRN_ALL%IU,NQTRV,FLWSIM,
     1                      FLWOBS,OBSNAM,0)
      END IF
Cx------RETURN
      RETURN
...
C
      ...
      DEALLOCATE(OBSRIVDAT(IGRID)%SKIP_OBS)
      ! GFORTRAN compiler error work-around for pointer data type FINAL statement
      FN_PRN_ALL=>OBSRIVDAT(IGRID)%FN_PRN_ALL
      OBSRIVDAT(IGRID)%FN_PRN_ALL=>NULL()
      DEALLOCATE(FN_PRN_ALL)
      FN_PRN_ALL=>NULL()
      !
      FN_PRN=>OBSRIVDAT(IGRID)%FN_PRN
      OBSRIVDAT(IGRID)%FN_PRN=>NULL()
      DEALLOCATE(FN_PRN)
      FN_PRN=>NULL()
      !DEALLOCATE(OBSRIVDAT(IGRID)%FN_PRN_ALL)
      !DEALLOCATE(OBSRIVDAT(IGRID)%FN_PRN)
C
...
C
      ...
      SKIP_OBS=>OBSRIVDAT(IGRID)%SKIP_OBS
      FN_PRN_ALL=>OBSRIVDAT(IGRID)%FN_PRN_ALL
      FN_PRN    =>OBSRIVDAT(IGRID)%FN_PRN
C
      RETURN
...
C
      ...
      OBSRIVDAT(IGRID)%SKIP_OBS=>SKIP_OBS
      OBSRIVDAT(IGRID)%FN_PRN_ALL=>FN_PRN_ALL
      OBSRIVDAT(IGRID)%FN_PRN    =>FN_PRN
C
      RETURN
      END
