...
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
      USE GLOBAL,      ONLY:NCOL,NROW,NLAY,NBOTM,BOTM,ITRSS,LAYHDT,KND,
...
C2A------READ FIRST RECORD AND WRITE
      ...
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,IOHUFFLWS,R,0,IN)
      !
...
C     ------------------------------------------------------------------
      ...
      REAL(KND):: HNEW
      REAL(KND):: TOP1,TOP2,TOP3
      REAL::LAMBDA,RMID1,RMID2,MULTKDEP
...
C
      REAL(KND):: HNEW
      REAL(KND)::CR,CC                                         !seb PREVIOUSLY IMPLICITLY DEFINED AS REAL
C
      DIMENSION HNEW(NCOL,NROW,NLAY),IBOUND(NCOL,NROW,NLAY),
...
C     
      USE GLOBAL, ONLY: KND
      REAL(KND):: CV                                           !seb PREVIOUSLY IMPLICITLY DEFINED AS REAL
...
C     ------------------------------------------------------------------
      ...
      USE GWFHUFMODULE,ONLY:LTHUF,LAYWT
      REAL(KND):: HNEW,HHD,BBOT,TTOP
      REAL(KND)::CC                                            !seb PREVIOUSLY IMPLICITLY DEFINED AS REAL
C
      DIMENSION HNEW(NCOL,NROW,NLAY),IBOUND(NCOL,NROW,NLAY),
     & CC(NCOL,NROW,NLAY),
...
C     ------------------------------------------------------------------
     ...
     1                      CR,CC,CV,HNEW,RHS,HCOF,HOLD,ISSFLG,IOUT,KND
...
C---------------Compute SC1 Component
          ...
          ENDIF
C---------------Compute SC2 Component
          CALL SGWF2HUF7SC2(1,J,I,K,TOP,BOT,HN,HO,TLED,CHCOF,STRG,  !SCOTT seems strange this is not CRHS
...
C     ------------------------------------------------------------------
      ...
      CHARACTER*16 TEXT(3)
      REAL(KND)::CV
      REAL(KND):: HNEW,H0,HXR,HYB,HZB,DHXR,DHYB,DHZB
...
C1------WRITE TWO UNFORMATTED RECORDS IDENTIFYING DATA.
     ...
     &                   DELT,PERTIM,TOTIM,IBOUND)
        IF(IOUT.NE.0) WRITE(IOUT,2) TEXT(ICNT),IHUFCB,KSTP,KPER
...
C     ------------------------------------------------------------------
      ...
      REAL LAMBDA,MULTKDEP
      REAL(KND):: CV                                           !seb PREVIOUSLY DEFINED AS IMPLICIT REAL
      REAL(KND):: HNEW,HDIFF,QN
      DIMENSION HNEW(NCOL,NROW,NLAY),HNWHGU(NCOL,NROW,NHUF),
     &    IBOUND(NCOL,NROW,NLAY),BOTM(NCOL,NROW,0:NBOTM),
     &    DELR(NCOL),DELC(NROW),CV(NCOL,NROW,NLAY),
...
C     ------------------------------------------------------------------
      USE GLOBAL, ONLY: KND
      REAL(KND)::CR,CC,RHS                                     !seb PREVIOUSLY IMPLICITLY DEFINED AS REAL 
      REAL(KND):: HNEW,H1
      REAL(KND):: TF9,TF5,FL5,FR5,FT5,FB5,
     &   CRL,CRR,CCT,CCB
...
C     ------------------------------------------------------------------
      USE GLOBAL, ONLY: KND
      REAL(KND):: HNEW,H1,H2,H3,H4,H5,H6,H7,H8,H9,FS1A,FS1B,
     &  FW1A,FW1B,FS2A,FS2B,FE2A,FE2B,FE3A,FE3B,FN3A,FN3B,
     &  FW4A,FW4B,FN4A,FN4B
      REAL(KND):: FL,FR,FT,FB
      REAL(KND)::      AS11234,AS21234,AS31234,AS41234,
     ...
     & DTAA1,DTAB1,DTBB1,TAA1,TAB1,TBB1
C
      DIMENSION HNEW(NCOL,NROW,NLAY), IBOUND(NCOL,NROW,NLAY),
...
C     ------------------------------------------------------------------
      USE GLOBAL, ONLY: KND
      REAL(KND):: HNEW,H1,H2,H3,H4,H5,H6,H7,H8,H9
      REAL(KND):: F
      DIMENSION HNEW(NCOL,NROW,NLAY), IBOUND(NCOL,NROW,NLAY),
     &     A9(NCOL,NROW,NLAY,5)
C     ------------------------------------------------------------------
...
C     ------------------------------------------------------------------
     ...
     3             AW11234,AW21234,AW31234,AW41234,D1234
      REAL(KND)::  TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
...
C     ------------------------------------------------------------------
      USE GLOBAL, ONLY:KND
      DIMENSION VDHT(NCOL,NROW,NLAY,3),IBOUND(NCOL,NROW,NLAY)
      REAL(KND):: AS16145,AS26145,AS36145,AS46145,TBB1,TAA4,TBB5,
     1  AE16145,AE26145,AE36145,AE46145,D6145,TAB6,TAB1,TBB6,TAA6,TAA1,
     2  TBB4,TAA5,TAB4,TAB5
C-----------------------------------------------------------------------
C
C-------Local cell 1, I,J
      TAA1 = VDHT(J,I,K,1)
...
C     ------------------------------------------------------------------
      USE GLOBAL, ONLY: KND
      DIMENSION VDHT(NCOL,NROW,NLAY,3),IBOUND(NCOL,NROW,NLAY)
      REAL(KND):: AN17816,AN27816,AN37816,AN47816,
     1            AE17816,AE27816,AE37816,AE47816,D7816,
     2TAB8,TAA7,TBB7,TAA8,TAB7,TBB8,TAB1,TBB6,TAB6,TAA6,TAA1,TBB1
C-----------------------------------------------------------------------
C
C-------Local cell 1, I,J
      TAA1 = VDHT(J,I,K,1)
...
C     ------------------------------------------------------------------
      USE GLOBAL, ONLY: KND
      DIMENSION VDHT(NCOL,NROW,NLAY,3),IBOUND(NCOL,NROW,NLAY)
      REAL(KND):: AN18921,AN28921,AN38921,AN48921,
     1            AW18921,AW28921,AW38921,AW48921,D8921,
     2  TBB2,TAB8,TAB9,TBB9,TAA9,TBB8,TAA8,TAA2,TBB1,TAB1,TAB2,TAA1
C-----------------------------------------------------------------------
C
C-------Local cell 1, I,J
      TAA1 = VDHT(J,I,K,1)
...
C     ------------------------------------------------------------------
      USE GLOBAL, ONLY: KND
      REAL(KND):: D,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &            TAA3,TAB3,TBB3,TAA4,TAB4,TBB4
C-----------------------------------------------------------------------
C
...
C     ------------------------------------------------------------------
      USE GLOBAL, ONLY: KND
      REAL(KND):: C1,C2
      REAL(KND):: AS1,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &            TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D
C-----------------------------------------------------------------------
C
...
C     ------------------------------------------------------------------
      USE GLOBAL, ONLY:KND
      REAL(KND):: C1,C2
      REAL(KND):: AS2,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &            TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D
C-----------------------------------------------------------------------
C
      C1=(TAA3+TAA4)*(TBB1+TBB4)*(TBB2+TBB3)
...
C     ------------------------------------------------------------------
      USE GLOBAL, ONLY:KND
      REAL(KND):: C1,C2
      REAL(KND):: AS3,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &            TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D
C-----------------------------------------------------------------------
C
...
C     ------------------------------------------------------------------
      ...
      REAL(KND):: C1,C2
      REAL(KND):: AS4,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &            TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D
C-----------------------------------------------------------------------
C
      C1=-TAB1*TAB4*(TBB2+TBB3)
...
C     ------------------------------------------------------------------
      USE GLOBAL, ONLY:KND
      REAL(KND):: C1,C2
      REAL(KND)::AW1,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &           TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D
C-----------------------------------------------------------------------
C
      C1=TAB1*TAB3*TAB3
...
C     ------------------------------------------------------------------
      USE GLOBAL, ONLY:KND
      REAL(KND):: C1,C2
      REAL(KND)::AW2,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &           TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D
C-----------------------------------------------------------------------
C
      C1=TAB1*TAB3*TAB3
...
C     ------------------------------------------------------------------
      USE GLOBAL, ONLY:KND
      REAL(KND):: C1,C2
      REAL(KND)::AW3,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &           TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D
C-----------------------------------------------------------------------
C
      C1=TAB1*TAB2*TAB3
...
C     ------------------------------------------------------------------
      USE GLOBAL, ONLY:KND
      REAL(KND):: C1,C2
      REAL(KND):: AW4,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &            TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D
C-----------------------------------------------------------------------
C
      C1=TAB1*TAB2*TAB3
...
C     ------------------------------------------------------------------
      USE GLOBAL, ONLY:KND
      REAL(KND):: C1,C2
      REAL(KND)::AE1,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &            TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D
C-----------------------------------------------------------------------
C     6145
C     1234
...
C     ------------------------------------------------------------------
      USE GLOBAL, ONLY:KND
      REAL(KND):: C1,C2
      REAL(KND)::AE2,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &           TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D
C-----------------------------------------------------------------------
C     6145
C     1234
...
C     ------------------------------------------------------------------
      USE GLOBAL, ONLY:KND
      REAL(KND):: C1,C2
      REAL(KND)::AE3,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &           TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D
C-----------------------------------------------------------------------
C     6145
C     1234
...
C     ------------------------------------------------------------------
      USE GLOBAL, ONLY:KND
      REAL(KND):: C1,C2
      REAL(KND):: AE4,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &            TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D
C-----------------------------------------------------------------------
C     6145
C     1234
...
C     ------------------------------------------------------------------
      USE GLOBAL, ONLY:KND
      REAL(KND):: C1,C2
      REAL(KND)::AN1,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &           TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D
C-----------------------------------------------------------------------
C     7816
C     1234
...
C     ------------------------------------------------------------------
      USE GLOBAL, ONLY:KND
      REAL(KND):: C1,C2
      REAL(KND)::AN2,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &           TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D
C-----------------------------------------------------------------------
C     7816
C     1234
...
C     ------------------------------------------------------------------
      USE GLOBAL, ONLY:KND
      REAL(KND):: C1,C2
      REAL(KND)::AN3,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &           TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D
C-----------------------------------------------------------------------
C     7816
C     1234
...
C     ------------------------------------------------------------------
      USE GLOBAL, ONLY:KND
      REAL(KND):: C1,C2
      REAL(KND)::AN4,TAA1,TAB1,TBB1,TAA2,TAB2,TBB2,
     &           TAA3,TAB3,TBB3,TAA4,TAB4,TBB4,D
C-----------------------------------------------------------------------
C     7816
C     1234
