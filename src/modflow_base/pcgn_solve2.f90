...
contains
  ...
  subroutine a_multiply(X_i, rhs)
    ...
    ! ...  argument list
    ! ...
    real(kind=kv), intent(in ), dimension(:), contiguous :: X_i
    real(kind=kv), intent(out), dimension(:), contiguous :: rhs
    ! ...
    ! ...  local variables
    ! ...
    integer :: node, nxy, m
...
module MiUDU
  ...
  implicit none
  real(kind=kv), dimension(:), contiguous, pointer :: XX
...
contains
  ...
  subroutine U_tranpose_solve_0(rhs)
    ...
    ! ... argument list
    ! ...
    real(kind=kv), dimension(:), intent(inout), contiguous  :: rhs
    ! ...
    ! ... local variables
    ! ...
    integer :: node, nxy, m
...
module PCG_SOLVE
  ...
  ! ... rhs, r_v: A*r_v=rhs
  implicit none
  real(kind=kv), dimension(:), contiguous, pointer :: x_dev, r_dev
  real(kind=kv) :: nu_i
contains
  ...
  subroutine CG_iter(precond_fn, convg_fn, X, res, max_iter, eps_i, &
    ...
    real(kind=kv), intent(inout), dimension(:), contiguous :: X, res
...
666 format(tr2,i4,tr8,1pe12.5)
  ...
  subroutine CG_iter_xt(precond_fn, convg_fn, X, res, max_iter, eps_i, &
    ...
    do i=1, max_iter
       ! ... r_dev=A*x_dev
       call a_multiply(x_dev,r_dev)
       mu = dot_product(x_dev,r_dev)
       IF(mu.NE.0_kv) mu=nu/mu
       !mu=nu/dot_product(x_dev,r_dev)   seb fix div/zero issue
       ! ... update solution
       call update_1(X,x_dev,mu)
...
666 format(tr2,i4,tr8,1pe12.5)
  ...
  subroutine update_1(y,x,r)
    ...
    ! ... y, x: arrays
    ! ... r: scalar
    real(kind=kv), dimension(:), intent(inout), contiguous :: y
    real(kind=kv), dimension(:), intent(in   ), contiguous :: x
...
module PCG_MAIN
  ...
  character(len=32), dimension(:), contiguous, pointer :: marker_name
...
contains
  ...
  subroutine PCG_init(fill, n_x, n_y, n_z, d_d, d_x, d_y, d_z, &
    ...
    real(kind=kv), dimension(:), intent(in), contiguous, target :: d_d, d_x, &
         d_y, d_z
...
500 format(1x,'Allocation error in subroutine PCG_init')
  ...
  subroutine PCG(resid, convg_i, xx, max_iter, c_flag, convg_o, marker, &
    ...
    ! ... argument list
    ! ...
    real(kind=kv), dimension(:), intent(inout), contiguous :: resid
    real(kind=kv), intent(inout) :: convg_i
    real(kind=kv), dimension(:), optional, intent(inout), contiguous, target :: xx
    ...
    integer :: error, iter_max, iter_no, start_time, solv_time!, end_time
    integer :: cum_time=0
    integer, external :: elapsed_time
    real(kind=kv), dimension(:), contiguous, pointer :: X
    ! ...
    ! .....................................................................
    ! ...
    nullify (X)
