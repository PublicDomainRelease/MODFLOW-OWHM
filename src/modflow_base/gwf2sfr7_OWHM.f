...
C       ISTRM(6,NSTRM)=IFACE value for modpath
C
...
C       STRM(31,NSTRM)= Cross-sectional area from XSA for LMT
C
C       SEG(1 ,NSEG)=Segment Lenth
C       SEG(2 ,NSEG)=Specified Inflow/Diversion Flow/Split
C       SEG(3 ,NSEG)=Overland runoff into stream reach 
C       SEG(4 ,NSEG)=Stream ET 
C       SEG(5 ,NSEG)=Stream Precipitation 
C       SEG(6 ,NSEG)=
C       SEG(7 ,NSEG)=
C       SEG(8 ,NSEG)=
C       SEG(9 ,NSEG)=
...
C       SEG(16,NSEG)=
C       SEG(17,NSEG)=
C       SEG(18,NSEG)=thsslpe  UZF Stuff
C       SEG(19,NSEG)=thislpe  UZF Stuff
C       SEG(20,NSEG)=epsslpe  UZF Stuff
C       SEG(21,NSEG)=
C       SEG(22,NSEG)=
C       SEG(23,NSEG)=
C       SEG(24,NSEG)=
C       SEG(25,NSEG)=
C       SEG(26,NSEG)= UZF gwrunof
C       SEG(27,NSEG)=
C       SEG(28,NSEG)=
C
C       ISEG(1 ,NSEG)= ICALC
C       ISEG(2 ,NSEG)= IF ( ICALC.EQ.4 ) THEN QSTAGE(1+ISEG(2, NSEG), NSEG)
C       ISEG(3 ,NSEG)= IDENTIFY SEGMENTS THAT DIVERT FLO
C       ISEG(4 ,NSEG)= NREACH
C
C      IOTSG(NSEG) = Connection to downstream segment
C
C      IDIVAR(1,NSEG) = IUPSEG -- SEGMENT THAT DIVERSION ORIGINATES FROM
C      IDIVAR(2,NSEG) = IPRIOR
C
C     ******************************************************************
...
C     ******************************************************************
      ...
      USE CONSTANTS,      ONLY: NL,BLNK,Z
      IMPLICIT NONE
      PRIVATE:: BOTM, IBOUND, LBOTM, ISTRM, STRM, NSTRM
      PRIVATE:: STOP_ERROR, NL,BLNK,Z
      type check_bot
     ...
     +                STRM(4, self%IRCHNUM),BOTM(JRCH,IRCH,LBOTM(KRCH))
      ...
      IF ( self%IFLAG.GT.0 .AND. self%IRCHNUM.EQ.NSTRM ) THEN
        CALL STOP_ERROR(BLNK,Z,self%IUNIT,
     +   MSG='SFR ERROR: CHECK FOR STREAMBED BELOW CELL BOTTOM.'//NL//
     +      'MODEL STOPPING DUE TO REACH ALTITUDE ERROR.'//NL//
     +      'SEE LIST FOR MORE DETAILS ON SPECIFIC REACHES THAT '//
     +      'HAVE BAD STREAMBED ELEVATIONS')
...
C     ------------------------------------------------------------------
     ...
     +                        ITRSS,NLAY,NCOL,NROW,LAYHDT,IUNIT, NOCBC       !CJM added ncol and nrow
     ...
     +                    GET_INTEGER, GET_NUMBER
      ...
      USE CONSTANTS, ONLY: TRUE, FALSE, Z, ONE, DZ, 
...
!!    INTEGER nssar, nstrmar, Ltyp, NPP, MXVL, IRFG, ITRFLG
      ...
      REAL epsslpe
      character(len=23) :: keyvalue
      character(len=16)  :: text!        = 'SFR2'
      logical :: found, checktabfile, NO_LINEFEED, FOUND_BEGIN
...
C     ------------------------------------------------------------------
      INTERFACE 
               FUNCTION SFR_PREV_LINE(IN) RESULT(LINE)
...
C     ------------------------------------------------------------------
     ...
     +'$Id: gwf2sfr7_NWT.f 7541 2015-07-30 21:46:59Z rniswon $'
      ...
      IF(IUNIT(49).NE.0) ALLOCATE(NFLOWTYPE)
      ALLOCATE (FACTOR)
      ALLOCATE(HD_RELAX, SOURCE=1D0)
      !
      ALLOCATE(TIME_SERIES)
      !
      ALLOCATE(SFR_PRNT, SOURCE=TRUE)
      ALLOCATE(SFR_FRES_OUTER, SFR_FRES_NTERM, SFR_FRES)
      SFR_FRES_OUTER = inf_I
      !
      SFR_IN_USE = TRUE
      !
      LOUT = LIST_UNIT
      ...
      !
      ALLOCATE(SFR_FIX_BOT)
      SFR_FIX_BOT = FALSE
      !
      ALLOCATE(SFR_AUTO_NEG_ITMP)
      SFR_AUTO_NEG_ITMP = FALSE
      !
      ALLOCATE(CNVG_WRN)
      CALL CNVG_WRN%INIT()
C
...
C
      ...
      IF( LINE(ISTART:ISTOP)=='OPTION' .OR.
     +    LINE(ISTART:ISTOP)=='OPTIONS'     ) THEN
          LINE = 'BEGIN OPTION'
      ELSEIF(LINE(ISTART:ISTOP)=='PARAMETER') THEN
     ...
     +       BL%NAME == 'TIME'                  ) THEN
             ...
             !
             IF(TIME_SERIES%NFIL>Z .AND. .NOT. HAS_STARTDATE) THEN
               CALL ERR%ADD('SFR OPTION "TIME_SERIES" REQUIRES THAT '//
     +         'THE DIS PACKAGE SPECIFY A STARTING DATE WITH '//
     +         'KEYWORD STARTDATE'//BLN)
!                 CALL STOP_ERROR(BLNK,IN,IOUT,
!     +           MSG='SFR OPTION "TIME_SERIES" REQUIRES THAT THE DIS '//
!     +         'PACKAGE SPECIFY A STARTING DATE WITH KEYWORD STARTDATE')
             END IF
             !
      ELSEIF(BL%NAME == 'OPTION' .OR. BL%NAME == 'OPTIONS' ) THEN
        !
        WRITE(IOUT,'(/1X,A)') 'PROCESSING SFR OPTIONS BLOCK'
        !
        CALL BL%START()
        DO I=ONE, BL%NLINE
        LLOC = ONE
        ...
        CASE('NO_BUDGET_RECALCULATE_FLOW')
            DO_FM_BD = FALSE
            !
        CASE('FIX_STREAM_BOTTOM')
            SFR_FIX_BOT = TRUE
            !
        CASE('AUTOMATIC_NEGATIVE_ITMP')
            SFR_AUTO_NEG_ITMP = TRUE
            !
        CASE('NOPRINT', 'NO_PRINT')
            SFR_PRNT = FALSE
            !
        CASE('REACHINPUT')
           IRFG = 1
           WRITE(IOUT,'(/3A/)')'REACHINPUT Keyword found.',
     +                         'Some stream information will be read ',
     +                         'by reach. This option replaces NSTRM<0'
        CASE('TRANSROUTE')
            ITRFLG = 1
            WRITE(IOUT,'(/A/)')'TRANSROUTE KEYWORD FOUND: TRANSIENT '//
     +                         'ROUTING IN STREAMS IS ACTIVE'
        CASE('IFACE')
            IFACE=1
            WRITE(IOUT,'(/A/)') 'IFACE KEYWORD FOUND. '//NL//
     +                    'IFACE values will be read from reach records'
        CASE('TABFILES')
            CALL GET_INTEGER(BL%LINE,LLOC,ISTART,ISTOP,IOUT,IN,NUMTAB,
     +      MSG='SFR ERROR LOADING TABFILES "NUMTAB"')
            CALL GET_INTEGER(BL%LINE,LLOC,ISTART,ISTOP,IOUT,IN,ROWTAB,
     +      MSG='SFR ERROR LOADING TABFILES "ROWTAB"')
            IF(NUMTAB.LT.0) NUMTAB=0
            IF(ROWTAB.LT.0) ROWTAB=1
            WRITE(IOUT,'(/A,A,I10,A,I10,A/)')
     ...
     +       NUMTAB,'with a maximum of ',ROWTAB,' row entries per file'
        CASE('LOSSFACTOR')
            CALL URWORD(BL%LINE, lloc, istart, istop,3,i,FACTOR,IOUT,In)
            WRITE(IOUT,'(4A,ES16.9)') 
     +                 'LOSSFACTOR KEYWORD FOUND: ',
     +                 'Stream loss will be calculated as a factor ',
     +                 'of the streambed hydraulic conductivity. ',
     +                 'Multiplication factor is equal to ',FACTOR
        CASE('AUX','AUXILIARY')
            CALL PARSE_WORD_UP(LINE,LLOC,ISTART,ISTOP)
            IF(NSFRAUX.LT.20) THEN
               NSFRAUX=NSFRAUX+1
               SFRAUX(NSFRAUX)=BL%LINE(ISTART:ISTOP)
               WRITE(IOUT,'(3A)') 'AUXILIARY STREAM FLOW ROUTING ',
     +                            'VARIABLE: ',TRIM(SFRAUX(NSFRAUX))
            ELSE
               CALL ERR%ADD('SFR AUXILIARY (AUX) STREAM FLOW ROUTING '//
     +         'VARIABLES ARE SPECIFEID MORE THAN 20 TIMES.'//BLN)
!                 CALL STOP_ERROR(BL%LINE,IN,IOUT,
!     +           MSG='TOO MANY AUXILIARY ')
            END IF
            !
        CASE('PRINT_GW_FLOW_RESIDUAL') ! OUTER_START NTERM FILE
            !
            WRITE(IOUT,'(A)')' FOUND OPTION "PRINT_FLOW_RESIDUAL"'
            !
            CALL GET_INTEGER(BL%LINE,LLOC,ISTART,ISTOP,IOUT,IN,
     +                       SFR_FRES_NTERM,MSG=
     +      'FOUND SFR OPTION "PRINT_GW_FLOW_RESIDUAL"'//NL//
     ...
     +  'BUT FAILED TO LOAD THE STARTING OUTER ITERATION (OUTER_START)')
         !
         CALL SFR_FRES%OPEN(BL%LINE,LLOC,BL%IOUT,BL%IU,
     +                       NO_INTERNAL=TRUE)
         !
         IF(.NOT. SFR_FRES%BINARY)
     +   CALL SFR_FRES%SET_HEADER('   SP   TS  ITER  LAY  ROW  COL'//
     +   '  SEG  RCH           HEAD    FLOW_RESIDUAL    VOL_RESIDUAL'//
     +   '     CELL_VOLUME      DATE  CELL_ID')
         !
         ALLOCATE(SFR_FRES_LRC(SFR_FRES_NTERM) ,
     +            SFR_FRES_DIF(SFR_FRES_NTERM)  )
        CASE('DBFILE') 
            WRITE(IOUT,'(2A)') 'DBFILE OPTION FOUND. ',
     +       'SFR INFORMATION WRITTEN TO DATABASE FRIENDLY OUTPUT'
            CALL DBFILE%OPEN(BL%LINE,LLOC,IOUT,IN,SPLITMAXCOUNT=11)
            IF(DBFILE%BINARY) THEN
          WRITE(IOUT,'(/ /2A,/A, /A/)') 'SFR DATABASE FRIENDLY OUTPUT ',
     ...
     +   'SIMTIME (double), SEG (int), RCH (int), FLOW_IN (double), '//
     +   'FLOW_SEEPAGE (double), FLOW_OUT (double), RUNOFF (double), '//
     +   'PRECIP (double), STREAM_ET (double), HEAD_STREAM (double), '//
     +   'HEAD_AQUIFER (double), DEPTH_STREAM (double), '//
     +   'WIDTH_STREAM (double), LENGTH_STREAM (double), '//
     +   'HEAD_GRADIENT (double), COND_STREAM (double), '//
     +   'ELEV_UP_STREAM (double), FLOW_WT (double),  '//
     +   'CHNG_UNSAT_STOR (double)'
            END IF
        CASE('HEAD_RELAX','HEAD_RELAXATION')
            CALL GET_NUMBER(BL%LINE,LLOC,ISTART,ISTOP,IOUT,IN,
     +                       HD_RELAX,MSG=
     +  'FOUND SFR OPTION "HEAD_RELAXATION" OR "HEAD_RELAX"'//NL//
     +  'BUT FAILED TO THE RELAXATION FACTOR (HD_RELAX)')
        CASE DEFAULT
            ! -- NO OPTIONS FOUND
        CALL WRN%ADD('SFR WARNING: FAILED TO IDENTIFY OPTION: '//
     +                BL%LINE(ISTART:ISTOP)//NL//
     +               'THIS OPTION IS IGNORED AND NOT APPLIED.'//NL)
        END SELECT
        !
        CALL BL%NEXT()
        !
      ...
      ! ----------------------------------------------------------
      !
      IF(LINE(ISTART:ISTOP)=='PARAMETER') THEN
          CALL UPARLSTAL(IN,IOUT,LINE,NPP,MXVL)
          LLOC = ONE
          CALL PARSE_WORD_UP(LINE,LLOC,ISTART,ISTOP)
      END IF
...
!      CALL UPPER(KEYVALUE)
!      IF(ANY(KEYVALUE==['REACHINPUT','TRANSROUTE', 'TABFILES',
!     +             'LOSSFACTOR','AUX','AUXILIARY','DBFILE',
!     +             'LINEFEED' ])) THEN
!       CALL STOP_ERROR(TRIM(LINE),IN,IOUT,
!     +  'SFR OPTION ERROR. FOUND OPTION "'//TRIM(KEYVALUE)//
!     + '" BUT SFR REQUIRES THAT OPTIONS BE WITHIN AN OPTIONS BLOCK '//
!     + 'PLEASE ADD "BEGIN OPTIONS" AND THEN HAVE ONE OPTION PER LINE '//
!     + 'AND END OPTIONS BLOCK WITH THE WORD "END"')
!      END IF
!      IF( ANY( KEYVALUE.EQ.['BEGIN','OPTION','OPTIONS'] ) ) THEN
!        WRITE(IOUT,'(/1X,A)') 'PROCESSING '//TRIM(TEXT)//' OPTIONS'
!        DO
!        CALL READ_TO_DATA(LINE,IN,LOUT)
!        lloc = 1
...
!            CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,ROWTAB,R,IOUT,IN)
!            IF(NUMTAB.LT.0) NUMTAB=0
!            IF(ROWTAB.LT.0) ROWTAB=1
!            WRITE(IOUT,'(/A,I10,A,I10,A/)')
!     +               'Specifed inflow files will be read ',NUMTAB,
!     +               'with a maximum of ',ROWTAB,' row entries per file'
!        CASE('LOSSFACTOR')
!            CALL URWORD(line, lloc, istart, istop, 3, i, FACTOR,IOUT,In)
!            WRITE(IOUT,'(3A,ES16.9)') 
!     +                 'Stream loss will be calculated as a factor ',
!     +                 'of the streambed hydraulic conductivity. ',
!     +                 'Multiplication factor is equal to ',FACTOR
!        CASE('TABFILE') !seb ADDED TABFILE SUPPORT FROM TABFILE_INTERFACE
!            CALL TABFILEPARSE(IN,IOUT,LINE,SFRTABFILE)
!            CALL TABFILELINKS(IN,IOUT,LINE,SFRTABFILE)   
!            CHECKTABFILE = .FALSE.
!        CASE('AUX','AUXILIARY')
...
!     +           MSG='TOO MANY AUXILIARY STREAM FLOW ROUTING VARIABLES')
!            END IF
!
!        CASE('DBFILE') 
!            WRITE(IOUT,'(2A)') 'DBFILE OPTION FOUND. ',
!     +       'SFR INFORMATION WRITTEN TO DATABASE FRIENDLY OUTPUT'
!            CALL DBFILE%OPEN(LLOC,LINE,IOUT,IN,SPLITMAXCOUNT=11)
!            IF(DBFILE%BINARY) THEN
!          WRITE(IOUT,'(/ /2A,/A, /A)') 'SFR DATABASE FRIENDLY OUTPUT ',
!     +   'WRITTEN TO BINARY FILE USING STREAM UNFORMATTED STRUCTURE.',
!     +   'EACH THE RECORD IN BINARY HAS THE FOLLOWING STRUCTURE:',
...
!        CASE('LINEFEED')
!            ALLOCATE(SFRFEED)
!            CALL SFRFEED%INIT(IN,IOUT,LINE,.TRUE.)
!            NO_LINEFEED = .FALSE.
!        CASE ('END')
!            CALL READ_TO_DATA(LINE,IN,LOUT)
!            EXIT
...
!     +           MSG='SFR ERROR: FOUND BEING OPTIONS BLOCK, BUT '//
!     +               'FAILED TO LOCATE A CORRESPONDING "END" '//
!     +               'STATEMENT TO CLOSE THE BLOCK.')
!        CASE DEFAULT
!            ! -- NO OPTIONS FOUND
!            WRITE(IOUT,'(/2A,A/)')
!     +             'SFR WARNING: FAILED TO IDENTIFY OPTION: ', KEYVALUE,
...
!      FOUND = .FALSE.
!      !
!      IF(KEYVALUE.EQ.'REACHINPUT') THEN
!         FOUND = .TRUE.
!         IRFG = 1
!         WRITE(IOUT,32)
...
!      END IF
!      !
!      IF(KEYVALUE.EQ.'TRANSROUTE') THEN
!         FOUND = .TRUE.
!         ITRFLG = 1
!      END IF
!      !
!      IF ( FOUND ) CALL READ_TO_DATA(LINE,IN,LOUT)
!! Check keyword for tabular inflow rates.
!      CALL UPARLSTAL(IN,IOUT,LINE,NPP,MXVL)
!      !
!      IF(checktabfile) THEN !seb ADDED TABFILE SUPPORT FROM TABFILE_INTERFACE
!                      CALL TABFILEPARSE(IN,IOUT,LINE,SFRTABFILE)
!                      CALL TABFILELINKS(IN,IOUT,LINE,SFRTABFILE)   
!      END IF
!      !
!      !ALLOCATE SFRFEED VARIABLE AND OPTIONALLY READ IN FEED FILE LOCATIONS
      ...
      IF(LINE(ISTART:ISTOP).EQ.'TABFILES') THEN
         CALL WRN%ADD('"TABFILES" OPTION SELECTED '//  !seb ADDED WARNING NOTICE
     +   'THIS IS THE ORIGINAL FEATURE WITH SFR2, WHICH IS DIFFERENT '//
     +   'FROM THE NEW "TABFILE" OPTION'//NL)
         CALL GET_INTEGER(BL%LINE,LLOC,ISTART,ISTOP,IOUT,IN,NUMTAB,
     +   MSG='SFR ERROR LOADING TABFILES "NUMTAB"')
         CALL GET_INTEGER(BL%LINE,LLOC,ISTART,ISTOP,IOUT,IN,ROWTAB,
     +   MSG='SFR ERROR LOADING TABFILES "ROWTAB"')
     ...
     +    NUMTAB,'with a maximum of ',ROWTAB,' row entries per file'
         CALL READ_TO_DATA(LINE,IN,LOUT,LOUT)
      ELSE
         IF(SFRTABFILE%NTAB.EQ.0) THEN                    !seb
...
!        CALL USTOP(' ')
        IRFG = 1
        NSTRM = ABS(NSTRM)
        !ITRFLG = 1    !option for transient routing --RICH UPDATE, BUT CHANGES INPUT...TURN ON WITH TRANSROUTE KEYWORD
        WRITE(IOUT,'(/4A/)')'NSTRM<0 Indicates acts as surrogate to ',
     +                      '"REACHINPUT" Option in OPTIONS Block.',
     +                      'Some stream information will be read ',
     +                      'by reach. This option replaces NSTRM<0'
      END IF
C
C3------READ ISFROPT FLAGS WHEN NSTRM IS LESS THAN ZERO.
      IF ( IRFG.GT.0 ) THEN
        IF ( NSFRPAR.GT.0 ) THEN
                 CALL ERR%ADD('SFR "REACHINPUT" (NSTRM < 0) OPTION '//
     +         'IS NOT ALLOWED WITH NSFRPAR > 0 (SFR PARAMETERS).'//BLN)
                 NSFRPAR = Z
        END IF
!               CALL STOP_ERROR(BLNK,IN,IOUT,
!     +           MSG='SFR "REACHINPUT" (NSTRM < 0) OPTION IS NOT '//
!     +           'ALLOWED WITH NSFRPAR > 0 (SFR PARAMETERS).')
!        NSTRM = ABS(NSTRM)
...
! RGN if ISFROPT=2 or 4 then you cannot use BCF Package.
     ...
     +           CALL ERR%ADD('If the BCF Package is used '//
     +            'and unsaturated flow is active then '//
     +            'ISFROPT must equal 3 or 5.'//BLN)
!        CALL STOP_ERROR(
!     +            BLNK,IN,IOUT,MSG='SFR: If the BCF Package is used '//
!     +            'and unsaturated flow is active then '//
!     +            'ISFROPT must equal 3 or 5.'                         )
      END IF
      nssar = 1
...
!!      ALLOCATE (DVRPERC(NCOL,NROW))   !cjm
      ...
      SWO_ENABLED = FALSE
      ALLOCATE(SEG_NSTRM(nsegdim+1))
      !
      ALLOCATE(SEG(28+NSFRAUX,nsegdim),ISEG(4,nsegdim),
...
Cdep  allocate space for stream outflow derivatives for lake package
      ...
      IF ( NUMTAB.GT.0 ) THEN
        ALLOCATE (TABFLOW(ROWTAB,nssar+NUMTAB+1), 
     +            TABTIME(ROWTAB,nssar+NUMTAB+1))
      ELSE
...
C6------PRINT INFORMATION THAT WAS READ.
        ...
        IF ( DBFILE%IU.NE.0 ) WRITE (IOUT, 1234) DBFILE%IU
 ...
 1234 FORMAT (' STREAM OUTPUT WILL BE WRITTEN TO TEXT FILE FOLLOWING ',
     +        'A DATABASE FRIENDLY FORMAT ON UNIT', I5)
 9035 FORMAT (' TRANSIENT STREAMFLOW ROUTING IS ACTIVE ')
C
C7------CHECK FOR ERRORS.
      IF ( NSFRPAR.GT.0 .AND. nparseg.LE.0 ) THEN
          CALL WRN%ADD(NL//'NO STREAM SEGMENTS DEFINED BY PARAMETERS.'//
     +        ' NSFRPAR > 0 AND NPARSEG <= 0'//NL//NL//
     +        ' SFR PACKAGE BEING TURNED OFF'//NL)
          CALL WRN%CHECK(HED='SFR COMMENTS AND WARNINGS'//NL,
     +                   INFILE=IN,OUTPUT=IOUT,TAIL=NL)
     ...
     +                   INFILE=IN,OUTPUT=IOUT,TAIL=NL,KILL=TRUE)
      ...
      IF ( IUZT.EQ.1 ) THEN
        IF ( NSTRAIL.LT.0 ) THEN
           CALL WRN%ADD('NUMBER OF TRAILING WAVES IS LESS THAN ZERO.'// 
     +        'SETTING VALUE TO ABS(NSTRAIL) TO MAKE POSITIVE.')
          NSTRAIL = ABS(NSTRAIL)
        END IF
        IF ( NSTRAIL.EQ.0 ) THEN
            CALL WRN%ADD('VERTICAL FLOW THROUGH UNSATURATED ZONE IS '// 
     +        'ACTIVE AND NUMBER OF TRAILING WAVES IS ZERO. '//
     +        ' RESETTING UNSATURATED FLOW TO BE INACTIVE'//NL)
          IUZT = 0
        END IF
      END IF
      IF ( DLEAK.LE.0.0 ) THEN
        DLEAK = 0.00001
        CALL WRN%ADD('DLEAK IS LESS THAN OR EQUAL'// 
     +        ' TO ZERO. DLEAK ASSIGNED A VALUE OF 0.0001'//NL)
      END IF
      !
      CALL WRN%CHECK(HED='SFR COMMENTS AND WARNINGS'//NL,
     +                   INFILE=IN,OUTPUT=IOUT,TAIL=NL,INIT=TRUE)
      CALL ERR%CHECK(HED='SFR FATAL ERRORS'//NL,
     +                   INFILE=IN,OUTPUT=IOUT,TAIL=NL,KILL=TRUE)
C
      IF ( IUZT.EQ.1 ) THEN
...
!IFACE
      ...
      END IF
!IFACE
        WRITE (IOUT,'(3X,200A)') REPEAT('-',ndash)
...
!IFACE
          IF(iface.eq.1) THEN
        READ(In,*,IOSTAT=I) krch, irch, jrch, jseg, ireach, STRM(1, ii),
     +                 ISTRM(6, ii)
          ELSE
        READ(In,*,IOSTAT=I) krch, irch, jrch, jseg, ireach, STRM(1, ii)
          END IF
...
!IFACE
     ...
     +                 STRM(6, ii)
          ...
          IF ( STRM(2, ii).LE.0.0 ) THEN
              CALL ERR%ADD('SLOPE IS SPECIFIED LESS THAN OR '// 
     +        'EQUAL TO ZERO FOR SEGMENT '//NUM2STR(jseg,-8)//
     +        ' REACH '//NUM2STR(ireach)//
...
!IFACE
     ...
     +                 ISTRM(6, ii)
        ...
        END IF
        !
        IF(I.NE.0) CALL SFR_STOP(IN, IOUT)
        !
        IF ( IUZT.EQ.1 ) THEN
            IF ( IBOUND(jrch, irch, krch).LE.0 )
     +          CALL WRN%ADD('CELL BENEATH STREAM REACH IS INACTIVE. '//
     +        'CELL BELOW MUST HAVE THE SAME SPECIFIC YIELD '//
     +        'AND THE TOP ELEVATION OF ACTIVE CELL MUST '//
     +        'EQUAL BOTTOM OF INACTIVE CELL. INACTIVE CELL IS BELOW '//
     +        'STREAM SEGMENT '//NUM2STR(jseg,-5)//' REACH '//
     +        NUM2STR(ireach)//
     +        '. WARN ON LINE "'//SFR_PREV_LINE(IN)//'"'//NL)
!     +          WRITE (LOUT, 9018) ireach, jseg
        END IF
...
C13-----CHECK RANGE AND ORDER FOR SEGMENTS AND REACHES.
     ...
     +                     'LESS THEN ZERO OR GREATER THEN NSS'//
     +        '. ERROR ON LINE "'//SFR_PREV_LINE(IN)//'"'//BLN)
            !WRITE (IOUT, 9023)
            !CALL USTOP(' ')
        END IF
        IF ( jseg.NE.nseg ) THEN
          ...
          nreach = 0
          IF ( jseg.NE.nseg ) THEN
              CALL ERR%ADD('SEGMENTS MUST BE IN ORDER FROM 1 '//
     +                     'THROUGH NSS. FOUND '//NUM2STR(jseg)//
     +                     ' WHEN WAS EXEPECTING '//NUM2STR(nseg)//
     +                 '. ERROR ON LINE "'//SFR_PREV_LINE(IN)//'"'//BLN)
              jseg=nseg
              !WRITE (IOUT, 9024)
              !CALL USTOP(' ')
        ...
        IF ( ireach.NE.nreach ) THEN
            CALL ERR%ADD('EACH SEGMENT MUST START WITH REACH 1, AND'//
     +                   ' REACHES MUST BE NUMBERED CONSECUTIVELY. '//
     +                     'FOUND REACH '//NUM2STR(ireach)//
     +                     ' WHEN WAS EXEPECTING '//NUM2STR(nreach)//
     +                     ' FOR SEGMENT '//NUM2STR(jseg)//
     +                '. ERROR ON LINE "'//SFR_PREV_LINE(IN)//'"'//BLN)
            ireach=nreach
            !WRITE (IOUT, 9025)
            !CALL USTOP(' ')
...
!     +          ' REACHES MUST BE NUMBERED CONSECUTIVELY')
        !
        IF(krch > Z   .AND. irch > Z   .AND. jrch > Z  .AND. 
     +     krch<=NLAY .AND. irch<=NROW .AND. jrch<=NCOL
     +                .AND. jseg > Z   .AND. ireach > Z) THEN
          !
          IF(krch < UPLAY(jrch,irch)) THEN
             CALL WRN2%ADD(
     +        NUM2STR(jseg,6)//BLNK//NUM2STR(ireach,6)//BLNK//
     +        NUM2STR(krch,6)//'    '//NUM2STR(UPLAY(jrch,irch))//NL)
              krch = UPLAY(jrch,irch) !seb automatically set krch = the upper most active layer
          END IF
          !
          ISTRM(1, ii) = krch
          ISTRM(2, ii) = irch
...
C         Number of reaches in segment added to ISEG
          ...
          IF ( ISFROPT.LT.4.AND.ISFROPT.GT.0 )THEN  
            ...
            IF ( LAYHDT(krch) > 0 .AND.IBOUND(JRCH,IRCH,KRCH) > 0 )THEN
              DTMP = STRM(4, ii)-BOTM(JRCH,IRCH,LBOTM(KRCH))
              IF ( DTMP < -1.0D-12 ) THEN
                !
                CALL ERR2%ADD(
     +            NUM2STR(jseg,6)//BLNK//NUM2STR(ireach,6)//'  '//
     +            NUM2STR(krch,6)//BLNK//NUM2STR(IRCH,6)//BLNK//
     +            NUM2STR(JRCH,6)//'   '//
     +            NUM2STR(STRM(4, ii),12)//'  '//
     +            NUM2STR(BOTM(JRCH,IRCH,LBOTM(KRCH)),12)//NL)
                !
                IF(SFR_FIX_BOT) STRM(4,ii) = BOTM(JRCH,IRCH,LBOTM(KRCH))
                !
                IFLG = 1
              ELSEIF ( DTMP < 0D0 ) THEN
                    STRM(4, ii) = BOTM(JRCH,IRCH,LBOTM(KRCH))
              END IF    
            END IF
          END IF
        ELSE
         CALL ERR%ADD('BAD MODEL LAY, ROW, COL, OR SEG, REACH ON LINE'//
     +              '. ERROR ON LINE "'//SFR_PREV_LINE(IN)//'"'//BLN)
        END IF
      END DO !NSTRM
...
C14-----READ SEGMENT INFORMATION FOR FIRST STRESS PERIOD.
     ...
     +                   MSG='SFR INPUT FAILED TO LOAD ITMP"')
        CALL GET_INTEGER(LINE,LLOC,ISTART,ISTOP,IOUT,IN,IRDFLG,
     +                   MSG='SFR INPUT FAILED TO LOAD IRDFLG"')
        CALL GET_INTEGER(LINE,LLOC,ISTART,ISTOP,IOUT,IN,IPTFLG,
     +                   MSG='SFR INPUT FAILED TO LOAD IPTFLG"')
        !
...
C         SATURATED WATER CONTENT.
            ...
            CALL ERR%ADD('RESIDUAL WATER CONTENT IS EQUAL OR GREATER '//
     +        'THAN SATURATED WATER CONTENT. FOR SEGMENT '//
     +        NUM2STR(nseg)//' REACH '//NUM2STR(irch)//
     +        'CHECK INPUT DATA FOR SPECIFIC YIELD AND '//
     +        'SATURATED WATER CONTENT.'//NL)
               THTR(irch)=THTS(irch) - THTS(irch)*1D-6
                    !WRITE (IOUT, 9026)
                    !CALL USTOP(' ')
                END IF
                IF ( THTI(irch).GT.THTS(irch) ) THEN
            CALL ERR%ADD('INITIAL WATER CONTENT IS GREATER THAN '// 
     +        'SATURATED WATER CONTENT. FOR SEGMENT '//
     +        NUM2STR(nseg)//' REACH '//NUM2STR(irch)//NL)
              THTI(irch) = THTS(irch) - THTS(irch)*1D-6
                    !WRITE (IOUT, 9027)
                    !CALL USTOP(' ')
                END IF
Cdep  Added check that THTI is greater than THTR.
                IF ( THTI(irch).LT.THTR(irch) ) THEN
                  CALL WRN%ADD('INITIAL WATER CONTENT IS LESS THAN '//
     +        'RESIDUAL WATER CONTENT FOR STREAM SEGMENT: '//
     +        NUM2STR(ISTRM(4,irch),-5)//' REACH: '//
     +        NUM2STR(ISTRM(5,irch),-5)//'. ITS INITIAL WATER '//
     +        'CONTENT RESET TO RESIDUAL OF '//NUM2STR(THTR(irch))//NL)
...
!     +                            THTR(irch)
     ...
     +               INFILE=IN,OUTPUT=IOUT,TAIL=NL,INIT=TRUE)
      !
      CALL WRN2%CHECK(HED='SFR FOUND SEGMENT/REACHES WITH A '//
     +          'INPUT LAYER SPECIFIED AS IBOUND=0.'//NL//
     +          'THE FOLLOWING HAD THEIR LAYER NUMBER LOWERED '//
     +          '(INCREASED) TO THE UPPER MOST ACTIVE LAYER'//BLN//
     +          '   SEG    RCH    LAY    LAY_NEW',
...
C18-----CHECK IF STREAM REACH IS IN ACTIVE CELL.
      ...
      DO ichk = 1, NSTRM
        ...
        IF ( IBOUND(jrck, irck, krck).EQ.0 ) THEN
          kkptflg = kkptflg + 1
          CALL WRN%ADD(NUM2STR(jsegck,-5)//BLNK//NUM2STR(ireachck,-5)//
     +          BLNK//NUM2STR(krck,-5)//BLNK//NUM2STR(irck,-5)//
     +          BLNK//NUM2STR(jrck)//NL)
        ELSE IF ( IBOUND(jrck, irck, krck).LT.0 ) THEN
           CALL ERR%ADD(NUM2STR(jsegck,-5)//BLNK//NUM2STR(ireachck,-5)//
     +          BLNK//NUM2STR(krck,-5)//BLNK//NUM2STR(irck,-5)//
     +          BLNK//NUM2STR(jrck)//NL)
!            WRITE(LOUT, 9030)jsegck, ireachck, IBOUND(jrck, irck, krck),
!     +                       krck, irck, jrck
        END IF
      END DO
      IF ( kkptflg.EQ.1 ) THEN
           CALL WRN%ADD(NL//'ONLY 1 STREAM REACH WAS '// 
     +      'ASSIGNED TO A CELL WHERE THE IBOUND ARRAY WAS ZERO.'//NL//
     +      ' PROGRAM SEARCHES FOR UPPERMOST ACTIVE CELL IN VERTICAL'//
     +      ' COLUMN,IF ALL CELLS ARE INACTIVE, STREAM LEAKAGE WILL'//
     +      ' NOT BE ALLOWED. THIS OCCURED AT ROW '//NUM2STR(irck,-5)//
     +      '; COLUMN '//NUM2STR(jrck)//NL)
          !WRITE (LOUT, 9031)
      ELSE IF ( kkptflg.GT.1 ) THEN
           CALL WRN%ADD(NL//'A TOTAL OF '//NUM2STR(kkptflg,-5)//
     +     ' STREAM REACHES WERE ASSIGNED TO CELLS WHERE THE IBOUND '//
     +     'ARRAY WAS ZERO.'//NL//'SFR WILL SEARCH FOR UPPERMOST '//
     +     'ACTIVE CELL BENEATH THE STREAM REACH.'//BLN//
     +     'IF ALL CELLS BENEATH STREAM REACH ARE INACTIVE,'//
     +     ' STREAM LEAKAGE WILL NOT BE ALLOWED.'//NL)
          !WRITE (LOUT, 9032) kkptflg
      END IF
      !
      IF(WRN%RAISED) 
     +CALL WRN%CHECK(HED='SFR CONTAINS STREAM REACHES THAT ARE '//
     +     'ASSIGNED TO CELLS THAT HAVE IBOUND=0'//NL//
     +     'SFR WILL SEARCH FOR THE UPPER MOST ACTIVE MODEL CELL '//
     +     'BENEATH THE STREAM REACH'//NL//
     +     'IF ALL CELLS BENEATH REACH ARE IBOUND=0, THEN STREAM '//
     +     'LEAKAGE WILL NOT OCCUR (NO GROUNDWATER TO LEAK TOO).'//BLN//
     +     'THE FOLLOWING STREAM REACHES HAVE THE IBOUND=0'//BLN//
     +     'SEG   RCH LAY    ROW    COL',
     +      INFILE=IN,OUTPUT=IOUT,TAIL=NL,NO_NL=TRUE,INIT=TRUE)
      IF(ERR%RAISED)
     +CALL ERR%CHECK(HED='SFR CONTAINS STREAM REACHES THAT ARE '//
     +  'ASSIGNED TO CELLS THAT HAVE CONSTANT HEAD (IBOUND<0)'//BLN//
     +  'THE FOLLOWING STREAM REACHES HAVE THE IBOUND<0'//BLN//
     +  'SEG   RCH LAY    ROW    COL',
     +   INFILE=IN,OUTPUT=IOUT,TAIL=NL,NO_NL=TRUE,INIT=TRUE)
C
...
C     ------------------------------------------------------------------
C
      IF ( Iunitlpf.NE.0 .OR. Iunithuf.NE.0 .OR. Iunitupw.NE.0 ) THEN 
        IF ( ISFROPT.EQ.2.OR.ISFROPT.EQ.4 )
...
! set streambed K when option to use KV or KH are defined
      ...
      END IF
      !
      IF(.NOT. DBFILE%BINARY) THEN 
         IF(    IUZT.EQ.0 .AND. DBFILE%IU.NE.0) THEN                    !WRITE HEADER FOR DBFILE IF REQUESTED
             CALL DBFILE%SET_HEADER( ' DATE_START               '//
     +   'PER     STP             DELT          '//
     +   'SIMTIME    SEG    RCH          FLOW_IN     FLOW_SEEPAGE'//
     +   '         FLOW_OUT           RUNOFF           PRECIP        '//
     +   'STREAM_ET      HEAD_STREAM     HEAD_AQUIFER     '//
     +   'DEPTH_STREAM     WIDTH_STREAM    LENGTH_STREAM    '//
     +   'HEAD_GRADIENT      COND_STREAM   ELEV_UP_STREAM'
     +                             )
            !
         ELSEIF(IUZT.NE.0 .AND. DBFILE%IU.NE.0) THEN
            !
            CALL DBFILE%SET_HEADER( ' DATE_START               '//
     +   'PER     STP             DELT          '//
     +   'SIMTIME    SEG    RCH          FLOW_IN     FLOW_SEEPAGE'//
     ...
     +                           )
      ...
      jseg = 0
      DO I = 1, NSTRM 
          IF( jseg .NE. ISTRM(4,I)) THEN
              jseg = ISTRM(4,I)
              SEG_NSTRM(jseg) = I - 1  !NRCH = SEG_NSTRM(jseg+1) - SEG_NSTRM(jseg) and ISTRM = SEG_NSTRM(jseg) + irch
          END IF
      END DO
      SEG_NSTRM(nsegdim+1) = NSTRM
...
C     ******************************************************************
      USE GWFSFRMODULE
      USE GLOBAL,       ONLY: ISSFLG, IBOUND, BOTM, HNEW, NLAY, 
     +                        LAYHDT, IUNIT, SUBLNK, UPLAY, LBOTM
      USE PARAMMODULE,  ONLY: MXPAR, PARTYP, IACTIVE, IPLOC
      USE GWFSUBMODULE, ONLY: LPFLNK,DVZ,DVZC                    !wschmid added 
      USE ICHKSTRBOT_MODULE
      USE UTIL_INTERFACE,            ONLY: STOP_ERROR
      USE WARNING_TYPE_INSTRUCTION,  ONLY: WARNING_TYPE
      USE CONSTANTS,                 ONLY: BLNK, BLNK3, NL, BLN, TRUE
...
Cdep added NSFRPAR to IF statement
      ...
      !END IF
      !
      IF ( NSFRPAR.GT.0 .OR. Kkper.GT.1 ) THEN
         !
         NP   = 0
         !
         IF(SFR_AUTO_NEG_ITMP) THEN
             ITMP = -1
         ELSE
             LLOC = 1
             CALL READ_TO_DATA(LINE,IN,LOUT)
             !
             CALL GET_INTEGER(LINE,LLOC,ISTART,ISTOP,IOUT,IN,ITMP,
     +                    MSG='SFR INPUT FAILED TO LOAD "ITMP"'//BLN//
     +          'IF YOU WANT TO AUTOMATICALLY SET'//NL//
     +          'ITMP = -1'//NL//
     +          'AFTER THE FIRST STRESS PERIOD.'//NL//
     +          'YOU MUST INCLUDE THE OPTION "AUTOMATIC_NEGATIVE_ITMP"')
            !
            IF ( ITMP > 0 .OR. NSFRPAR.GT.0) THEN                     !ITMP > 0 INDICATES THE OTHER VARIABLES MUST BE READ
              CALL GET_INTEGER(LINE,LLOC,ISTART,ISTOP,IOUT,IN,IRDFLG,
     +                         MSG='SFR INPUT FAILED TO LOAD "IRDFLG"')
              CALL GET_INTEGER(LINE,LLOC,ISTART,ISTOP,IOUT,IN,IPTFLG,
     +                         MSG='SFR INPUT FAILED TO LOAD "IPTFLG"')
              !
            ELSE                            ! ASSUME ITMP < 0 HAS OPTIONAL REUSE OF IRDFLG and IPTFLG
                  I = IRDFLG
              CALL GET_INTEGER(LINE,LLOC,ISTART,ISTOP,IOUT,IN,IRDFLG,
     +                         HAS_ERROR=HAS_ERROR)
              IF( HAS_ERROR ) THEN                                     !Erorr thrown so just reuse previous value
                  IRDFLG = I
              ELSE
                  I = IPTFLG
                  CALL GET_INTEGER(LINE,LLOC,ISTART,ISTOP,IOUT,IN,      !If error is thrown just set to previous value
     +                             IPTFLG, ERROR_VAL=I)
              END IF
            END IF
            !
     ...
     +                          MSG='SFR INPUT FAILED TO LOAD "NP"')
            END IF
         END IF
      END IF
C
...
C         SEGMENTS. Moved NSEGCK below ELSE IF 6/9/2005 dep
      ...
      ELSE IF ( Kkper.EQ.1 ) THEN
        CALL USTOP('  ***  STREAM SEGMENTS MUST BE DEFINED FOR '// 
...
C14-----IDENTIFY SEGMENTS THAT DO NOT RECEIVE TRIBUTARY FLOW.
            ...
            ELSE
              ISEG(3, nseg) = 7
              k7 = k7 + 1
                IF ( jj.NE.1 ) WRITE (LOUT, 9011) nseg, jj
            END IF
          END IF
        END DO
C
...
!             
              ...
              !IF ( IERR.GT.0 )IFLG = IERR
          !!!!!
          IF ( LAYHDT(krck).GT.0 .AND.IBOUND(Jrck,Irck,Krck).GT.0 ) THEN
              !
              DTMP = STRM(4, irch)-BOTM(Jrck,Irck,LBOTM(Krck))
              IF ( DTMP < -1.0D-12 ) THEN
                !
              CALL ERR%ADD(
     +          NUM2STR(nseg,6)//BLNK//NUM2STR(II,6)//'  '//
     +          NUM2STR(krck,6)//BLNK//NUM2STR(Irck,6)//BLNK//
     +          NUM2STR(Jrck,6)//'   '//
     +          NUM2STR(STRM(4, irch),12)//'  '//
     +          NUM2STR(BOTM(Jrck,Irck,LBOTM(Krck)),12)//NL)
                !
                IF(SFR_FIX_BOT) STRM(4,irch)=BOTM(Jrck,Irck,LBOTM(Krck))
                !
                IFLG = 1
              ELSEIF ( DTMP < 0D0 ) THEN
                    STRM(4, irch) = BOTM(Jrck,Irck,LBOTM(Krck))
              END IF  
          END IF
          !!!!!
!
...
!dep 4/28/2008 Added check and warning for streambed thickness
            IF (STRM(8, irch).LT.CLOSEZERO)THEN
                WRITE (LOUT, 9030) nseg, irch, STRM(8, irch)
              STRM(8, irch) = 1.0
            END IF
...
C21-----STOP IF ICALC LESS THAN 0 AND GREATER THAN 4.
            ELSE
               CALL STOP_ERROR(MSG='SFR SEGMENT '//
     +                       NUM2STR(nseg)//' AND REACH '//NUM2STR(II)//
     +                       ' HAS AN ICALC = '//NUM2STR(icalc)//
     +      ' WHICH IS NOT ALLOWED. ICALC MAY ONLY BE 0, 1,2,3, OR 4')
            !WRITE(*,'(A)') 'icalc problem, < 0 or > 4'
            !ERROR STOP
            END IF
            !
            IF(STRM(5, irch) < NEARZERO) THEN  !Very small stream width defined
                !
                CALL WRN%ADD(NUM2STR(ICALC,6)//BLNK//
     +          NUM2STR(nseg,6)//BLNK//NUM2STR(II,6)//BLNK3//
     +          NUM2STR(STRM(5, irch))//NL)
            END IF
            !
            sumlen = sumlen + rchlen
            irch = irch + 1
        ...
        !
      !!!
      CALL WRN%CHECK(HED='SFR HAS SEGMENT/REACHES WITH ICALC = 0 OR 1'//
     +          ' WITH VERY SMALL WIDTHS.'//NL//
     +          'THIS MAY CAUSE FLOATING INVALID ERRORS'//NL//
     +          'THE FOLLOWING HAVE VERY SMALL WIDTHS:'//BLN//
     +          ' ICALC    SEG    RCH   WIDTH',
     +          INFILE=IN,OUTPUT=IOUT,TAIL=NL, INIT=TRUE, NO_NL=TRUE)
        !
      !!!
...
C22-----CHECK VALUES IN STREAM CROSS SECTION LIST (XSEC).
        DO nseg = 1, NSS
          ...
          IF ( icalc.EQ.2 ) THEN
            IF ( ABS(XSEC(1,nseg)).GT.zero ) THEN
                CALL ERR%ADD(NUM2STR(nseg,6)//BLNK//NUM2STR(jj,6)//
     +          BLNK//NUM2STR(XSEC(jj, nseg),10)//
     ...
     +          '  NEGATIVE X DISTANCE')
              ...
              IF ( XSEC(kk, nseg).LT.0.0 ) THEN
                CALL ERR%ADD(NUM2STR(nseg,6)//BLNK//NUM2STR(kk,6)//
     +          BLNK//NUM2STR(XSEC(kk, nseg),10)//
     +          '  NEGATIVE Z DISTANCE')
 ...
 9030   FORMAT (/, ' *** WARNING *** STREAMBED THICKNESS', 
     ...
     +          ' IS ', ES12.4,' WHICH IS ZERO OR LESS. '/,
        ...
        DO nseg = 1, NSS
          ...
          IF ( icalc.EQ.1 ) THEN
            ...
            IF ( rough.LE.0.0 ) THEN
                CALL ERR%ADD(NUM2STR(nseg,6)//BLNK//NUM2STR(icalc,6)//
     +          BLNK//NUM2STR(rough,10)//'  = ROUGHCH')
          ...
          ELSE IF ( icalc.EQ.2 ) THEN
            ...
            IF ( roughch.LE.0.0 ) THEN
                CALL ERR%ADD(NUM2STR(nseg,6)//BLNK//NUM2STR(icalc,6)//
     +          BLNK//NUM2STR(roughch,10)//'  = ROUGHCH')
                !WRITE (IOUT, 9018) roughch
                !CALL USTOP(' ')
            END IF
            !    
            IF ( roughbnk.LE.0.0 ) THEN
                CALL ERR%ADD(NUM2STR(nseg,6)//BLNK//NUM2STR(icalc,6)//
     +          BLNK//NUM2STR(roughbnk,10)//'  = ROUGHBK')
                !WRITE (IOUT, 9019) roughbnk
                !CALL USTOP(' ')
            END IF
          END IF
        END DO
      CALL ERR%CHECK(HED='SFR HAS SEGMENTS HAVE MANNING’S ROUGHNESS '//
     +         'COEFFICIENTS THAT ARE LESS THAN OR EQUAL TO ZERO.'//NL//
     +         'THE FOLLOWING SEGMENT AND POINT NUMBER (PNT) '//
     +         'NEED TO BE FIXED. VALUE IS HE LOADED INPUT.'//BLN//
     +         '   SEG ICALC      VALUE  INPUT',
     +          INFILE=IN,OUTPUT=IOUT,TAIL=NL,KILL=TRUE, NO_NL=TRUE)
        ...
        DO nseg = 1, NSS
          ...
          IF ( icalc.EQ.4 ) THEN
            ...
            IF ( flwlw.LE.0.0 ) THEN
                CALL WRN%ADD(NUM2STR(nseg,6)//
     +               '  FIRST FLOW VALUE <0.1, RESET TO 0.1')
            ...
            IF ( dpthlw.LE.0.0 ) THEN
                CALL WRN%ADD(NUM2STR(nseg,6)//
     +               '  FIRST DEPTH VALUE <0.01, RESET TO 0.01')
                !WRITE (LOUT, 9021) nseg
              QSTAGE(1+nstrpts, nseg) = 0.01
            END IF
            wdthlw = QSTAGE(1+2*nstrpts, nseg)
            IF ( wdthlw.LE.0.0 ) THEN
                CALL WRN%ADD(NUM2STR(nseg,6)//
     +           '  FIRST WITH VALUE <=0.0, RESET TO 1.0 - CHECK INPUT')
                !WRITE (LOUT, 9022) nseg
            ...
            DO ipt = 2, nstrpts
              ...
              IF ( flw2.LE.flw1 ) THEN
                CALL ERR%ADD(NUM2STR(nseg,6)//BLNK//NUM2STR(ipt,6)//
     +               BLNK//NUM2STR(flw2,10)//'  BAD FLOWTAB')
                 !WRITE (IOUT, 9023) nseg, flw2, ipt
                 !CALL USTOP(' ')
              END IF
              IF ( dpth2.LE.dpth1 ) THEN
                CALL ERR%ADD(NUM2STR(nseg,6)//BLNK//NUM2STR(ipt,6)//
     +               BLNK//NUM2STR(dpth2,10)//'  BAD DPTHTAB')
                  !WRITE (IOUT, 9024) nseg, dpth2, ipt
                  !CALL USTOP(' ')
              END IF
                !IF (wdth2.LT.wdth1) WRITE (LOUT, 9025) nseg, wdth2, ipt
                IF (wdth2.LT.wdth1) CALL WRN%ADD(NUM2STR(nseg,6)//
     +              '  WIDTH AT TABLE POINT '//NUM2STR(ipt)//
     +              ' IS SMALLER THAN PREVIOUS TABLE POINT; '//
     +              'TYPICALLY, STREAM WIDTH INCREASES WITH FLOW')
            END DO
          END IF
        END DO
C
          !!WRITE (IOUT, 9026)
      END IF
      CALL WRN%CHECK(HED='SFR SEGMENTS WITH ICALC = 4'//BLN//
     +     'HAVE WARNINGS ASSOCAITED WITH THEIR "TABLE OF '//
     +     'FLOW VERSUS DEPTH AND WIDTH".'//BLN//
     +     ' -- THAT IS WITH INPUT VAIRABLES: FLOWTAB, DPTHTAB, WDTHTAB'
     +     //BLN//'   SEG  COMMENT',
     +     INFILE=IN,OUTPUT=IOUT,TAIL=NL,INIT=TRUE, NO_NL=TRUE)
      !
      CALL ERR%CHECK(HED='SFR SEGMENTS WITH ICALC = 4'//BLN//
     +     'HAVE FATAL ERRORS ASSOCAITED WITH THEIR "TABLE OF '//
     +     'FLOW VERSUS DEPTH AND WIDTH".'//BLN//
     +     ' -- THAT IS WITH INPUT VAIRABLES: FLOWTAB, DPTHTAB, WDTHTAB'
     +     //BLN//'   SEG POINT      VALUE  COMMENT',
     +     INFILE=IN,OUTPUT=IOUT,TAIL=NL,KILL=TRUE, NO_NL=TRUE)
      !
...
Crth --Include adjustment to BOTM if linked to SUB
          ...
          IF( ISTRM(4, irch).GT.1 ) THEN
            IF( IDIVAR(1,ISTRM(4, irch)-1).GT.0 ) THEN
              ...
              IF( ISTRM(5, irch).EQ.1 )  THEN
                IF(SUBLNK) THEN
                    SEG(13,ISTRM(4,irch)-1)=SEG(13,ISTRM(4,irch)-1)-
     1              DVZC(ICP,IRP,ILP)+DVZ(ICP,IRP,ILP)                   !subtract cum. displacement (but not last timestep's displacement: will be subtracted in AD routine)
                    DDIFF(ISTRM(4,irch)-1)=
     +                          BOTM(icp,irp,0)-SEG(13,ISTRM(4,irch)-1)
                ELSE
                    SEG(13, ISTRM(4,irch)-1) =
     +                    BOTM(icp, irp, 0) - SEG(13, ISTRM(4, irch)-1 )
                    DDIFF(ISTRM(4,irch)-1)=SEG(13, ISTRM(4,irch)-1)
                END IF
              END IF
            END IF
          END IF
...
C
          WRITE (IOUT, 9029)
 9029 FORMAT (//)
      DEALLOCATE(DDIFF, STAT=irch)  !STAT IS TO PREVENT ANY DEALLOCATION ERRORS
      END IF
C
...
!            IF ( HNEW(ic, ir, ilay).LE.BOTM(ic,ir,ilay) .OR.
!     +           IBOUND(ic, ir, il).EQ.0 ) THEN
!              ilay = ilay + 1
!            ELSE
!              EXIT TOPCELL
!            END IF
!          END DO TOPCELL
          !
          ilay = UPLAY(IC,IR)    !all layers are IBOUND=0
          IF(ilay == 0) THEN
              ilay = NLAY+1
          ELSEIF(IL > ilay) THEN !User specified deeper layer then found
              ilay = IL
          END IF
          !
...
C     *****************************************************************
      ...
      USE GLOBAL,       ONLY: NLAY, ISSFLG, IBOUND, HNEW, HCOF, 
     +                        RHS, BOTM, LBOTM, DELR, DELC, HNEW_OLD
      ...
      USE CONSTANTS, ONLY: NL, BLN, BLNK, TRUE, FALSE
      USE NUM2STR_INTERFACE, ONLY: NUM2STR
      IMPLICIT NONE
...
C         TO ZERO FOR LAKE PACKAGE.
      IF (CNVG_WRN%RAISED) CALL CNVG_WRN%INIT()
      itstr = 0
      iprvsg = -1
...
!          flowin = 0.0D0
          flowin = strm(10, l)
          dvrsn = 0.0D0
!         rhsh1 = 0.0d0   rgn commented this out
!         rhsh2 = 0.0d0
!         hcofh1 = 0.0d0
!         hcofh2 = 0.0d0
          Transient_bd = 0.0
     ...
     +         ( Lgriter.GT.1 .OR. Igrid.GT.1 )) THEN
              DO i = 1, NSFRAUX
                IF ( TRIM(SFRAUX(i)).EQ.'LGRGRID' ) THEN
                  lgrgrid = INT(SEG(28+i, istsg))
                  IF ( istsg .GT. 1 ) THEN
                    iprvsg = ISTRM(4, ll)
                    lgrgridprv = INT(SEG(28+i, iprvsg))
                  END IF
                END IF
                IF ( TRIM(SFRAUX(i)).EQ.'LGRSEG' )
     +            lgrseg = INT(SEG(28+i, istsg))
              END DO
              IF ( lgrgrid.GT.0 ) THEN
                SEG(28,istsg) =  GWFSFRDAT(lgrgrid)%SGOTFLW(lgrseg)
                SEG(2,istsg) = SEG(28,istsg)
              END IF
...
C         STRIN FOR LAKE PACKAGE.
            IF ( istsg.GT.1 ) THEN
              ...
              IF ( Iunitlak.NE.0 ) STRIN(iprvsg) = STRM(9, ll)
            END IF
C
C7------COMPUTE INFLOW OF A STREAM SEGMENT EMANATING FROM A LAKE.
            IF ( (Iunitlak.NE.0) .AND. (IDIVAR(1,istsg).LT.0) ) THEN
...
Cwschmid: end of insert
          ...
          !
          NO_GW = IBOUND(ic, ir, il).EQ.0
          !
            SUMLEAK(l) = 0.0D0
            SUMRCH(l)  = 0.0D0
            h = HNEW(ic, ir, il)
            !
            IF(HD_RELAX < 1D0 .AND. .NOT. NO_GW) THEN
             !
             h = HNEW_OLD(ic,ir,il) + HD_RELAX*(h - HNEW_OLD(ic,ir,il)) !OLD + RELAX*(NEW-OLD)
             !
            END IF
C
...
! RGN might need to correct depth for smooth width
            IF(flwmpt<NEARZERO .OR. qcnst<NEARZERO) THEN
                depth = 0D0
            ELSE
...
C         AND SET DEPTH TO DEPTHP AND FLOBOT TO FLOBOTP.
              IF ( itot.GE.100 ) THEN
                iflg = 0
                CALL CNVG_WRN%ADD(
     +            NUM2STR(istsg,6)//BLNK//NUM2STR(nreach,6)//'  '//
     +            NUM2STR(il,6)//BLNK//NUM2STR(ir,6)//BLNK//
     +            NUM2STR(ic,6)//NL )
!!!                  WRITE (LOUT, 9003) istsg, nreach, Kkiter, err, errold
...
C         STRIN FOR LAKE PACKAGE.
          IF ( l.EQ.NSTRM ) THEN
            ...
            IF ( Iunitlak.NE.0 ) STRIN(istsg) = STRM(9, l)
          END IF
...
C73-----ROUTE SEEPAGE THROUGH UNSATURATED ZONE.
            ...
            !
            ! seb do not calcualte unsaturated flow if underlying cells are all dry
            !
            IF( NO_GW ) kerp = 0
            !
...
C     *****************************************************************
      ...
      USE GLOBAL,       ONLY: NCOL, NROW, NLAY, ISSFLG, IBOUND, UPLAY,
     +                        HNEW, BUFF, BOTM, LBOTM, IUNIT, SPTIM
!IFACE
...
!      USE GWFUZFMODULE,ONLY:FINF  !cjm
      ...
      USE CONSTANTS, ONLY: NL, BLN, TRUE, FALSE
      IMPLICIT NONE
...
C-------SET POINTERS FOR THE CURRENT GRID.
      ...
      IF(ILGR .NE. 0) CALL SGWF2LAK7PNT(IGRID)  !seb lgr
C
      IF(DBFILE%IU.NE.0 .AND. MOD(KKPER,11)==0) CALL DBFILE%SIZE_CHECK() !CHECK SIZE EVERY 10 STRESS PERIODS
C
C     If there was a convergence error for the previous iteration, print warning
C
      IF (CNVG_WRN%RAISED) CALL CNVG_WRN%CHECK( HED='SFR WARNING'//BLN//
     +'FOR SOME SFR SEGMENTS/REACHES THE '//
     +'ESTIMATE OF STREAM DEPTH AND STREAMBED LEAKAGE'//NL//
     +'FAILED TO CONVERGE TO DLEAK AFTER 100 BISECTION ITERATIONS.'
     +                                                      //BLN//
     +'STREAM DEPTH AND STREAMBED LEAKAGE WILL USE THE LAST'//
     +'CALCULATED VALUED FROM THE SFR INTERNAL SOLVER.'
     +                                                      //BLN//
     +'THE FOLLOWING ARE THE SEGMENTS/REACHES THAT FAILED TO CONVERGE:'
     +                                                      //BLN//
     +   '   SEG    RCH     LAY    ROW    COL',
     +   OUTPUT=IOUT, TAIL=NL, INIT=TRUE, NO_NL=TRUE)
C
C
...
C         ACCUMULATORS (RATIN AND RATOUT).
      ...
      END IF
      !
      ! seb Reset GW flow to streams by segment so sum is done correctly
      IF(DO_FM_BD) THEN
      DO CONCURRENT (l=1:NSEGDIM)
                                  SEG(27,l) = 0.0
      END DO
      END IF
C
...
Cdep    revised to allow for compact and non compact budgets
      ...
      ELSE IF ( ISTCB1.GT.0 .AND. ICBCFL.NE.0 ) THEN
        ...
        iout1 = ISTCB1
      END IF
      IF ( ISTCB2.GT.0) THEN  ! .AND. ICBCFL.NE.0  seb -- made it so it is always written to external file
        ibdst = -1
        iout2 = ISTCB2
...
C         WHEN THERE ARE STREAMS.
      iprvsg = -1
      IF ( Iunitlak.NE.0 .AND. DO_FM_BD) THEN
        DO i = 1, NSS
          STRIN(i) = 0.0
Cdep          STROUT(i) = 0.0
Cdep          FXLKOT(i) = 0.0
...
C5-----START INTERNAL TIME LOOP FOR ROUTING STREAMFLOWS.
      deltinc = DELT
      IF ( IRTFLG .GT. 0 .AND. iss.EQ.0 .AND. DO_FM_BD) THEN
        deltinc = DELT/FLOAT(numdelt)
      ELSE
        numdelt = 1
      END IF
...
! RGN 5/9/09 set slope for all icalc
          ...
          IF(.NOT. DO_FM_BD ) THEN
            ...
            totflwt = 0.0D0
            totdelstor = 0.0D0
            IF ( icalc.GE.1 ) depth   = STRM(7, l)  
            IF ( icalc.GE.2 ) width   = STRM(5, l)  
...
C         STRIN FOR LAKE PACKAGE.
            IF ( istsg.GT.1 ) THEN
              ...
              SGOTFLW(iprvsg) = STRM(9, ll)
              IF ( Iunitlak.NE.0 ) STRIN(iprvsg) = STRM(9, ll)
              IF ( Iunitlak.NE.0 .AND. IDIVAR(1,istsg).LT.0 ) 
...
C22-----SUM TRIBUTARY OUTFLOW AND USE AS INFLOW INTO DOWNSTREAM SEGMENT.
            IF ( istsg.GE.1 .AND. ISEG(3, istsg).EQ.7 ) THEN
              ...
              IF(IUNIT(49).NE.0) THEN  !IUNIT(49): LMT
              IF(ABS(SEG(2,ISTSG)).GT.CLOSEZERO) THEN  !Possible to have both tributary inflow and specified inflow. if the latter exist, count it next
                  NINTOT = NINTOT + 1   !EDM
              ENDIF
              END IF
C
...
C26-----DETERMINE LEAKAGE THROUGH STREAMBED.
 ...
 !               WETPER(1, l) = width
          ...
          END IF
C26a----CHECK IF LAKE INUNDATES STREAM. IF SO THEN SET avhc and cstr = 0.
          IF ( Iunitlak.NE.0 ) THEN                                     !SCOTT why are you doing illake = il-1? What about when il=1
            illake = il - 1
            IF ( illake.GE.1 ) THEN
              lakid = LKARR1(ic, ir, illake)
              IF ( lakid.GT.0 ) THEN
...
!  First, need some terms to send to CALC_XSA
            qlat = (runof + runoff + precip - etstr)/strlen
            qa = STRM(25,l)  !seb originally qa = STRM(10,l), but changed by RGN
            qb = STRM(9,l)
            IF ( icalc.EQ.3 ) THEN
              cdpth = SEG(9, istsg)
              fdpth = SEG(10, istsg)
...
C         REQUESTED (ISTCB1<0 and IBD<0)AND NO UNSATURATED FLOW.
     ...
     +                           STRM(3, l)           
            ...
            END IF
C
C41A-----PRINT STREAMFLOWS AND RATES FOR EACH REACH TO STREAM LIST
C         IF REQUESTED (DBFILE.NE.0).
         IF(DBFILE%IU.NE.0 .AND. irt==numdelt) THEN
            IF(HAS_STARTDATE) THEN
                DATE = DATE_SP(KKPER)%TS(KKSTP-1)%STR('T')
            ELSE
                DATE='   NaN'
            END IF
!            WRITE (DBFILE%IU,'(A, 2( "," 1x ES16.9), 
!     +                         2( "," 1x I6), 13( "," 1x ES16.9))')
              DB_OUT(1) = STRM(10, l)
              DB_OUT(2) = STRM(11, l)
              DB_OUT(3) = STRM(9, l) 
              DB_OUT(4) = runof+runoff
              DB_OUT(5) = precip    
              DB_OUT(6) = etstr   
              DB_OUT(7) = STRM(15, l)  
              DB_OUT(8) = h           
            ...
            IF(DBFILE%BINARY) THEN
              WRITE(DBFILE%IU)
     +         DATE, Kkper, Kkstp, SPTIM(KKPER)%DT(KKSTP), 
     +         DBLE(TOTIM), ISTRM(4, l), ISTRM(5, l), DB_OUT!INCLUDES UZF TERMS ZEROED OUT
            ELSE
              WRITE (DBFILE%IU,'(1x A, 1x 2I8, 2( 1x ES16.9), 
     +                           2( 1x I6), 14( 1x ES16.9))')
     +         DATE, Kkper, Kkstp, SPTIM(KKPER)%DT(KKSTP),
     +         TOTIM, ISTRM(4, l), ISTRM(5, l), DB_OUT(:14)
            END IF
         END IF
...
!     +                         2( "," 1x I6), 15( "," 1x ES16.9))')
              ...
              DB_OUT(13) = STRM(16, l)
              DB_OUT(14) = STRM(3, l)
              DB_OUT(15) = totflwt/SPTIM(KKPER)%DT(KKSTP)
              DB_OUT(16) = totdelstor/SPTIM(KKPER)%DT(KKSTP)
            IF(DBFILE%BINARY) THEN
              WRITE (DBFILE%IU)
     +         DATE, Kkper, Kkstp,SPTIM(KKPER)%DT(KKSTP),
     +         DBLE(TOTIM), ISTRM(4, l), ISTRM(5, l), DB_OUT
            ELSE
              WRITE (DBFILE%IU,'(1x A, 1x 2I8, 2( 1x ES16.9), 
     +                           2( 1x I6), 16( 1x ES16.9))')
     +         DATE, Kkper, Kkstp,SPTIM(KKPER)%DT(KKSTP),
     +         TOTIM, ISTRM(4, l), ISTRM(5, l), DB_OUT
            END IF
         END IF
C
...
C45-----RECORD STREAM GAGING STATION DATA (IF SOLUTE TRANSPORT NOT ACTIVE).
          IF ( Iunitgwt.EQ.0 .AND. Iunitgage.NE.0 ) THEN
...
Cadded 6/27/2008
 ...
 9004 FORMAT (1X, ///1X, A, '   PERIOD ', I6, '   STEP ', I8, //, 
     ...
     +        'STREAMBED  STREAMBED  STREAMBED', /16X, 
...
!--------REVISED FOR MODFLOW-2005 RELEASE 1.9, FEBRUARY 6, 2012
C     ******************************************************************
      USE GWFSFRMODULE, ONLY: SFRUZBD, CLOSEZERO, IOUT
      IMPLICIT NONE
      INTRINSIC ABS
C     ------------------------------------------------------------------
C     SPECIFICATIONS:
...
C
 9001 FORMAT ('1', /2X, 
     ...
     +        'STREAMS AT END OF TIME STEP', I6, ' STRESS PERIOD',
     +         I4/2X, 96('-'))
...
C7------RETURN.
      ...
      END FUNCTION
      !
      SUBROUTINE SFR_STOP_MSG(IN, IOUT, MSG)
      USE UTIL_INTERFACE, ONLY: STOP_ERROR
      ...
      READ(IN, '(A)') LINE
      !
      CALL STOP_ERROR(LINE,IN,IOUT,'SFR INPUT ERROR: '//MSG)
      !
      END SUBROUTINE
      !
      SUBROUTINE SFR_STOP(IN, IOUT)
      USE UTIL_INTERFACE, ONLY: STOP_ERROR
      INTEGER,      INTENT(IN):: IN, IOUT
      CHARACTER(700):: LINE
      !
      ...
      !
      CALL STOP_ERROR(LINE,IN,IOUT,
     +    'SFR INPUT ERROR: FAILURE TO READ A NUMBER ON THE INPUT LINE')
      !
      END SUBROUTINE
C
...
C2------ONLY READ FIRST 4 VARIABLES TO DETERMINE VALUE OF IUPSEG.
        READ (In, *, IOSTAT=I) n, icalc, noutseg, iupseg
        !
        IF(I.NE.0) CALL SFR_STOP(IN, IOUT) !, IOSTAT=I
        !
!       IF ( n.GT.NSS .OR. n.LT.1 ) THEN  !cjm (commented this line out)
        IF ( n.GT.NSS .OR. n.EQ.0 ) THEN              !cjm
            CALL SFR_STOP_MSG(IN, IOUT, 
     +                  'SEGMENT NUMBER (NSEG) IS OUT OF RANGE ON LINE')
!            WRITE (IOUT, 9001) n
...
C4------READ DATA SET 4B FOR SEGMENTS THAT ARE NOT DIVERSIONS.
        ...
        IF ( iupseg.LE.0 ) THEN
          IF ( icalc.LE.0 ) THEN
            READ (In, *, IOSTAT=I) idum, ISEG(1, nseg), IOTSG(nseg), 
...
C6------READ DATA SET 4C.
        IF ( ISFROPT.EQ.0 ) THEN
          IF ( icalc.LE.0 ) THEN
            READ (In, *, IOSTAT=I) (SEG(jj, nseg), jj=6, 10)
          ELSE IF ( icalc.EQ.1 ) THEN
            READ (In, *, IOSTAT=I) (SEG(jj, nseg), jj=6, 9)
          ELSE IF ( icalc.GE.2 .AND. icalc.LE.4 ) THEN
            READ (In, *, IOSTAT=I) (SEG(jj, nseg), jj=6, 8)
          END IF
        ELSE IF ( ISFROPT.EQ.1 ) THEN
          IF ( icalc.LE.0 ) THEN
            READ (In, *, IOSTAT=I) SEG(9, nseg), SEG(10, nseg)
          ELSE IF ( icalc.EQ.1 ) THEN
            READ (In, *, IOSTAT=I) SEG(9, nseg)
          END IF
...
C7------READ DATA SET 4D.
        IF ( ISFROPT.EQ.0 ) THEN
          IF ( icalc.LE.0 ) THEN
            READ (In, *, IOSTAT=I) (SEG(jj, nseg), jj=11, 15)
          ELSE IF ( icalc.EQ.1 ) THEN
            READ (In, *, IOSTAT=I) (SEG(jj, nseg), jj=11, 14)
          ELSE IF ( icalc.GE.2 .AND. icalc.LE.4 ) THEN
            READ (In, *, IOSTAT=I) (SEG(jj, nseg), jj=11, 13)
          END IF
        ELSE IF ( ISFROPT.EQ.1 ) THEN
          IF ( icalc.LE.0 ) THEN
            READ (In, *, IOSTAT=I) SEG(14, nseg), SEG(15, nseg)
          ELSE IF ( icalc.EQ.1 ) THEN
            READ (In, *, IOSTAT=I) SEG(14, nseg)
          END IF
        ELSE IF ( ISFROPT.EQ.2 ) THEN
          IF ( icalc.LE.0 ) THEN
            READ (In, *, IOSTAT=I) SEG(14, nseg), SEG(15, nseg)
...
C       ADDED CONDITIONAL IF WHEN UNSATURATED FLOW INACTIVE DEP
          IF ( Kkper.EQ.1 .OR. ISFROPT.LE.1 ) THEN
            READ (In, *, IOSTAT=I) (XSEC(jj, nseg), jj=1, 8)
            !
            IF(I.NE.0) CALL SFR_STOP(IN, IOUT)
            !
            READ (In, *, IOSTAT=I) (XSEC(jj, nseg), jj=9, 16)
            !
            IF(I.NE.0) CALL SFR_STOP(IN, IOUT)
          END IF
        END IF
...
C9------READ DATA SET 4F FOR SEGMENT WHEN ICALC IS 4.
     ...
     +              'SEGMENT '//NUM2STR(N)//' IS LESS THAN TWO')
!              WRITE (IOUT, 9004) n
...
!            CALL USTOP(' ')
          ELSE IF ( nstrpts.GT.MAXPTS/3 ) THEN
            CALL SFR_STOP_MSG(IN, IOUT,
     +           'FOR SEGMENT '//NUM2STR(N)//' THE NUMBER OF '//  
     +           'POINTS USED TO RELATE STREAMFLOW WITH DEPTH AND '//  
     +           'WIDTH IS '//NUM2STR(nstrpts)//', BUT THE '// 
     +           'MAX ALLOWED SIZE IS 50 POINTS')
...
!            CALL USTOP(' ')
          ELSE
            READ (In, *, IOSTAT=I) (QSTAGE(jj, nseg), jj=1, nstrpts)
            !
            IF(I.NE.0) CALL SFR_STOP(IN, IOUT)
            !
       READ (In, *,IOSTAT=I) (QSTAGE(jj, nseg), jj=nstrpts+1, 2*nstrpts)
            !
            IF(I.NE.0) CALL SFR_STOP(IN, IOUT)
            !
       READ(In,*,IOSTAT=I) (QSTAGE(jj, nseg), jj=2*nstrpts+1, 3*nstrpts)
            !
            IF(I.NE.0) CALL SFR_STOP(IN, IOUT)
          END IF
        END IF
        ...
        IF ( Iunitgwt.NE.0 ) THEN
          DO isol = 1, Nsol
            IF ( IDIVAR(1, nseg).EQ.0 ) THEN
             READ(In,*,IOSTAT=I) CONCQ(nseg, isol), CONCRUN(nseg, isol),
     +                     CONCPPT(nseg, isol)
            ELSE
            READ(In,*,IOSTAT=I) CONCRUN(nseg, isol), CONCPPT(nseg, isol)
            END IF
            !
            IF(I.NE.0) CALL SFR_STOP(IN, IOUT)
            !
...
C     ******************************************************************
     ...
     +                        CONCQ, CONCRUN, CONCPPT, NSEGCK, IOUT,LOUT
      USE PARAMMODULE,  ONLY: IACTIVE, IPLOC, PARNAM, INAME, B, MXNAMLEN
      IMPLICIT NONE
C     ------------------------------------------------------------------
C     SPECIFICATIONS:
C     ------------------------------------------------------------------
...
C     ------------------------------------------------------------------
     ...
     +        numinst, lstbeg
      CHARACTER*4 package
      CHARACTER*700 line
      CHARACTER(MXNAMLEN):: pname, ctmp3, ctmp4                         !seb CHANGED FROM CHAR LEN OF 10
C     ------------------------------------------------------------------
C
      package = 'SFR '
C
C1------READ PARAMETER NAME AND FIND IT IN THE PARAMETER LIST.
      READ (In, '(A)') line
      lloc = 1
      CALL URWORD(line, lloc, istart, istop, 0, idum, rdum, LOUT, In)
      pname = line(istart:istop)
        WRITE (IOUT, 9001) pname
 9001 FORMAT (/, ' Parameter:  ', A)
      CALL UPARFIND(pname, 'SFR', 'SFR', ip, LOUT)
C
C2------DESIGNATE CELLS CORRESPONDING TO CORRECT PARAMETER INSTANCE.
      ...
      IF ( numinst.GT.0 ) THEN
        ...
        IF ( ctmp3.EQ.' ' ) THEN
            WRITE (LOUT, 9002) package, PARNAM(ip)
 ...
 9003   FORMAT (3X, 'Instance:  ', A)
        ...
        DO ki = 1, numinst
          ...
          END IF
        END DO
          WRITE (LOUT, 9004) package, ctmp3, PARNAM(ip)
...
C     ******************************************************************
     ...
     +                        QSTAGE, CONCQ, CONCRUN, CONCPPT, IOUT
      IMPLICIT NONE
C     ------------------------------------------------------------------
C     SPECIFICATIONS:
C     ------------------------------------------------------------------
...
C         FROM STREAMFLOW FOR SEGMENTS WITH ICALC=4.
 ...
 9032       FORMAT (5X, I6, 2X, 3(3X, ES13.4))
            ipt = ipt + 1
          END DO
        END IF
      END DO
...
C         FOR PRINTING WATER CONTENT PROFILES.
      IF ( Ibd.NE.0 .AND. Iunitgage.NE.0 ) THEN
        IF ( H.LT.Sbot ) THEN
...
C         STREAM.
      ...
      ! seb do not calcualte unsaturated flow if underlying cells are all dry
      !
      IF(ABS(H - HDRY) < CLOSEZERO .OR. ABS(Hld - HDRY) < CLOSEZERO)THEN
          htest2 = 1D0
      ELSE
      htest2 = Hld - Sbot
      END IF
      ...
      ELSE IF ( (Dlength-Zoldist).GT.feps ) THEN
        ...
        IF ( thetadif.GT.1.0E-6 ) THEN
          ...
          IF ( Numwaves.GT.NSFRSETS*NSTRAIL ) THEN
               CALL STOP_ERROR(OUTPUT=LOUT,
     +         MSG='SFR SEGMENT '//NUM2STR(ISTRM(4, I))//
     +         ' AND REACH '//NUM2STR(ISTRM(5, I))//BLN//
     +         'HAS TOO MANY UZFLOW-1 WAVES'//NL//
     +         'THE NUMBER OF WAVES ARE '//NUM2STR(Numwaves)//BLN//
     +         'PLEASE INCREASE "NSFRSETS" AND RESTART SIMULATION')
            !
!            WRITE (*, *) 'TOO MANY WAVES IN STREAM CELL', I, Numwaves, 
!     +                   '   PROGRAM TERMINATED IN UZFLOW-1'
!            WRITE (LOUT, *)'TOO MANY WAVES IN STREAM CELL', I, Numwaves,
...
C         CALL LEADWAVE IF SURFACE FLUX INCREASES.
     ...
     +         MSG='SFR SEGMENT '//NUM2STR(ISTRM(4, I))//
     +         ' AND REACH '//NUM2STR(ISTRM(5, I))//BLN//
     +         'HAS TOO MANY UZFLOW-2 WAVES'//NL//
     +         'THE NUMBER OF WAVES ARE '//NUM2STR(Numwaves)//BLN//
     +         'PLEASE INCREASE "NSFRSETS" AND RESTART SIMULATION')
!          WRITE (*, *) 'TOO MANY WAVES IN STREAM CELL', I, Numwaves, 
...
C4------LEAD WAVE INTERSECTING TRAIL WAVE.
     ...
     +                           Thetar)/(Theta(Jpnt+jj-1)-Thetar)
                  ...
                  IF((Speed(Jpnt+jj-1)
     +               *(fhold**eps_m1)-Speed(Jpnt+j-1)).NE.0.0) THEN
...
C     ******************************************************************
     ...
     +                        FLUXHLD2, FLUXB, THETAB, LOUT, ISTRM
      USE UTIL_INTERFACE,    ONLY: STOP_ERROR
      USE NUM2STR_INTERFACE, ONLY: NUM2STR
      USE CONSTANTS,         ONLY: BLN, NL
      IMPLICIT NONE
      INTRINSIC FLOAT
C     ------------------------------------------------------------------
C     SPECIFICATIONS:
...
C1------INITIALIZE TRAIL WAVES WHEN SURFACE FLUX DECREASES.
     ...
     +         MSG='SFR SEGMENT '//NUM2STR(ISTRM(4, I))//
     +         ' AND REACH '//NUM2STR(ISTRM(5, I))//BLN//
     +         'HAS TOO MANY UNSATURATED FLOW TRAILWAVES.'//NL//
     +         'THE NUMBER OF WAVES ARE '//NUM2STR(Numwaves)//BLN//
     +         'PLEASE INCREASE "NSFRSETS" AND RESTART SIMULATION')
!            WRITE (*, *) 'TOO MANY WAVES IN UNSAT CELL', I, Numwaves, 
!     +                   '   PROGRAM TERMINATED IN TRAILWAVE SFR2'
!            WRITE (LOUT, *) 'TOO MANY WAVES IN UNSAT CELL', I, Numwaves,
...
!            ERROR STOP
     ...
     +         MSG='SFR SEGMENT '//NUM2STR(ISTRM(4, I))//
     +         ' AND REACH '//NUM2STR(ISTRM(5, I))//BLN//
     +         'HAS TOO MANY UZFLOW-4 WAVES'//NL//
     +         'THE NUMBER OF WAVES ARE '//NUM2STR(Numwaves)//BLN//
     +         'PLEASE INCREASE "NSFRSETS" AND RESTART SIMULATION')
!            WRITE (*, *) 'TOO MANY WAVES IN STREAM CELL', I, Numwaves, 
...
!--------REVISED FOR MODFLOW-2005 RELEASE 1.9, FEBRUARY 6, 2012
C***********************************************************************
      USE GWFSFRMODULE, ONLY: ISEG, WEIGHT, SEG, FLWTOL, NEARZERO, IOUT
      IMPLICIT NONE
C     ------------------------------------------------------------------
C     SPECIFICATIONS:
C     ------------------------------------------------------------------
...
C     ------------------------------------------------------------------
     ...
     +                        IPTFLG, SFRTABFILE, SFRFEED, LOUT,        !seb  ADDED IPTFLG, SFRTABFILE, SFRFEED
     +                        TIME_SERIES
      IMPLICIT NONE
      EXTERNAL FLOWTERP
      REAL FLOWTERP
...
C     ------------------------------------------------------------------
      ...
      DEALLOCATE (GWFSFRDAT(IGRID)%SEG_NSTRM)
      DEALLOCATE (GWFSFRDAT(IGRID)%SWO_ENABLED)
      ...
      !
      IF(GWFSFRDAT(IGRID)%SFR_FRES%IS_OPEN) THEN
         DEALLOCATE(GWFSFRDAT(IGRID)%SFR_FRES_LRC,STAT=I)
         DEALLOCATE(GWFSFRDAT(IGRID)%SFR_FRES_DIF,STAT=I)
      END IF
      DEALLOCATE(GWFSFRDAT(IGRID)%SFR_FRES_OUTER)
      DEALLOCATE(GWFSFRDAT(IGRID)%SFR_FRES_NTERM)
      !
      DEALLOCATE(GWFSFRDAT(IGRID)%HD_RELAX)
      ...
      DBFILE =>NULL()
      !
      SFR_FRES=>GWFSFRDAT(IGRID)%SFR_FRES
      GWFSFRDAT(IGRID)%SFR_FRES=>NULL()
      DEALLOCATE(SFR_FRES)
      SFR_FRES=>NULL()
      !
      TIME_SERIES=>GWFSFRDAT(IGRID)%TIME_SERIES
      GWFSFRDAT(IGRID)%TIME_SERIES=>NULL()
      DEALLOCATE(TIME_SERIES)
      TIME_SERIES=>NULL()
      !
      CNVG_WRN=>GWFSFRDAT(IGRID)%CNVG_WRN
      GWFSFRDAT(IGRID)%CNVG_WRN=>NULL()
      DEALLOCATE(CNVG_WRN)
      CNVG_WRN=>NULL()
      !
...
C     ------------------------------------------------------------------
      ...
      LOUT      =>GWFSFRDAT(IGRID)%LOUT
      !
      THETAB    => GWFSFRDAT(IGRID)%THETAB 
      FLUXB     => GWFSFRDAT(IGRID)%FLUXB
      FLUXHLD2  => GWFSFRDAT(IGRID)%FLUXHLD2
      Nfoldflbt => GWFSFRDAT(IGRID)%Nfoldflbt
      NUMTAB    => GWFSFRDAT(IGRID)%NUMTAB
      ROWTAB    => GWFSFRDAT(IGRID)%ROWTAB
      TIME_SERIES       => GWFSFRDAT(IGRID)%TIME_SERIES      
      SEG_NSTRM         =>GWFSFRDAT(IGRID)%SEG_NSTRM
      SWO_ENABLED       =>GWFSFRDAT(IGRID)%SWO_ENABLED
      ...
      CNVG_WRN          =>GWFSFRDAT(IGRID)%CNVG_WRN
      !
      SFR_FRES_OUTER => GWFSFRDAT(IGRID)%SFR_FRES_OUTER
      SFR_FRES_NTERM => GWFSFRDAT(IGRID)%SFR_FRES_NTERM
      SFR_FRES       => GWFSFRDAT(IGRID)%SFR_FRES
      SFR_FRES_LRC   => GWFSFRDAT(IGRID)%SFR_FRES_LRC
      SFR_FRES_DIF   => GWFSFRDAT(IGRID)%SFR_FRES_DIF
      !
...
C     ------------------------------------------------------------------
      ...
      GWFSFRDAT(IGRID)%LOUT   =>LOUT
      !
      GWFSFRDAT(IGRID)%THETAB    => THETAB 
      GWFSFRDAT(IGRID)%FLUXB     => FLUXB
      GWFSFRDAT(IGRID)%FLUXHLD2  => FLUXHLD2
      GWFSFRDAT(IGRID)%Nfoldflbt => Nfoldflbt
      GWFSFRDAT(IGRID)%NUMTAB    => NUMTAB
      GWFSFRDAT(IGRID)%ROWTAB    => ROWTAB
      !
      GWFSFRDAT(IGRID)%TIME_SERIES       => TIME_SERIES      
      GWFSFRDAT(IGRID)%SEG_NSTRM         => SEG_NSTRM
      GWFSFRDAT(IGRID)%SWO_ENABLED       => SWO_ENABLED
      ...
      !
      GWFSFRDAT(IGRID)%SFR_FRES_OUTER => SFR_FRES_OUTER
      GWFSFRDAT(IGRID)%SFR_FRES_NTERM => SFR_FRES_NTERM
      GWFSFRDAT(IGRID)%SFR_FRES       => SFR_FRES
      GWFSFRDAT(IGRID)%SFR_FRES_LRC   => SFR_FRES_LRC
      GWFSFRDAT(IGRID)%SFR_FRES_DIF   => SFR_FRES_DIF
      !
