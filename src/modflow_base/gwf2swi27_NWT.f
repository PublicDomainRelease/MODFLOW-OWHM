      MODULE GWFSWIMODULE
        ...
        TYPE TSWIPCG
          ...
          REAL    :: DAMPPCGT
          INTEGER :: IHCOFADD = 1
          REAL(KND), ALLOCATABLE, DIMENSION(:,:,:) :: VPCG
          REAL(KND), ALLOCATABLE, DIMENSION(:,:,:) :: SS
          REAL(KND), ALLOCATABLE, DIMENSION(:,:,:) :: P
          REAL(KND), ALLOCATABLE, DIMENSION(:,:,:) :: HPCG
...
C         SWI PARAMETERS
        ...
        REAL(KND), SAVE, DIMENSION(:,:), POINTER :: SWIHCOF
        REAL(KND), SAVE, DIMENSION(:,:), POINTER :: SWISOLCR
        REAL(KND), SAVE, DIMENSION(:,:), POINTER :: SWISOLCC
        REAL(KND), SAVE, DIMENSION(:,:), POINTER :: SWISOLCV
        ...
        REAL(KND), SAVE, DIMENSION(:,:,:), POINTER :: BRHS
        REAL(KND),       SAVE, DIMENSION(:,:), POINTER :: DUM
...
C           SWI PARAMETERS
          ...
          REAL(KND), DIMENSION(:,:), POINTER :: SWIHCOF
          REAL(KND), DIMENSION(:,:), POINTER :: SWISOLCR
          REAL(KND), DIMENSION(:,:), POINTER :: SWISOLCC
          REAL(KND), DIMENSION(:,:), POINTER :: SWISOLCV
          ...
          REAL(KND), DIMENSION(:,:,:), POINTER :: BRHS
          REAL(KND),       DIMENSION(:,:), POINTER :: DUM
...
C---------READ DATASET 1
        ...
        lloc = 1
        CALL URWORD(line, lloc, istart, istop, 2,   NSRF, r, LOUT, In)
        CALL URWORD(line, lloc, istart, istop, 2, ISTRAT, r, LOUT, In)
        CALL URWORD(line, lloc, istart, istop, 2,   NOBS, r, LOUT, In)
        CALL URWORD(line, lloc, istart, istop, 2, ISWIZT, r, LOUT, In)
        CALL URWORD(line, lloc, istart, istop, 2, ISWICB, r, LOUT, In)
        CALL URWORD(line, lloc, istart, istop, 2,ISWIOBS, r, LOUT, In)
        !
...
C---------CHECK TO SEE THAT NUMBER OF SURFACES IS AT LEAST 1
     ...
     +                 MSG='SWI2 ERROR: NUMBER OF SURFACES LESS THAN 1')
        ENDIF
C
...
C---------READ DATASET 2A - SOLVER DATA FOR BOTH SOLVERS
        ...
        lloc = 1
        CALL URWORD(line, lloc, istart, istop, 2,NSOLVER, r, LOUT, In)
        CALL URWORD(line, lloc, istart, istop, 2, IPRSOL, r, LOUT, In)
        CALL URWORD(line, lloc, istart, istop, 2, MUTSOL, r, LOUT, In)
        IF ( NSOLVER.LT.1 .OR. NSOLVER.GT.2 ) THEN
          CALL STOP_ERROR(LINE,IN,LOUT, 
     +    MSG='SWI2 ERROR: ERROR SPECIFYING NSOLVER.'//NEW_LINE(' ')//
     +        'IT ME BE SET TO 1 OR 2')
        END IF
...
C---------READ DATASET 2B IF NECESSARY
        SELECT CASE (NSOLVER)
          CASE (2)
            CALL READ_TO_DATA(LINE,IN,LOUT)
            lloc = 1
            CALL URWORD(line, lloc, istart, istop, 2, i, r, LOUT, In)
            SWIPCG%MXITER = i
            CALL URWORD(line, lloc, istart, istop, 2, i, r, LOUT, In)
            SWIPCG%ITER1  = i
            CALL URWORD(line, lloc, istart, istop, 2, i, r, LOUT, In)
            SWIPCG%NPCOND = i
            CALL URWORD(line, lloc, istart, istop, 3, i, r, LOUT, In)
            SWIPCG%ZCLOSEPCG = r
            CALL URWORD(line, lloc, istart, istop, 3, i, r, LOUT, In)
            SWIPCG%RCLOSEPCG = r
            CALL URWORD(line, lloc, istart, istop, 3, i, r, LOUT, In)
...
C-------READ DATASET 3A PARAMETERS
        CALL READ_TO_DATA(LINE,IN,LOUT)
        lloc = 1
        CALL URWORD(line, lloc, istart, istop, 3, i, TOESLOPE, LOUT, In)
        CALL URWORD(line, lloc, istart, istop, 3, i, TIPSLOPE, LOUT, In)
        CALL URWORD(line, lloc, istart, istop, 3, i,    ALPHA,-LOUT, In)
        IF ( ALPHA.NE.0.0 ) THEN
          CALL URWORD(line, lloc, istart, istop, 3, i,  BETA, LOUT, In)
        ELSE
          ALPHA = 0.1
          BETA  = 0.1
        END IF
...
C-------READ DATASET 3B PARAMETERS
        ...
        IF ( iadptflg.NE.0 ) THEN   
          CALL READ_TO_DATA(LINE,IN,LOUT)
          lloc = 1
          CALL URWORD(line,lloc,istart,istop,2,  NADPTMX, r,LOUT, In)
          CALL URWORD(line,lloc,istart,istop,2,NADPTMN, r, LOUT, In)
          CALL URWORD(line,lloc,istart,istop,3,i,ADPTFCT,LOUT,In)
          IF ( NADPTMN.GT.NADPTMX ) THEN
            WRITE (LOUT,2245) NADPTMN, NADPTMX
            CALL USTOP('SWI2 ERROR: NADPTMN EXCEEDS NADPTMX')
          END IF
          IF ( ADPTFCT.LE.0.0 ) THEN
            WRITE (LOUT,2250) ADPTFCT
...
C-----------READ NU FOR EACH ZONE (NZONES)
          CALL U1DREL(NUS,ANAME(1),NZONES,In,LOUT)
...
C-----------READ NUSRF FOR EACH SURFACE (NZONES+1)
          CALL U1DREL(NUSRF,ANAME(2),NZONES+1,In,LOUT)
C
          ...
          DO i = 1, NROW
            DO j = 1, NCOL
              ...
              IF ( iusezone.GT.NZONES ) THEN
                ierr = ierr + 1
                IF ( ierr.EQ.1 ) WRITE(LOUT,'(//)')
                WRITE(LOUT,2270) IZONENR(j,i,k),k,i,j 
              END IF
            END DO
          END DO
      END DO K_IZONENR
      IF ( ierr.GT.0 ) THEN
        WRITE (LOUT,'(A)') 'INVALID ISOURCE VALUES SPECIFIED'
        WRITE (LOUT,'(A,1X,I4)') 
...
C-------------READ OBSERVATION DATA
            CALL READ_TO_DATA(LINE,IN,LOUT)
            lloc = 1
            CALL URWORD(line, lloc, istart, istop, 0, i, r, LOUT, In)
            SWIOBS(n)%OBSNAM = line(istart:istop)
            CALL URWORD(line, lloc, istart, istop, 2, k, r, LOUT, In)
            SWIOBS(n)%KLAY = k
            CALL URWORD(line, lloc, istart, istop, 2, i, r, LOUT, In)
            SWIOBS(n)%IROW = i
            CALL URWORD(line, lloc, istart, istop, 2, j, r, LOUT, In)
            SWIOBS(n)%JCOL = j
          END DO
...
C-------SET ICONV FOR EACH LAYER
      DO k = 1, NLAY
        ...
        ELSE IF ( Ilpf.NE.0 ) THEN
          IF ( LCL(k).EQ.0 ) ic = 0
        ELSE IF ( Ihuf.NE.0 ) THEN
          IF ( LCH(k).EQ.0 ) THEN
            ic = 0
          ELSE
            WRITE (LOUT,'(A)') 
...
C     SPECIFICATIONS:
C     ------------------------------------------------------------------
        USE GLOBAL,      ONLY:NCOL,NROW,NLAY,BOTM,LBOTM,IBOUND,HNEW
        USE GWFSWIMODULE
        IMPLICIT NONE
C       + + + DUMMY ARGUMENTS + + +
...
C                 SET ZETA SURFACE FOR FIRST AND LAST SURFACE
                ...
                h = HNEW(j,i,k)
                IF ( bbot.GT.ttop .AND. IBOUND(J,I,K).NE.0 ) THEN    !seb added IBOUND.NE.0
                  WRITE(IOUT,35) k,i,j
...
C     SPECIFICATIONS:
C     ------------------------------------------------------------------
        USE GLOBAL,      ONLY:BOTM,LBOTM, KND
        IMPLICIT NONE
C       + + + DUMMY ARGUMENTS + + +
        INTEGER, INTENT(IN) :: J,I,K,IZ
        INTEGER, INTENT(IN) :: NCOL,NROW,NLAY,NZONES
        REAL, INTENT(INOUT) :: Q
        REAL(KND), DIMENSION(NCOL,NROW,NLAY), INTENT(IN) :: HNEW
...
C       + + + DUMMY ARGUMENTS + + +
        ...
        REAL, INTENT(IN) :: FAC
        REAL(KND), DIMENSION(NCOL,NROW), INTENT(INOUT) :: B
        REAL, DIMENSION(NCOL,NROW), INTENT(IN)    :: CRLAY
        REAL, DIMENSION(NCOL,NROW), INTENT(IN)    :: CCLAY
        REAL(KND), DIMENSION(NCOL,NROW), INTENT(IN) :: VAR
C       + + + LOCAL DEFINITIONS + + +
C       + + + CODE + + +
C
...
C         SWI PARAMETERS
        ...
        DEALLOCATE(GWFSWIDAT(Igrid)%IOUT   )
        DEALLOCATE(GWFSWIDAT(Igrid)%LOUT   )
C
...
C         SWI PARAMETERS
        ...
        IOUT    => GWFSWIDAT(Igrid)%IOUT
        LOUT    => GWFSWIDAT(Igrid)%LOUT
C
C---------RETURN
        RETURN
...
C         SWI PARAMETERS
        ...
        GWFSWIDAT(Igrid)%IOUT    => IOUT
        GWFSWIDAT(Igrid)%LOUT    => LOUT
C
C---------RETURN
        RETURN
...
C---------------CALL DE45AP TO SOLVE FOR ZETA
     ...
     2            SWIDE4%NUP,SWIDE4%NBW,ierr)
                ...
                IF (ierr.EQ.0) THEN
                  WRITE(LOUT,'(A,I4,A,I5,A,I4,A,I4,A,I5,A,I8)')
     &             ' STRESS PERIOD=',Kkper,
     &             ' TIMESTEP=',kkstp,
     &             ' LAYER=',k,
     &             ' ZONE=',iz,
...
C-------------------SET TEMPORARY AQUIFER THICKNESS VARIABLES
                  ...
                  IF ( h.LT.top ) top = h
                  aqb   = ( top - bot )
                  IF(ABS(aqb) < 1D-30) aqb = 1D-30   !fix div zero error
C
C-------------------SET TEMPORARY ZETA VARIABLES
                  zt    = ZETA(j,i,k,1)
                  zb    = ZETA(j,i,k,NZONES+1)
