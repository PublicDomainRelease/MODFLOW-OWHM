...
Cx------FOR THE CURRENT TIME STEP.
  ...
  70          FORMAT (/,' DRN  OBS#',I5,', ID ',A,' IS BEING SKIPPED',
     &          ' DUE TO NOT BEING WITHIN SIMULATED TIME (OBS2DRN7SE)')
              CYCLE
          END IF
...
C
      ...
      DEALLOCATE(OBSDRNDAT(IGRID)%SKIP_OBS)
      ! GFORTRAN compiler error work-around for pointer data type FINAL statement
      FN_PRN_ALL=>OBSDRNDAT(IGRID)%FN_PRN_ALL
      OBSDRNDAT(IGRID)%FN_PRN_ALL=>NULL()
      DEALLOCATE(FN_PRN_ALL)
      FN_PRN_ALL=>NULL()
      !
      FN_PRN=>OBSDRNDAT(IGRID)%FN_PRN
      OBSDRNDAT(IGRID)%FN_PRN=>NULL()
      DEALLOCATE(FN_PRN)
      FN_PRN=>NULL()
      !DEALLOCATE(OBSDRNDAT(IGRID)%FN_PRN_ALL)
      !DEALLOCATE(OBSDRNDAT(IGRID)%FN_PRN)
C
...
C
      ...
      OBSDRNDAT(IGRID)%SKIP_OBS  =>SKIP_OBS
      OBSDRNDAT(IGRID)%FN_PRN_ALL=>FN_PRN_ALL
      OBSDRNDAT(IGRID)%FN_PRN    =>FN_PRN
C
      RETURN
      END
