...
C     ------------------------------------------------------------------
     ...
     +        NRL, NRN, NROW, NSTP,IHCOFADD
      ...
      REAL(KND):: HNEW, HHCOF, RRHS, DEL, HPCG
      REAL(KND):: Z, B, D, E, F, H, S, ALPHA, V, SS, P
      REAL(KND):: DIA
      REAL(KND):: ZHNEW, BHNEW, DHNEW, FHNEW, HHNEW, SHNEW
      REAL(KND):: SRNEW, SROLD, SSCR, SSCC, SSCV, VCC, VCR, VCV, FRAC
      REAL(KND):: CDCC, CDCR, CDCV
      REAL(KND):: PN, VN, HCHGN, RCHGN, PAP
      REAL(KND):: FCC, FCR, FCV, FV
      REAL(KND):: DDAMP, BIGGESTPOS, BIGGESTNEG
...
!        V(N) = 0.
!   10 CONTINUE
      DO CONCURRENT (N=1:NODES); SS(N) = DZ
      END DO
      DO CONCURRENT (N=1:NODES); P(N) = DZ
      END DO
      DO CONCURRENT (N=1:NODES); V(N) = DZ
      END DO
...
!         HPCG(N)=HNEW(N)
!   15    CONTINUE
         DO CONCURRENT (N=1:NODES); HPCG(N)=HNEW(N)
         END DO
      END IF
...
!          CD(N) = 0.
!   20   CONTINUE
         DO CONCURRENT (N=1:NODES); CD(N) = DZ
         END DO
      ENDIF
C
...
!            IF (B+H+D+F+Z+S.EQ.0.) THEN
            IF (B+H+D+F+Z+S+DIA.EQ.0.) THEN
              ...
              HCOF(N) = 0.
              RHS(N) = 0.
              CALL WRN%ADD(BLNK//NUM2STR(K,6)//BLNK//NUM2STR(I,6)//
     +                     BLNK//NUM2STR(J,6)//NLc)
!                WRITE(IOUT,27) K,I,J
!   27           FORMAT (/,
...
C-------CALCULATE THE DIAGONAL OF THE A MATRIX, AND STORE IT IN HCOF.
   ...
   50 CONTINUE
     ...
     +  'PCG7 FOUND ISOLATED MODEL CELLS THAT HAD TO BE REMOVED'//NLc//
     +  'THE FOLLOWING ISOLATED MODEL CELLS HAVE THEIR '//
     +  'IBOUND CHANGED TO ZERO'//NLc//'[IBOUND(COL,ROW,LAY)=0]:'//BLN//
     +      '    LAY    ROW    COL',
     +      OUTPUT=IOUT)
C
...
C-------NOTE : ABS. VAL. OF THE DIAG. OF THE SCALED A MATRIX IS 1.
              HHCOF = HCOF(N)
              IF (NORM.EQ.1) HHCOF = UNO
              T = ABS(Z) + ABS(B) + ABS(D) + ABS(HHCOF) + ABS(F)
     +            + ABS(H) + ABS(S)
...
C                 MODIFIED FROM HILL(1990) 9/27/90: 2 REPLACES 1
                ...
                ENDIF
                IF (NORM.EQ.1) HHCOF = -UNO
                CD(N) = (UNO+DEL)*HHCOF - CDCR - CDCC - CDCV - 
     +                  RELAX*(FCR+FCC+FCV)
...
C
      END
C
      SUBROUTINE PCG7OT(HCHG,LHCH,RCHG,LRCH,KITER,NITER,
     +                 HCLOSE,RCLOSE,ICNVG,KSTP,KPER,IPRPCG,
...
C     ------------------------------------------------------------------
      USE GLOBAL, ONLY: KND
      REAL C, RES
      REAL(KND):: CC, CR, CV                                   !seb ORIGINALLY DEFINED AS REAL
      REAL(KND):: HCOF                                             !seb ORIGINALLY DEFINED AS REAL
      INTEGER I, IBOUND, J, K, N, NCD, NCF, NCL, NCN, NCOL, NLAY, NLL, 
     +        NLN, NLS, NLZ, NODES, NORM, NRB, NRC, NRH, NRL, NRN, NROW
      REAL(KND)::  VN, CRHS, Z, B, D, F, H, S, ZV, BV, DV, FV, HV, 
     +                 SV, DZERO, VIN, VOUT
