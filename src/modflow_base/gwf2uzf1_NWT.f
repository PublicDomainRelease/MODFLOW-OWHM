...
C     ******************************************************************
      ...
      USE GLOBAL,       ONLY: NCOL, NROW, NLAY, ITRSS, ISSFLG, 
     +                        DELR, DELC, IBOUND, LBOTM, BOTM, IUNIT
      USE GLOBAL,       ONLY: ITMUNI, LENUNI, LAYHDT, NOCBC
      USE GWFLPFMODULE, ONLY: SCLPF=>SC2,SC1LPF=>SC1, LAYTYP
      USE GWFBCFMODULE, ONLY: SC1, SC2, LAYCON
      USE GWFHUFMODULE, ONLY: SC2HUF
      USE GWFUPWMODULE, ONLY: SC1UPW=>SC1,SC2UPW
      USE UTIL_INTERFACE, ONLY:UPPER, STOP_ERROR,
     +                         WARNING_MESSAGE
...
C1------IDENTIFY PACKAGE AND INITIALIZE.
        WRITE (LIST_UNIT, 9001) In
 9001 FORMAT (1X, /' UZF1 -- UNSATURATED FLOW PACKAGE, VERSION 1.4', 
     +        ', 02/06/2012', /, 9X, 'INPUT READ FROM UNIT', I5)
!
      CALL URDCOM(In, LIST_UNIT, line)
      !
      LOUT = LIST_UNIT
      IOUT = LIST_UNIT
! Check for alternate input.
      ...
      IF(KEYVALUE == 'SPECIFYTHTR'  .OR.  
     +   KEYVALUE == 'SPECIFYTHTI'  .OR.  
     +   KEYVALUE == 'ETSQUARE'     .OR.  
     +   KEYVALUE == 'NOSURFLEAK'   .OR.  
     +   KEYVALUE == 'SPECIFYSURFK' .OR.  
     +   KEYVALUE == 'REJECTSURFK'  .OR.  
     +   KEYVALUE == 'SEEPSURFK'    .OR.  
     +   KEYVALUE == 'NETFLUX'            
     +  ) THEN
       CALL STOP_ERROR(TRIM(LINE),IN,IOUT,
     +  'UZF OPTION ERROR. FOUND OPTION "'//TRIM(KEYVALUE)//
     + '" BUT UZF REQUIRES THAT OPTIONS BE WITHIN AN OPTIONS BLOCK '//
     + 'PLEASE ADD "BEGIN OPTIONS" AND THEN HAVE ONE OPTION PER LINE '//
     + 'AND END OPTIONS BLOCK WITH THE WORD "END"')
      END IF
      !
      IF( KEYVALUE == 'BEGIN'     .OR.
     -    KEYVALUE == 'OPTION'    .OR.
     -    KEYVALUE == 'OPTIONS'      ) THEN
        ...
        CASE('SPECIFYTHTR')
            ITHTRFLG = 1
            WRITE(IOUT,'(/3A/)')'RESIDUAL WATER CONTENT (THTR) WILL ',
     +                'BE READ AND USED FOR THE FIRST TRANSIENT ',
     +                'STRESS PERIOD.'
        CASE('SPECIFYTHTI')
            ITHTIFLG = 1
            WRITE(IOUT,'(/2A/)')'INITIAL WATER CONTENT (THTI) WILL ',
     +                 'BE READ FOR THE FIRST SS OR TR STRESS PERIOD'
        CASE('ETSQUARE')
            I=1
            CALL URWORD(LINE,LLOC,ISTART,ISTOP,3,I,R,IOUT,IN)
            READ(LINE(ISTART:ISTOP),*) SMOOTHET
     ...
     +            'A SQUARE ET FUNCTION WILL BE USED TO SIMULATE GW ET'
        ...
        CASE('SPECIFYSURFK')
            IREADSURFK = 1
            WRITE(IOUT,'(/2A/)')'HYDRAULIC CONDUCTIVITY OF ',
     +                          'LAND SURFACE WILL BE READ'
        CASE('REJECTSURFK')
            ISURFKREJECT = 1
            WRITE(IOUT,'(/2A/)')'INFILTRATION WILL BE REJECTED ',
   ...
   31 FORMAT(1X,' OUTPUT WILL BE WRITTEN TO NETRECH AND NETDIS ',
     ...
     +   MSG='UZF: FAILED TO IDENTIFY OPTION: '//KEYVALUE//
     +             'THIS OPTION IS IGNORED.', INLINE=.TRUE.)
        END SELECT
      END DO
      END IF
      !
     ...
     +       MSG='UZF ERROR: FOUND KEYWORD OUTSIDE OF OPTIONS BLOCK.'//
     + 'KEYWORDS ARE NOW REQUIRED TO BE WITHIN THE FOLLOWING BLOCK: '//
     +  NEW_LINE('a')//'BEGIN OPTIONS'//NEW_LINE('a')//'END'//
     +  NEW_LINE('a'))
        END IF
 !
      if ( Ireadsurfk == 0 .and. Isurfkreject == 1) then
        CALL WARNING_MESSAGE(INFILE=IN,OUTPUT=IOUT,
     +   MSG='UZF: REJECTSURFK SPECIFIED BUT '//
     +   'NO SURFK SPECIFIED. OPTION IGNORED', INLINE=.TRUE.)
          Isurfkreject = 0
      end if
!            
      if ( Ireadsurfk == 0 .and. Iseepreject == 1 ) then
        CALL WARNING_MESSAGE(INFILE=IN,OUTPUT=IOUT,
     +   MSG='UZF: SEEPSURFK SPECIFIED BUT '//
     +   'NO SURFK SPECIFIED. OPTION IGNORED', INLINE=.TRUE.)
          Iseepreject = 0
      END IF
!
      if ( Iseepsupress > 0 .and. Iseepreject == 1 ) then
        CALL WARNING_MESSAGE(INFILE=IN,OUTPUT=IOUT,
     +   MSG='UZF: NOSURFLEAK SPECIFIED BUT '//
     +   'SEEPSURFK WAS SPECIFIED. SEEPSURFK '//
     +   'OPTION IGNORED', INLINE=.TRUE.)
          Iseepreject = 0
      END IF
 !
      ...
      IF( NOCBC>0 ) IUZFCB1 = 0
      IF( NOCBC>0 ) IUZFCB2 = 0
      !
      IUZFB22 = IUZFCB2
      IUZFB11 = IUZFCB1
      NTRAIL = 1
...
C3------CHECK FOR ERRORS.
 ...
 9003   FORMAT (//' UNSATURATED FLOW IN VADOSE ZONE IS IGNORED ', 
     ...
     +          'INFILTRATION RATE MINUS REJECTED RECHARGE'///)
        CALL WARNING_MESSAGE(INFILE=IN,OUTPUT=IOUT,
     +   MSG='UZF: IUZFOPT IS ZERO. UNSATURATED  '//NL// 
     +   'VARIABLES, EPS, THTS, and THTI ARE NOT READ. '//NL// 
     +   'IF ET IS ACTIVE EXTWC IS NOT READ.',INLINE=.TRUE.)
!        WRITE (IOUT, 9999)
! 9999   FORMAT (//'***WARNING*** IUZFOPT IS ZERO. UNSATURATED  ',/ 
!     +          'VARIABLES, EPS, THTS, and THTI ARE NOT READ. ',/ 
!     +          'IF ET IS ACTIVE EXTWC IS NOT READ.'///)
      END IF
      IF ( ABS(IUZFOPT).EQ.2 .AND. Iunitbcf.NE.0 ) THEN
 ...
 9007   FORMAT (//' VERTICAL FLOW THROUGH UNSATURATED ZONE IS ', 
     ...
     +          ' RESETTING THE NUMBER OF WAVE SETS TO BE 20'///)
        NSETS = 20
      END IF
!      IF ( ABS(IUZFOPT).EQ.2 .AND. Iunithuf.NE.0 ) THEN
...
!                     CALL USTOP(' ')
                    END IF
                  ELSE IF ( Iunitupw.NE.0 ) THEN
                    IF ( LAYHDT(nlth).EQ.0 ) THEN
                      FOUND = .TRUE.
                      THICK = BOTM(ncth, nrth, LBOTM(nlth)-1)-
     +                      BOTM(ncth, nrth, LBOTM(nlth))
                      sy = SC2UPW(ncth, nrth, nlth)/       !seb, note UPW dims SC2UPW by NLAY
     +                     (THICK*DELR(ncth)*DELC(nrth)) 
                    ELSE
...
C       UNSATURATED FLOW AT CELL INACTIVE.
 ...
 9018           FORMAT (1X/, 'SPECIFIC YIELD FOR UPPERMOST ACTIVE ', 
     ...
     +                  'FLOW IN CELL TO INACTIVE')
            ...
            END DO
          END DO
          IF(FOUND) CALL WARNING_MESSAGE(INFILE=IN,OUTPUT=IOUT,
     +   MSG='UZF HAS MODEL CELLS WITH IUZFBND>0 WITH AN UPPER MOST '//
     +  'ACTIVE MODEL CELL THAT IS IN A CONFINE LAYER,'//NL//
     +    'SO THERE IS '//
     +  'NO DEFINED SPECIFIC YIELD.'//NL//'SPECIFIC STORAGE IS USED '//
     +  'INSTEAD AS IF IT WAS SPECIFIED AS SPECIFIC YIELD.')
        END IF
      END IF
C
C21-----READ FILES FOR PRINTING TIME SERIES WATER CONTENT PROFILES.
 ...
 9019   FORMAT (1X/, 'UNSATURATED FLOW RESULTS WILL BE PRINTED TO ', 
     ...
     +          'OUTPUT OPTION'//)
        ...
        CALL WARNING_MESSAGE(INFILE=IN,OUTPUT=IOUT,
     +   MSG='UZF - PRINTING OF WATER CONTENT PROFILES IS NOT '//
     +  'POSSIBLE WHEN IUZFOPT <= 0.'//NL//
     +  'REMOVING GAUGE '//NUM2STR(igage),INLINE=.TRUE.)
...
C     ******************************************************************
      USE GWFUZFMODULE, ONLY: VKS, IUZFBND, CLOSEZERO, NUZTOP, IOUT,LOUT
...
CRGN made il! = 0 when all layers for column are inactive 2/21/08
            ...
            il = UPLAY(ncck, nrck)
            IF(IL > 0) THEN
                IF ( IBOUND(ncck, nrck, IL).LT.0 ) IL = 0
            END IF
          END IF
          krck = il
          IF ( krck.NE.0 ) THEN
...
C     ******************************************************************
     ...
     +                        WTABLE, UPLAY
      ...
      USE NUM2STR_INTERFACE,        ONLY: NUM2STR
      USE CONSTANTS,                ONLY: BLNK, NL, TRUE
      IMPLICIT NONE
C     -----------------------------------------------------------------
C     SPECIFICATIONS:
C     -----------------------------------------------------------------
...
!
     ...
     +                  BOTM(ncck, nrck,LBOTM(il))
                IF ( ROOTDPTH(ncck, nrck).GT.0.9*thick ) THEN
                  ...
                  CALL WRN%ADD(NUM2STR(nrck,5)//BLNK//NUM2STR(ncck))
                END IF
              END IF
            END DO
          END DO
          IF(WRN%RAISED) THEN
           CALL WRN%CHECK(HED='UZF WARNING: ET EXTINCTION DEPTH IS '//
     +     'BELOW LAYER BOTTOM'//NL//'RESETTING DEPTH TO 90% OF CELL '//
     +     'THICKNESS FOR THE FOLLOWING:'//NL//
     +     '  ROW   COL',INFILE=IN,OUTPUT=IOUT,TAIL=NL,INIT=TRUE)
          END IF
        END IF
...
!     +                      'CONTENT')
            ...
            END DO
            IF(WRN%RAISED) THEN
             CALL WRN%CHECK(HED='UZF WARNING: EXTINCTION WATER '//
     +       'CONTENT IS LESS THAN RESIDUAL WATER CONTENT'//NL//
     +       'SETTING EXTINCTION WATER CONTENT EQUAL TO RESIDUAL '//
     +       'WATER CONTENT FOR THE FOLLOWING:'//NL//
     +       '  ROW   COL',INFILE=IN,OUTPUT=IOUT,TAIL=NL,INIT=TRUE)
            END IF
          END IF
        END IF
...
C16B-----SEARCH FOR UPPER MOST ACTIVE CELL WITH A WATER LEVEL.
              ilay = il
              IF ( il.GT.0 ) THEN
!!!                IF ( IBOUND(ic, ir, il).GT.0 ) THEN  seb entire look does not make sense
...
C       ADD Iunitswr - SWR - JDH
      IF ( IRUNFLG.GT.0 .AND. (Iunitsfr.NE.0.OR.
     +     Iunitlak.NE.0.OR.Iunitcfp.NE.0.OR.
     +     Iunitswr.NE.0) )
...
C     ******************************************************************
     ...
     +                        VBNM, VBVL, HNOFLO, HDRY
      ...
      USE WARNING_TYPE_INSTRUCTION, ONLY: WARNING_TYPE
      USE CONSTANTS,                ONLY: BLNK, NL, BLN, TRUE
      USE NUM2STR_INTERFACE,        ONLY: NUM2STR
      IMPLICIT NONE
C     -----------------------------------------------------------------
C     SPECIFICATIONS:
...
C6------PRINT WARNING WHEN NUZTOP IS 1 OR 2 AND ALL LAYERS ARE INACTIVE.
        IF ( il.EQ.0 .AND. ibnd.NE.0 )
     +                    CALL WRN%ADD(NUM2STR(IR,5)//BLNK//NUM2STR(IC))
!        IF ( il.EQ.0 ) THEN
!          IF ( ibnd.NE.0 ) THEN
!            CALL WRN%ADD(NUM2STR(IR,5)//BLNK//NUM2STR(IC))
!            IF ( NUZTOP.LT.2 ) THEN
...
! Suppress seepout and ET beneath a lake
        ...
        lakflginf = 0
        lakid = 0
        IF ( Iunitlak.NE.0 .AND. il.GT.1 ) THEN
          lakid = LKARR1(ic, ir, il-1)
          IF ( lakid.GT.0 ) THEN
            lakflg = 1
            IF( STGNEW(lakid).GT.BOTM(ic, ir, il-1) )
...
!     +                                (ths-thr))**epsilon
                                    fhold = (UZTHST(k, l)-thr)/(ths-thr)
                                    IF ( fhold>0D0) fhold=fhold**epsilon
     ...
     +                                  *(fhold**eps_m1)
                                  ELSE
                                    ! seb
                                   IF(UZTHST(k-1,l)<thr.AND.ths>thr)THEN
                                        fhold = 0.0
                                    ELSEIF(ths<thr) THEN
                                        fhold = 0.0
                                    ELSE
     ...
     +                                   fhold = 0.0
                                    ftheta1 = fks*fhold
                                    ! seb
                                    IF(UZTHST(k,l)<thr.AND.ths>thr) THEN
                                        fhold = 0.0
                                    ELSEIF(ths<thr) THEN
                                        fhold = 0.0
                                    ELSE
...
C SET UZ INTERCELL FLUX TO ZERO WHEN BELOW WATER TABLE
      ...
      IF(WRN%RAISED) THEN
        IF      ( NUZTOP.EQ.1 ) THEN
           CALL WRN%CHECK(HED='UZF WARNING: NUZTOP IS 1 AND '//
     +     'HAS CELLS WITH UPPERMOST LAYER BEING INACTIVE '//
     +     '(IBOUND=0)'//NL//'UNSATURATED FLOW WILL NOT BE ADDED TO '//
     +     'AN ACTIVE LAYER'//NL//
     +     'IT IS SUGGESTED TO CHANGE NUZTOP TO 3'//NL//
     +     'THE FOLLOWING HAVE UZF DISABLED BECAUSE OF THIS:'//BLN//
     +     '  ROW   COL',OUTPUT=IOUT,TAIL=NL,INIT=TRUE)
        ELSE IF ( NUZTOP.EQ.2 ) THEN
           CALL WRN%CHECK(HED='UZF WARNING: NUZTOP IS 2 AND '//
     +     'SPECIFIED LAYER IS INACTIVE (IBOUND=0)'//NL//
     +     'UNSATURATED FLOW WILL NOT BE ADDED TO AN ACTIVE LAYER'//NL//
     +     'THE FOLLOWING HAVE UZF DISABLED BECAUSE OF THIS:'//BLN//
     +     '  ROW   COL',OUTPUT=IOUT,TAIL=NL,INIT=TRUE)
        ELSE IF ( NUZTOP.EQ.3 ) THEN
           CALL WRN%CHECK(HED='UZF WARNING: NUZTOP IS 3 AND '//
     +     'ALL LAYERS WERE INACTIVE (IBOUND=0) OR FLOW TO '//
     +     'FIXED HEAD  (IBOUND<0)'//NL//
     +     'UNSATURATED FLOW CANNOT BE ADDED SINCE THERE IS NO '//
     +     'ACTIVE LAY TO FLOW TOO.'//NL//
     +     'THE FOLLOWING HAVE UZF FLOW BEYOND :'//
     +     'THE BOTTOM OF THE MODEL:'//BLN// 
     +     '  ROW   COL',OUTPUT=IOUT,TAIL=NL,INIT=TRUE)
        END IF
      END IF
C
...
! seb commented out bad formats and replaced with NUM2STR
          val1=NUM2STR(CUMUZVOL(1)); val1=ADJUSTR(val1)
          val2=NUM2STR(UZTSRAT(1)) ; val2=ADJUSTR(val2)
!          IF ( ABS(CUMUZVOL(1)).LT.NEARZERO .AND. 
!     +         (ABS(CUMUZVOL(1)).GE.bigvl1 .OR. ABS(CUMUZVOL(1))
!     +         .LT.small) ) THEN
!            WRITE (val1, '(1PE20.4)') CUMUZVOL(1)
!          ELSE
!            WRITE (val1, '(F20.4)') CUMUZVOL(1)
!          END IF
!          IF ( ABS(UZTSRAT(1)).LT.NEARZERO .AND. 
!     +         (ABS(UZTSRAT(1)).GE.bigvl1 .OR. ABS(UZTSRAT(1)).LT.small)
!     +         ) THEN
!            WRITE (val2, '(1PE20.4)') UZTSRAT(1)
!          ELSE
!            WRITE (val2, '(F20.4)') UZTSRAT(1)
!          END IF
            WRITE (IOUT, 9004) uzinftxt, val1, uzinftxt, val2
            WRITE (IOUT, 9005)
! seb commented out bad formats and replaced with NUM2STR
          val1=NUM2STR(CUMUZVOL(2)); val1=ADJUSTR(val1)
          val2=NUM2STR( UZTSRAT(2)); val2=ADJUSTR(val2)
!          IF ( ABS(CUMUZVOL(2)).LT.NEARZERO .AND. 
!     +         (ABS(CUMUZVOL(2)).GE.bigvl1 .OR. ABS(CUMUZVOL(2))
!     +         .LT.small) ) THEN
!            WRITE (val1, '(1PE20.4)') CUMUZVOL(2)
!          ELSE
!            WRITE (val1, '(F20.4)') CUMUZVOL(2)
!          END IF
!          IF ( ABS(UZTSRAT(2)).LT.NEARZERO .AND. 
!     +         (ABS(UZTSRAT(2)).GE.bigvl1 .OR. ABS(UZTSRAT(2)).LT.small)
!     +         ) THEN
!            WRITE (val2, '(1PE20.4)') UZTSRAT(2)
!          ELSE
!            WRITE (val2, '(F20.4)') UZTSRAT(2)
!          END IF
            WRITE (IOUT, 9004) uzettext, val1, uzettext, val2
         !ENDIF  SCOTT strange unused endif, think it might be accidentally left in.
! seb commented out bad formats and replaced with NUM2STR
          val1=NUM2STR(CUMUZVOL(3)); val1=ADJUSTR(val1)
          val2=NUM2STR( UZTSRAT(3)); val2=ADJUSTR(val2)
!          IF ( ABS(CUMUZVOL(3)).LT.NEARZERO .AND. 
!     +         (ABS(CUMUZVOL(3)).GE.bigvl1 .OR. ABS(CUMUZVOL(3))
!     +         .LT.small) ) THEN
!            WRITE (val1, '(1PE20.4)') CUMUZVOL(3)
!          ELSE
!            WRITE (val1, '(F20.4)') CUMUZVOL(3)
!          END IF
!          IF ( ABS(UZTSRAT(3)).LT.NEARZERO .AND. 
!     +         (ABS(UZTSRAT(3)).GE.bigvl1 .OR. ABS(UZTSRAT(3)).LT.small)
!     +         ) THEN
!            WRITE (val2, '(1PE20.4)') UZTSRAT(3)
!          ELSE
!            WRITE (val2, '(F20.4)') UZTSRAT(3)
!          END IF
...
C58----CALCULATE DIFFERENCE AND ERROR BETWEEN INFLOW AND OUTFLOW.
          ...
          aratdif = ABS(ratedif)
! seb commented out bad formats and replaced with NUM2STR
          val1=NUM2STR(cumdiff); val1=ADJUSTR(val1)
          val2=NUM2STR(ratedif); val2=ADJUSTR(val2)
!          IF ( acumdif.LT.NEARZERO .AND. 
!     +         (acumdif.GE.bigvl2 .OR. acumdif.LT.small) ) THEN
!            WRITE (val1, '(1PE20.4)') cumdiff
!          ELSE
!            WRITE (val1, '(F20.4)') cumdiff
!          END IF
!          IF ( aratdif.LT.NEARZERO .AND. 
!     +         (aratdif.GE.bigvl2 .OR. aratdif.LT.small) ) THEN
!            WRITE (val2, '(1PE20.4)') ratedif
!          ELSE
!            WRITE (val2, '(F20.4)') ratedif
!          END IF
...
! seb commented out bad formats and replaced with NUM2STR
          val1=NUM2STR(CUMUZVOL(4)); val1=ADJUSTR(val1)
          val2=NUM2STR( UZTSRAT(4)); val2=ADJUSTR(val2)
!          IF ( unsatvol.LT.NEARZERO .AND. 
!     +         (unsatvol.GE.bigvl1 .OR. unsatvol.LT.small) ) THEN
!            WRITE (val1, '(1PE20.4)') CUMUZVOL(4)
!          ELSE
!            WRITE (val1, '(F20.4)') CUMUZVOL(4)
!          END IF
!          IF ( unsatrat.LT.NEARZERO .AND. 
!     +         (unsatrat.GE.bigvl1 .OR. unsatrat.LT.small) ) THEN
!            WRITE (val2, '(1PE20.4)') UZTSRAT(4)
!          ELSE
!            WRITE (val2, '(F20.4)') UZTSRAT(4)
!          END IF
...
C     ******************************************************************
     ...
     +                        ZEROD6, ZEROD7, ZEROD9, IOUT, LOUT
...
C         WHEN LEAD TRAIL WAVE INTERSECTS A LEAD WAVE.  Modified to prevent taking exponent of a negative number rth/seb              
     ...
     +                                                CLOSEZERO ) THEN
                      fhold = (Theta(jpntm1+j)-Thetar)*thsrinv
                      IF ( fhold.LT.NEARZERO ) THEN
                          fhold = 0.0D0
                          Speed(jpntm1+j) = Fhold
                      ELSE
                          Speed(jpntm1+j) = epsfksths * (fhold**eps_m1)
                      END IF
                      Ltrail(jpntm1+j) = 1
...
C1------INITIALIZE TRAILING WAVES.
     ...
     +              '   PROGRAM TERMINATED IN UZFLOW-4; INCREASE NSETS2'
      ...
      ELSE
        ...
        IF ( fhold.LT.NEARZERO ) THEN !seb
          fhold = 0.0D0
          Speed(jpnwavesm1) = 0.0D0
!          Flux(jpnwavesm1)  = 0.0D0  SCOTT
        ELSE
          Speed(jpnwavesm1) = epsfksths * (fhold**eps_m1)
!          Flux(jpnwavesm1) = Fksat*(((Theta(jpnwavesm1)-Thetar)
!     +                       *thsrinv)**Eps)
        END IF
        Flux(jpnwavesm1) = Fksat*(fhold**Eps)
...
C     ******************************************************************
     ...
     +                        ZEROD9, ZEROD7, IOUT, LOUT, ETOFH_FLAG
      USE GLOBAL,       ONLY: NLAY, LBOTM, BOTM
      IMPLICIT NONE
C     ------------------------------------------------------------------
C     SPECIFICATIONS:
C     ------------------------------------------------------------------
...
!  rgn added new calculation for speed 5/26/09.
     ...
     +                  '   PROGRAM TERMINATED IN TRANSPIRATION UZF - 3'
           WRITE (LOUT, *)'TOO MANY WAVES IN UNSAT CELL',IC,IR,Numwaves,
...
C     ------------------------------------------------------------------
      ...
      DEALLOCATE ( GWFUZFDAT(Igrid)%THETAB   )
      DEALLOCATE ( GWFUZFDAT(Igrid)%FLUXB    )
      DEALLOCATE ( GWFUZFDAT(Igrid)%FLUXHLD2 )
      DEALLOCATE ( GWFUZFDAT(Igrid)%LOUT     )
      DEALLOCATE ( GWFUZFDAT(Igrid)%IOUT     )
...
C     ------------------------------------------------------------------
      ...
      THETAB   => GWFUZFDAT(Igrid)%THETAB
      FLUXB    => GWFUZFDAT(Igrid)%FLUXB
      FLUXHLD2 => GWFUZFDAT(Igrid)%FLUXHLD2
      LOUT    => GWFUZFDAT(Igrid)%LOUT   
      IOUT    => GWFUZFDAT(Igrid)%IOUT   
...
C     ------------------------------------------------------------------
      ...
      GWFUZFDAT(Igrid)%THETAB   => THETAB
      GWFUZFDAT(Igrid)%FLUXB    => FLUXB
      GWFUZFDAT(Igrid)%FLUXHLD2 => FLUXHLD2
      GWFUZFDAT(Igrid)%LOUT    => LOUT   
      GWFUZFDAT(Igrid)%IOUT    => IOUT   
