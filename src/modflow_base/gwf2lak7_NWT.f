...
!         MODFIED AND IMPROVED FOR USE WITH MODFLOW-OWHM
C     ******************************************************************
...
C
      USE GWFLAKMODULE
      USE GLOBAL,       ONLY: NCOL, NROW, NLAY, IFREFM, ITRSS,
     +                        NODES, IUNIT, NOCBC, LIST_OUT=>IOUT
      USE GWFSFRMODULE, ONLY: NSS
      USE UTIL_INTERFACE, ONLY: STOP_ERROR, READ_TO_DATA, GET
C
C      ******************************************************************
...
C       argument lists are defined     
      ...
      !swm: added allocation below - copied from NWT
...
Cdep  transient simulations when THETA is negative.
        IF(IFREFM.EQ.0) THEN
           READ(LINE,'(2I10)', IOSTAT=I)NLAKES,ILKCB
           !
           IF(I.NE.0) CALL STOP_ERROR(LINE,IN,IOUT,MSG=
     + 'LAK PACKAGE FIXED FORMAT FAILED TO LOAD NLAKES, ILKCB WITH '//
     + 'FORTRAN FORMATTED READ "2I10"')
           CALL READ_TO_DATA(LINE,IN,IOUT,NOSHIFT=NOSHIFT)
           IF (ITRSS.LE.0) THEN
            READ(LINE,'(F10.2,I10,F10.2)', IOSTAT=I) THETA,NSSITR,SSCNCR
            !
           IF(I.NE.0) CALL STOP_ERROR(LINE,IN,IOUT,MSG=
     + 'LAK PACKAGE FIXED FORMAT FAILED TO LOAD THETA,NSSITR,SSCNCR '//
     + 'WITH FORTRAN FORMATTED READ "F10.2,I10,F10.2"')
             ! IF (THETA.LT.0.0) BACKSPACE IN
           ELSE
              READ(LINE,'(F10.2)', IOSTAT=I) THETA
           IF(I.NE.0) CALL STOP_ERROR(LINE,IN,IOUT,MSG=
     + 'LAK PACKAGE FIXED FORMAT FAILED TO LOAD THETA '//
     + 'WITH FORTRAN FORMATTED READ "F10.2"')
            !  IF (THETA.LT.0.0) BACKSPACE IN
           END IF
           ELSE
           LLOC = 1
           CALL GET(LINE,LLOC,ISTART,ISTOP,IOUT,IN,NLAKES,
     + MSG='LAK PACKAGE FAILED TO LOAD NLAKES')
           CALL GET(LINE,LLOC,ISTART,ISTOP,IOUT,IN,ILKCB,
     + MSG='LAK PACKAGE FAILED TO LOAD ILKCB')
           !
           ...
           LLOC = 1
           IF (ITRSS.LE.0) THEN
           CALL GET(LINE,LLOC,ISTART,ISTOP,IOUT,IN,THETA,
     + MSG='LAK PACKAGE FAILED TO LOAD THETA')
           CALL GET(LINE,LLOC,ISTART,ISTOP,IOUT,IN,NSSITR,
     + MSG='LAK PACKAGE FAILED TO LOAD NSSITR')
           CALL GET(LINE,LLOC,ISTART,ISTOP,IOUT,IN,SSCNCR,
     + MSG='LAK PACKAGE FAILED TO LOAD SSCNCR')
...
!rsr, old data sets may not have SURFDEPTH, may need to trap this for some compilers 
     ...
     +                                         SURFDEPTH 
            ...
            ELSE 
            LLOC=1
           CALL GET(LINE,LLOC,ISTART,ISTOP,IOUT,IN,DUM,
     + MSG='LAK PACKAGE FAILED TO LOAD FIRST VARIABLE')
           CALL GET(LINE,LLOC,ISTART,ISTOP,IOUT,IN,NSSITR,
     + MSG='LAK PACKAGE FAILED TO LOAD NSSITR')
           CALL GET(LINE,LLOC,ISTART,ISTOP,IOUT,IN,SSCNCR,
     + MSG='LAK PACKAGE FAILED TO LOAD SSCNCR')
           CALL GET(LINE,LLOC,ISTART,ISTOP,IOUT,IN,SURFDEPTH,
     + MSG='LAK PACKAGE FAILED TO LOAD SURFDEPTH')
            !READ(IN,*,IOSTAT=IOS) DUM,NSSITR,SSCNCR,SURFDEPTH 
            IF ( IOS.NE.0 ) SURFDEPTH = 0.0     !swm: This causes problems.  IOS is not defined.  Not sure what the correct fix is?
          END IF
        END IF
...
Cdep added format statement when starting with transient simulation
  10  FORMAT(//1X,'LAKE PACKAGE HAS BEEN MODIFIED TO ITERATIVELY ',
     ...
     3 'CLOSURE CRITERIA FOR LAKE STAGE (SSCNCR) = ',ES13.6,/1X, !gsf
...
Cdep   ALLOCATE SPACE FOR FLOB ARRAY WHEN TRANSPORT ACTIVE.   
      IF (IUNITGWT.NE.0 .OR. IUNIT(49) .NE. 0) THEN
        MXLKAR = MXLKND
      ELSE
        MXLKAR = 1
      END IF
...
7     FORMAT(/1X,'LAKE',7X,'PRECIP',5X,'EVAP',5X,'RUNOFF',
   ...
   77 FORMAT(/1X,'LAKE',7X,'PRECIP',5X,'EVAP',5X,'RUNOFF',
     2     3X,'WITHDRAW',3X,'BOTTOM',5X,'AREA',5X,/70('-'))
      IF (IUNITGWT.NE.0) WRITE (IOUTS,8)
...
C        6 is type 0
 ...
 900  IF (IUNITBCF.NE.0) THEN  ! rsr, moved if block from main
      ...
      ELSE
        ...
        CALL USTOP(' ')
      END IF
      IF (IUNITSFR.NE.0) CALL SGWF2LAK7SFR7RPS()     
C
C7------RETURN
      RETURN
      END
        ...
        if(LPFLNK)then
         DO IDV=1,NDV
          ...
          IF (INODE.GT.0) THEN
           ...
           IF (IUNITBCF.NE.0) THEN  ! rth, update vertical and horizontal conductances based on changes in BOTM with SUBLNK
            CALL SGWF2LAK7BCF7RPS(LWRT)
           ELSE IF (IUNITLPF.NE.0) THEN
            CALL SGWF2LAK7LPF7RPS(LWRT)
           ELSE IF (IUNITHUF.NE.0) THEN
...
C
      USE GWFLAKMODULE
      USE GLOBAL,       ONLY: NCOL, NROW, NLAY, IBOUND, ISSFLG, 
...
C15-----SUM UP OUTFLOWS FROM OUTFLOWING STREAM REACHES.
 ...
 ! 11/10 Outflow is zero when all is lost to ET. ******* NEW(.AND.FLWITER(LAKE).GT.CLOSEZERO)
            IF(IUNITSFR.NE.0.AND.FLWITER(LAKE).GT.CLOSEZERO) THEN
              DO IDV=1,NDV
                INODE=IDIV(LAKE,IDV)
                IF (INODE.GT.0) THEN   
...
Cdep  202  FORMAT(i5,8(1X,E20.10)) !format used for debugging
 ...
 1004  FORMAT(1X,'ITERATION ',I4,2X,'LAKE ',I4,2X,'NEW STAGE ',ES15.8, !gsf
...
C     ------------------------------------------------------------------
      USE GWFLAKMODULE
      USE GLOBAL,       ONLY: NCOL, NROW, NLAY, NODES, IBOUND,
...
! Save seepage to UZF for writing Lake-to-UZF in GAG Package.
                 ...
                 RATE=FLOBOT    
                 IF (IUNITGWT.NE.0.OR.IUNIT(49).NE.0) FLOB(L)=FLOBOT !EDM
