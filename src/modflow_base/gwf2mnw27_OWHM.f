...
c      Row #  = Description
c------------------------------------------------------------------
c         1   = IACTIV (0: inactive; 1: active, -1 disabled for simulation)
...
c         2   = Row    
c         3   = Column 
c         4   = Qact - Flow rate in or out of node
c         5   = Radius
c         6   = Skin Radius
c         7   = Skin hydraulic conductivity
c         8   = B factor for Node
c         9   = C factor for Node
c        10   = P loss factor associated with C factor (Ploss)
c        11   = Specified Conductance of Well when LOSSTYPE = SPECIFIEDCWC
c        12   = First Interval that node represents
c        13   = Last  Interval that node represents
c        14   = Conductance that is calcualted for node
c        15   = Set to Hwell, Hdry, Cell Bottom, or INF depending if Node is Wet, Dry, Seepage, or not yet calculated
c        16   = Tranissivity in the X-direction
C        17   = Tranissivity in the Y-direction
C        18   = Drawdown correction due to partially penetrating well
c        19   = Fraction of model cell for partial penetration flag when NOT using Ztop/Zbotm
C        20   = Ztop  of composite screen used for partial penetration - set to 1d30 initially
c        21   = Zbotm of composite screen used for partial penetration - set to 1d30 initially
c        22   = Unknown use??? - seb taking over for 0 and 1 Flag to indicate if node is active or deactivated for time step
c        23   = 
c        24   = 
c        25   = 
c        26   = 
...
c        31   = 
c        32   = 
c        33   = Vertical Hydraulic Concductivity of Model cell
c        34   = Specific Storage of Model cell
c
c------------------------------------------------------------------
c
c MNWINT
...
c
      MODULE GWFMNW2MODULE
        ...
        USE LINE_FEEDER,              ONLY: LINE_FEED                     !seb
        ...
        PRIVATE:: TABFILETYPECHARIDX, LINE_FEED, BUDGET_GROUP, 
     +            WARNING_TYPE
        !
        ...
        LOGICAL,   SAVE, DIMENSION(:),POINTER,CONTIGUOUS:: MNW_FMP_LINK
     ...
     +                                                  CapTable
        DOUBLE PRECISION, SAVE,DIMENSION(:),POINTER,CONTIGUOUS::QDES_OLD
        TYPE(TABFILETYPECHARIDX),   POINTER,SAVE:: MNW2TABFILE             !seb ADDED TABFILE SUPPORT FROM TABFILE_INTERFACE
        TYPE(LINE_FEED),            POINTER,SAVE:: MNW_FEED
        TYPE(BUDGET_GROUP),         POINTER,SAVE:: MNW2BUD
        INTEGER,       DIMENSION(:),POINTER,SAVE:: GRPNODE
        TYPE(WARNING_TYPE),         POINTER, SAVE:: MNW2_WARN
        TYPE(GENERIC_OUTPUT_FILE), POINTER, SAVE:: PRNT_MNW2
        TYPE(GENERIC_OUTPUT_FILE), POINTER, SAVE:: PRNT_MNW2_Q_NODE
        TYPE(GENERIC_OUTPUT_FILE), POINTER, SAVE:: PRNT_MNW2_Q_INOUT
      TYPE GWFMNWTYPE
        ...
        LOGICAL,         DIMENSION(:),POINTER,CONTIGUOUS:: MNW_FMP_LINK
        ...
        DOUBLE PRECISION, DIMENSION(:),POINTER,CONTIGUOUS::QDES_OLD
        ...
        INTEGER,    DIMENSION(:),POINTER:: GRPNODE
...
C     LFK  Nov 2015  Incorporate several code fixes and changes made by Scott Boyce for MF-OWHM (seb) [CA WSC]
C
C     SPECIFICATIONS:
C     ------------------------------------------------------------------
      USE GLOBAL,       ONLY:LIST_OUT => IOUT, NLAY, NOCBC
...
C-LFK     2                       CapTable,SMALL,NTOTNOD,WELLID
     ...
     3               MNW2TABFILE, MNW_FEED,MNW2BUD, GRPNODE,IOUT,LOUT,
     ...
     7               PRNT_MNW2, PRNT_MNW2_Q_NODE,PRNT_MNW2_Q_INOUT
      ...
      USE LINE_FEEDER,        ONLY: LINE_FEED                           !seb
      USE UTIL_INTERFACE,     ONLY: READ_TO_DATA, WARNING_MESSAGE, 
     1                              PARSE_WORD_UP, 
     2                              GET_INTEGER, GET_NUMBER
      USE NUM2STR_INTERFACE,  ONLY: NUM2STR
      USE GENERIC_BLOCK_READER_INSTRUCTION, ONLY: GENERIC_BLOCK_READER
      USE CONSTANTS,          ONLY: NL, TRUE, FALSE, BLNK, inf_I, Z
C
      TYPE(GENERIC_BLOCK_READER):: BL
      !CHARACTER*700 LINE --NOTE THAT BL%LN IS ALREADY AVAILBLE, SO USE THAT SPACE
      LOGICAL:: FOUND_BEGIN
C     ------------------------------------------------------------------
C
...
C1------grids to be defined.
     ...
     1 NMNWVL)
      ALLOCATE(MNW2BUD,IOUT,LOUT,MNW2_WARN)
      ...
      !CALL READ_TO_DATA(LINE,IN,LOUT,LOUT)
      !!
      !CALL MNWLIST%INIT(LINE,LOUT,IN)
      !IOUT = MNWLIST%IOUT
C
C2------IDENTIFY PACKAGE AND INITIALIZE NMNW2.
      WRITE(IOUT,1) IN
C---LFK--modify dates & version
    1 format(/,1x,'MNW2 -- MULTI-NODE WELL 2 PACKAGE, VERSION 7.5,',
     +' 04/23/2017.',/,4X,'INPUT READ FROM UNIT ',i3)
      NMNW2=0
      ntotnod=0
c-lfk-Dec 2012
      nodtot=0
...
!     +          NUM2STR(HCOF_RHS_FLIP), ' SOLVER ITERATIONS.'
     ...
     +    'RATE_IN (double),  RATE_OUT (double),  HEAD_WELL (double)' 
      ...
      !
      CALL MNW2BUD%RESET()
      ALLOCATE(GRPNODE(MNW2BUD%NGRP), SOURCE = 0)
      !
      !CALL READ_TO_DATA(BL%LN,IN,LOUT)
      !
...
!     1        BL%LN(ISTART:ISTOP).EQ.'AUX') THEN
...
C5------FOR WELLID array, add an extra spot 
      ...
      ALLOCATE(QDES_OLD(MNWMAX), SOURCE=0D0)
C-LFK
...
C7------SAVE POINTERS TO DATA AND RETURN.
     ...
     1                       IBOUND,LAYCBD, UPLAY
      USE GWFMNW2MODULE, ONLY:NMNW2,MNWMAX,NMNWVL,IWL2CB,MNWPRNT,
     1                       NODTOT,INTTOT,MNWAUX,MNW2,MNWNOD,MNWINT,
     2                       CapTable,SMALL,WELLID,NTOTNOD,IOUT,LOUT,
     ...
     +                       WARNING_MESSAGE, JOIN_TXT
      USE NUM2STR_INTERFACE,  ONLY: NUM2STR, SEQ2STR
      USE BAS_UTIL, ONLY: GET_LOWEST_LAYER!(IR,IC,IBOUND) RESULT(IL)
      USE WARNING_TYPE_INSTRUCTION, ONLY: WARNING_TYPE
      USE CONSTANTS, ONLY: NL,BLN,TRUE,FALSE,BLNK,DZ,UNO,DNEG,
     +                     NEARZERO_20,ninf, Z, ONE
...
!     +        'MNW2 ERROR: FAILED TO IDENTIFY LOSSTYPE, '//NL//
!     +        'THE FOLLOWING LOSSTYPE IS NOT ACCEPTABLE: '//LOSSTYPE)
        end if
...
!          CALL STOP_ERROR(LINE,IN,LOUT,'MWN2 ERROR: "'//
!     +         WELLID(MNWID)//'" WELLID IS NOT UNIQUE. PLEASE FIX THIS')
          end if                
    2   continue
...
!            CALL STOP_ERROR(LINE,IN,LOUT,'MWN2 ERROR: "'//
!     +                             'PUMPCAP CANNOT BE GREATER THAN 25')
        end if
...
!              CALL STOP_ERROR(LINE,IN,LOUT,'MWN2 ERROR: "'//
!     +                   'LOSSTYPE "NONE" REQUIRES NNODES=1 AND NOT >1')
          end if
          MNW2(3,MNWID)=0
        elseif(LOSSTYPE.EQ.'THIEM') then
          MNW2(3,MNWID)=1 
...
!            CALL WARNING_MESSAGE(INFILE=IN,OUTPUT=LOUT,
...
c     read Data Set 2c, depending on LOSSTYPE (MNW2(3,MNWID)
        Rw=1D-30; Rskin = Rw; Kskin = Rw; B = Rw; C = Rw; P = 1D0
        SELECT CASE (INT(MNW2(3,MNWID)))
          ...
          CASE (4)
            READ(in,*) CWC
          CASE (-1)
            READ(in,*) LINE  !ERROR FOUND JUST BYPASS LOADING
        END SELECT
c     don't allow Rw = 0
            if(SNGL(Rw).EQ.0.0) then
          CALL ERR%ADD(WELLID(MNWID)//
     +            ' -- Rw = 0.0??? This is not allowed'//NL)
            endif
            if(SNGL(Rskin).EQ.0.0) then
          CALL ERR%ADD(WELLID(MNWID)//
     +            ' -- Rskin = 0.0??? This is not allowed'//NL)
            end if
            if(SNGL(Kskin).EQ.0.0) then
          CALL ERR%ADD(WELLID(MNWID)//
     +            ' -- Kskin = 0.0??? This is not allowed'//NL)
            endif
            if(P.gt.0D0.and.(P<1D0.or.P>3.5D0)) then
          CALL ERR%ADD(WELLID(MNWID)//
...
c     count nodes (will uses this to check vs. allocation)
          ...
          if(ntotnod.gt.nodtot) then
            CALL STOP_ERROR(INFILE=IN,OUTPUT=LOUT,MSG=
...
c     loop over nodes 
        ...
        BNode=RwNode; CNode=RwNode; PNode=1D0
          !
...
c     save node location, set Qdes=0.0, set PP, flag ZPD
            ...
            !
            if(SNGL(RwNode).LE.0.0) then
          CALL ERR%ADD(WELLID(MNWID)//
     +            ' -- Rw <= 0.0??? This is not allowed'//NL)
            endif
            if(SNGL(RskinNode).LE.0.0) then
          CALL ERR%ADD(WELLID(MNWID)//
     +            ' -- Rskin <= 0.0??? This is not allowed'//NL)
            end if
            if(SNGL(KskinNode).LE.0.0) then
          CALL ERR%ADD(WELLID(MNWID)//
     +            ' -- Kskin <= 0.0??? This is not allowed'//NL)
            endif
            if(PNode.gt.0D0.and.(PNode<1D0.or.PNode>3.5D0)) then
          CALL ERR%ADD(WELLID(MNWID)//
     +   ' -- P  MUST BE 1 <= P <=3.5, BUT FOUND: '//NUM2STR(PNode)//NL)
            endif
            !
...
c     Set var for interval
     ...
     +   'Ztop = '//NUM2STR(Ztop)//' and Zbotm  = '//NUM2STR(Zbotm)//NL)
            ...
            END IF
            !
            IF(UPLAY(IC,IR)==0) THEN  !No Active Cells, so kill search and move screen to bottom of model
                ITOP  = NLAY
                IBOT  = NLAY
                IL    = NLAY
                Ztop = ninf
     ...
     +   'for less than 1% into layer '//NUM2STR(I)//'.'//NL )
                ...
                !
                IF(IL==-1) THEN  ! This should not happen, deactivate node
                    IL = NLAY
                    Ztop  = BTM
                    Zbotm = BTM
                END IF
...
c     save IR and IC to check vs the subsequent nodes
            IF(IINT.EQ.1) THEN
              IRlast=IR
              IClast=IC
            ELSEIF((IR.NE.IRlast).OR.(IC.NE.IClast)) THEN
          CALL ERR%ADD(WELLID(MNWID)//
...
!                K=K+1
                !
               IF(IBOUND(IC,IR,K)==0) THEN  !NODE IS IN IBOUND=0 SO DO NOT STORE IT!!!
                      CALL WRN%ADD(WELLID(MNWID)//
...
!              write(LOUT,*) '***WARNING*** Pump location specified but
!     & not found within a MNW2 node'
!              write(LOUT,*) ' Pump assumed to be at top node'
            MNW2(11,MNWID)=0
          end if
C--lfk
          IF(ifound.eq.1) THEN
...
C
     ...
     +        'PUMPING_RATE_INI'//SEQ2STR('NOD_', MXNODE,17, PAD=I))
      ...
      IPRT = 0
      IF (MNWPRNT>0) IPRT = 1
      CALL MNW_FEED%CELLS(0,0,0,IPRT,WELLID)     !=>FEED_CELLS(LDIM,NPROP,NAUX,IPRT)
      !
      END SUBROUTINE
      !
...
!  888 CONTINUE
     ...
     + 'MNW2 ERROR:  FAILED TO READ STRESS PERIOD VARIABLE "ITMP".')
          END IF
          !
          CALL WARNING_MESSAGE(OUTPUT=LOUT,
...
c        if itmp less than zero, reuse data. print message and return.
        ...
        DO iw=1, MNWMAX
              ...
              IF(MNW2(1,IW).EQ.1) THEN
                  ...
                  !
                  MNWNOD(4,firstnode:lastnode)=0.0
...
c
     ...
     & kper
        ...
        !
        do iread=1,ITMP
          Cprime = 0D0                                            !seb Initialize to a value
...
c1------if number of wells <= 0 then return.
     ...
     +                       MNW2TABFILE%TABNAM(TABIDX)
      ...
      ! Reset nodal flag
      !
      nd =NINT( MNW2(4,MNWMAX) + ABS(MNW2(2,MNWMAX)) - 1D0 )  !Get position of the last node simulated
      !
      DO CONCURRENT (INODE=1:nd)
                               MNWNOD(22,INODE) = 1D0   !Set all nodes to active (note MNW2(1,:) supercedes this flag)
      END DO
c
...
c   Loop over nodes in well
            ...
            do INODE=firstnode,lastnode
              ...
              IF(IBOUND(ic,ir,il).ne.0) THEN
                !
                IF(.NOT. HAS_NWT .OR. LAYHDT(IL) == 0) THEN
                        !
                        CHKER = .TRUE.
                        !
                ELSEIF( LAYHDT(IL) > 0 .AND. 
...
!                write(LOUT,*) '***WARNING*** Specified-head condition s
!     &hould not exist in same cell as a multi-node well'
!                   write(LOUT,840) nd,wellid(iw),il,ir,ic
...
!                  write(LOUT,*)'***WARNING*** CWC<0 in Well ',WELLID(iw)
                cond=0.d0
              end if
...
c   check cond<0, reset to 0 and print warning
     ...
     +      MSG='MNW2 WELLID '//WELLID(IW)//': CWC<0 IN WELL, '//
     +      'SETTING CWC TO ZERO', INLINE=.TRUE.)
!                  write(LOUT,*) '***WARNING*** CWC<0 reset to CWC=0'
!                  write(LOUT,*) 'In Well ',WELLID(iw),' Node ',firstnode
                cond=0.d0
...
!                 write(LOUT,*) '***WARNING*** Specified-head condition s
!     &hould not exist in same cell as a multi-node well'
!                   write(LOUT,850) wellid(iw),il,ir,ic
...
C       print warning if found
            ...
            CALL WARNING_MESSAGE(OUTPUT=LOUT,
     +      MSG='MNW2 WELLID '//WELLID(IW)//': HAS THE TOP NODE '//
     +      "LOCATED IN AN IBOUND=0 CELL",INLINE=.TRUE.)
!                 write(LOUT,*) '***WARNING***  No-flow bdy. condition ex
!     &ists in same cell as a multi-node well:'
!                   write(LOUT,852) wellid(iw),il,ir,ic
...
c  If so, set Qact = 0.0
            firstnode=MNW2(4,iw)
            lastnode =IDINT(MNW2(4,iw)) + NNODES - 1
...
c  End do, loop over all wells
      ...
      !
      !IF(KPER==2.AND. KSTP==1) THEN
      ! OPEN(123321,FILE='MNW_OWHM.out',STATUS='REPLACE')  
      !END IF
      !!
      !IF(MNW2(1,95)==1) THEN
      !WRITE(123321, '(*(I7))') KPER, KSTP
      !WRITE(123321, '(*(1x ES16.9))') MNW2(:, 95)
      !WRITE(123321, '(*(1x ES16.9))') MNWNOD(:,211)
      !WRITE(123321, '(*(1x ES16.9))') MNWNOD(:,212)
      !WRITE(123321, '(*(1x ES16.9))') MNWINT(:,186)
      !WRITE(123321, '(*(1x ES16.9))') MNWINT(:,187)
      !END IF
c
      RETURN
      END
...
C-LFK     2                       CapTable,SMALL,WELLID
     ...
     3                       IOUT,LOUT,MNW2_WARN,SOLV_FLG,
...
C
      verysmall = 1.0D-20
      !
      EVEN_ITER = mod(kiter,2).eq.0
C
      CALL MNW2_WARN%INIT()
      !
      !CHECK FOR SOLVER FLIP ITERATIONS
      !
      IF(HCOF_RHS_FLIP > 0 .AND. KITER < HCOF_ONLY_ITER) THEN
          !
          IF(MOD(KITER,HCOF_RHS_FLIP) == 0) THEN
              IF(SOLV_FLG == 1) THEN
                  SOLV_FLG = 2
              ELSE
                  SOLV_FLG = 1
      ...
      !
      IF(HCOF_ONLY_ITER < KITER ) SOLV_FLG = 2
c
c   Compute cell-to-well conductance for each well node
c
...
c   Loop over nodes in well
          do INODE=firstnode,lastnode
            ...
            ic=MNWNOD(3,INODE)
            !
            IF(IBOUND(ic,ir,il).ne.0 .or. MNWNOD(22,INODE) < 0.5D0) THEN
              !
              IF(.NOT. HAS_NWT .OR. LAYHDT(IL) == 0) THEN
                      !
                      CHKER = .TRUE.
...
cdebug              if(iqslv.ne.0.and.kiter.gt.1.and.kiter.lt.NoMoIter) then
              if(iqslv.ne.0.and.kiter.gt.1) then
                ...
                if (mnw2(5,iw) < -1D-30 .and. abs(MNW2(2,iw)).eq.1) then  !RGN 1/15/16  --if desired pump is extraction but back calc results in injection
                  if ( qact > 0.0 ) then
                      qact = 0.0
                      cond = 0.0
                  end if
...
!RGN modifications for NWT. 3/21/12
                IF ( HAS_NWT ) THEN
                  ...
                  if (mnw2(5,iw) < -1D-30.and.abs(MNW2(2,iw)).eq.1) then  !RGN 1/15/16
                    if ( qact > 0.0 ) then
                        qact2 = 0.0
                        cond = 0.0
                    end if
...
C-LFK     2                       SMALL,WELLID,NTOTNOD
     ...
     3                       IOUT,LOUT,MNW2_WARN,HAS_NWT,
     4                       SOLV_FLG,HCOF_ONLY_ITER,MXNODE,LEN_WELLID,
     7                     PRNT_MNW2, PRNT_MNW2_Q_NODE,PRNT_MNW2_Q_INOUT
      USE UTIL_INTERFACE,    ONLY: WARNING_MESSAGE
      USE CONSTANTS,         ONLY: NL, TRUE, FALSE, inf_I
      USE NUM2STR_INTERFACE, ONLY: NUM2STR
...
c     ------------------------------------------------------------------
c
      IF(HCOF_ONLY_ITER < inf_I) SOLV_FLG = 0  !RESET TO DEFAULT
      !
      ibd=0
...
c -----print the header for individual rates if requested(IWL2CB<0).
     ...
     +l    Seepage elev.'
      ...
      GROUPS: DO IG=1, MNW2BUD%NGRP
         !
         RATIN =0.D0               !clear ratin and ratout accumulators.
         RATOUT=0.D0
         TEXT = MNW2BUD%GRP(IG)
         TEXT = ADJUSTR(TEXT)
C
...
c         IF(IAUXSV.EQ.0) NAUX=0
     ...
     1          GRPNODE(IG),IOUT,DELT,PERTIM,TOTIM,IBOUND)   !REPLACED ntotnod with GRPNODE
      END IF
c  clear the buffer.
      buff=0.0
c
c2------if there are no wells do not accumulate flow
      if(MNW2BUD%DIM(IG).gt.0) then  !formully nmnw2
...
c   Loop over nodes in well
          do INODE=firstnode,lastnode
            ...
            !
            nd=INODE-firstnode+1           
            DryTest = hcell - Hdry                             !Scott SHOULD THIS BE AN IBOUND CHECK? SEE NEW NEXT LINE
            IF(IBOUND(IC,IR,IL).EQ.0) DryTest=0D0                       !seb ADDED IBOUND CHECK FOR DRYCELL TEST
            IF(LAYHDT(IL).GT.0.AND.HAS_NWT .AND. 
     +         hcell.LE.BOTM(IC,IR,LBOTM(IL)))  DryTest=0D0    !seb ADDED CHECK FOR NWT DRY CELL WHERE HNEW<BOT  SCOTT THIS NEEDS TO BE DOUBLE CHECKED BUT BELOW IT FLAGS A WRITE SAYING THIS IS TRUE, BUT NO STATEMENT EXISTED FOR IT TO HAPPEN
            if(ABS(DryTest).lt.verysmall) then
              MNWNOD(4,INODE)= 0.0D0
!              MNW2(17,iw)=Hdry                              !seb 
!              iweldry=1
              if(MNWPRNT.gt.1) then
                  write(LOUT,*) 'MNW2 node in dry cell, Q set to 0.0'
                  write(LOUT,*) 'Well: ',WELLID(iw),' Node: ',ND        !seb CHANGED INODE TO ND
              end if
            end if
...
c  check to write message that inode was deactivated this time step
                if(mnwprnt.gt.1.and.mnwnod(14,inode).eq.0.0) then
                  IF(hcell.LT.BOTM(IC,IR,LBOTM(IL))
     +                                 .AND.hcell.NE.HDRY) THEN
      IF(MNWPRNT>-1) THEN
        write(LOUT,200) ND,WELLID(iw)
...
c    Report all wells with production less than the desired rate......
            if(ibound(ic,ir,il).NE.0.AND.ABS(DryTest).GT.verysmall) then!If it is active or dry? seb I AGREE< LOOKS LIKE IT SHOULD NOT be .NE.0 IF CELL IS DRY
              if(MNW2(6,iw).NE.0) then
                hlim = mnw2(7,iw)
              else
...
! for unconfined condtions. RGN 3/20/12
     ...
     +                  hlim = BOTM(ic,ir,lbotm(il))
              ...
              if ( mnw2(5,iw) < -1D-30 .and. abs(MNW2(2,iw)).eq.1) then
                if ( q > 0.0 ) q = 0.0
              end if                    !to hear
              END IF
c
...
!                write(LOUT,*)'Note-- the following MNW2 well went dry:',
!     +  WELLID(iw)                
...
c-lfk
            ...
            do INODE=firstnode,lastnode
              ...
              IF(LAYHDT(IL).GT.0.AND.HAS_NWT .AND. 
     +           hcell.LE.BOTM(IC,IR,LBOTM(IL))) MNWNOD(4,INODE) = 0.D0 !seb ADDED CHECK FOR WHEN HNEW<BOT, SCOTT SHOULD THIS BE HERE, WHAT IF Hwel>BOT, there would be flow, but very minimal. NOTE MNWNOD(4,INODE)=Qact
              q=MNWNOD(4,INODE)
...
! seb Commented out the next 4 lines because INODE is not defined and iseepflg is set at "ISEEPFLG=LIMQ(3,IW)"
...
c  lfk  active well check
                if(MNW2(1,iw).LE.0.0001) Q=0.0D0  !WELL IS INACTIVE
...
c10-----increment budget term counter(msum).
     ...
     +          'WHILE SOLVING THE CURRENT TIME STEP:',OUTPUT=LOUT)
...
c
      ...
      INTEGER, INTENT(IN):: KPER, IGRID, IBCF, ILPF, IUPW, IHUF
      !
      
      IF    (ILPF .NE. 0) THEN;  CALL GWF2MNW27LPF(KPER,IGRID)
      ELSEIF(IUPW .NE. 0) THEN;  CALL GWF2MNW27UPW(KPER,IGRID)
      ELSEIF(IBCF .NE. 0) THEN;  CALL GWF2MNW27BCF(KPER,IGRID)
      ELSEIF(IHUF .NE. 0) THEN;  CALL GWF2MNW27HUF(KPER,IGRID)
      ELSE
...
C     ------------------------------------------------------------------
     ...
     1                       BOTM,HNEW,LAYHDT,LAYCBD,CR,CC,ISSFLG,ITRSS
      ...
      USE GWFMNW2MODULE, ONLY:NMNW2,MNWMAX,MNWPRNT,
     1                       NODTOT,MNW2,MNWNOD,IOUT,LOUT,
     2                       WELLID
C     ------------------------------------------------------------------
      INTEGER firstnode,lastnode
...
C     ------------------------------------------------------------------
     ...
     1                       NODTOT,MNW2,MNWNOD,SMALL,WELLID,IOUT,LOUT,
     2                       MNW2_WARN,HAS_NWT,PP_CON_ITER_LIM
      USE UTIL_INTERFACE, ONLY: WARNING_MESSAGE
      USE NUM2STR_INTERFACE, ONLY: NUM2STR
      USE CONSTANTS,   ONLY: NL, TRUE, FALSE
      CHARACTER*10 ctext
...
c   Only operate on active wells (MNW2(1,iw)=1)
       if (MNW2(1,iw).EQ.1) then
        SKIN=0.0                         !seb set up SKIN so that it has a value when passed into CEL2WEL
...
c 2
         ...
         IF(LAYHDT(IZ).NE.0.AND.
     +      HAS_NWT.AND.hnew(ix,iy,iz).LE.BOTM(IX,IY,LBOTM(IZ))) THEN
                                                              irecalc=0  !seb FIX TO ENSURE THAT DRY CELLS OR THICK=0 DO NOT GET PROCESSED --SCOTT THIS MAY NEED TO ZERO OUT CWC AS WELL, but it should have a smoother later on
                                                 MNWNOD(14,INODE)=0.0D0  !seb if cell is inactive, then zero out conductance of node (CWC)
         END IF
c if confined (THICKNESS IS NOT HEAD-DEPENDENT), don't recalculate CWC
         if(LAYHDT(IZ).EQ.0.and.kiter.gt.1.and.PPFLAG==0) irecalc=0
c
c if PP then always recalculate
         if(LAYHDT(IZ).EQ.0.and.kiter.gt.PP_CON_ITER_LIM)  irecalc=0    !.and.PPFLAG>0 is implied
C
C only solve for active nodes
         if( MNWNOD(22,INODE) < 0.5D0 ) irecalc=0
c
...
c if confined (THICKNESS IS NOT HEAD-DEPENDENT), don't use hnew=top  --seb Moved above if(irecalc.eq.1) then
     ...
     &          top=BOTM(IX,IY,LBOTM(IZ)-1)
            ...
            bot = BOTM(IX,IY,LBOTM(IZ))
            thck = top-bot
            IF( thck<verysmall ) thck=0.9D0*verysmall  !seb make it slightly smaller for checks
            !
...
!     +      'SETTING CWC TO ZERO', INLINE=.TRUE.)
              ...
              END IF
                       ...
                       MNWNOD(22,INODE)=0D0 !No Screen Interval, deactivate
                  end if
                end if
              end do
c-LFK
...
c              go to 567
            ...
            IF(LAYHDT(IZ).GT.0.AND.PPFLAG.GT.0.and.KITER.GT.2) then
              ...
              PPC_CHK = alpha2 > 0.01
              !
              IF(PPC_CHK.AND.totlength.gt.0D0) 
...
c       write (iout,*) ' alpha reset to 0.0 '                
             ...
             END IF  !(alpha2.lt.0.01) then
             END IF  !(KITER.GT.25) THEN
            END IF
c567    continue  
c
...
c           only do partial penetration effect if PP>0 and alpha <1.0
            !PPFLAG=INT(MNW2(19,iw))
            IF(PPFLAG.GT.0.and.alpha.lt.1.D0.AND.PPC_CHK) then    !seb PPC_CHK is used to keep PP on for entire simulation
c
...
C                  or cond reset to 0 or ppflag reset to 0
                ...
                if (cond.eq.0) ippc=ippc+3  !seb changed mnwnod(14,inode) to cond
                if (ppflag.eq.0) ippc=ippc+4
                if (zpd.eq.zpl) ippc=ippc+5
                if(ITFLAG==0)   ippc=ippc+6 !seb since mnwnod(14,inode)=zero when ITFLAG=0, but now use COND for check
                if(MNWNOD(22,INODE) < 0.5D0 ) ippc=ippc+7 !seb node was deactivated
...
c          
                  ...
                  ISOLNFLAG=1         !Set to true only to keep code moving forward
                  dhp=0.0
                  MNWNOD(18,INODE)=dhp
              end if
...
c  if alpha <= 0.2, shut well off if PPC did not converge
                  if(alpha.lt.0.2) then
                   if (MNWPRNT.gt.1) then
                    nd=INODE-firstnode+1
                    write(LOUT,*) 'Partial penetration solution did not
...
c  LFK 6/2012  change check on dhp to include 0.0 to avoid unneeded warnings
     ...
     &          .OR.(Qact.gt.0.D0.AND.dhp.le.0.D0)) then
                ...
                if(cond.gt.verysmall .AND. ABS(dpp).gt.verysmall ) then
                  ABC=1/cond
                  ABCD=ABC+dpp
                  cond=1/ABCD
                end if
                ...
                dpp=0.d0
            CALL MNW2_WARN%ADD(WELLID(IW)//'HAS PARTIAL PENETRATION '//
...
!                  write(LOUT,*) '***WARNING***  Partial penetration term
...
C     ------------------------------------------------------------------
      ...
      USE GWFMNW2MODULE, ONLY:mnw2,IOUT,LOUT
      USE UTIL_INTERFACE, ONLY: WARNING_MESSAGE
      IMPLICIT NONE
!External functions
...
C      
      ...
      !
      ELSEif( rw.lt.verysmall ) THEN  !No flow cause well is super small
              cel2wel2 = 0D0
      ELSEif( Txx.lt.verysmall.or.Tyy.lt.verysmall )
     +  then
...
C
        ...
        cel2wel2 = A + B + C 
        !if ( cel2wel2>verysmall )cel2wel2 = 1.0D0 / cel2wel2
        IF ( cel2wel2 < verysmall .AND. A > verysmall) THEN
                                           cel2wel2 = 1.0D0 / A  !SWITCH LOSS TYPE TO THIEM DUE TO VERY SMALL VALUE
        ELSEIF ( cel2wel2 < verysmall) THEN
                                           cel2wel2 = 1D25    !Small A so large conductance
        ELSE
                                           cel2wel2 = 1.0D0 / cel2wel2
        END IF
      endif
...
C     ------------------------------------------------------------------
     ...
     1                       IBOUND,LAYCBD,HNEW,LAYHDT
      USE GWFBASMODULE, ONLY:HDRY
      USE GWFMNW2MODULE, ONLY:NMNW2,MNWMAX,MNWPRNT,IOUT,LOUT,MNW2_WARN,
C-LFK     1                       NODTOT,MNW2,MNWNOD,SMALL,WELLID
     1                       NODTOT,MNW2,MNWNOD,SMALL,WELLID,LIMQ,
     2                       HAS_NWT
      USE NUM2STR_INTERFACE, ONLY: NUM2STR
      USE CONSTANTS,   ONLY: NL, inf
      INTEGER firstnode,lastnode,nd  !RGN
...
c   Loop over nodes in well
     ...
     +                  HNEW(IC,IR,IL) > BOTM(IC,IR,LBOTM(IL)) ) THEN
              ...
              END IF
!!!              
!!!              (Iuupw.EQ.0 .AND. IBOUND(ic,ir,il).ne.0) .OR.
!!!     +        (Iuupw.NE.0 .AND. HNEW(IC,IR,IL).GT.BOTM(IC,IR,LBOTM(IL))
!!!     +                    .AND. IBOUND(ic,ir,il).ne.0 ) 
!!!     +                                                            ) THEN
!!!                csum  = csum  + MNWNOD(14,INODE)
...
c  Sum the seepage (there may be more than one node's worth
     ...
     & * MNWNOD(14,INODE)
               ...
               ELSE
                ...
                MNWNOD(15,INODE) = hdry
             END IF
            elseif(MNWNOD(22,INODE) < 0.5D0 ) THEN  !Node was deactivated
             MNWNOD(4,INODE) = 0.0D0
             MNWNOD(15,INODE) = hdry
            else
             MNWNOD(4,INODE) = 0.0D0
...
c                           
        ...
        if(MNWNOD(15,INODE) == inf )then
          IF(MNWNOD(14,INODE).GT.0D0 .AND. MNWNOD(22,INODE) > 0.5D0)THEN   !seb Added check to set qact=0 when COND=0
...
c     ******************************************************************
c
      USE GWFMNW2MODULE, ONLY:MNWMAX,MNW2,MNWNOD,LOUT,WELLID
      USE UTIL_INTERFACE,ONLY: STOP_ERROR
...
c 
       if(abs(denom1+denom2)<verysmall) then
            CALL STOP_ERROR(OUTPUT=LOUT,MSG=
...
C
        ...
        DEALLOCATE(GWFMNWDAT(IGRID)%LOUT)
