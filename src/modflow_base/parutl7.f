...
C1------Decode PARAMETER definitions if they exist
   ...
   31    FORMAT(1X,I5,' Named Parameters     ',I15,' List entries')
         !READ(IN,'(A)') LINE
         CALL READ_TO_DATA(LINE,IN,IOUT) !seb
      ELSE
           WRITE(IOUT,'(A)') ' No named parameters'
      END IF
C
...
C     ------------------------------------------------------------------
      ...
      CHARACTER*4 PTYP
      CHARACTER(MXNAMLEN):: PN,CTMP1,CTMP2                              !seb CHANGED FROM CHAR LEN OF 10
      CHARACTER*700 LINE
...
C9------Check if the parameter list will fit in the package list array.
        IF((LSTSUM-1) .GT. MXLST) THEN                                  !seb MADE THE FOLLOWING ERROR STATEMENTS CLEANER
            LINE=NL//
...
C10-----Check if number of instances exceeds the maximum allowed.
        IF((INAMLOC-1).GT.MXINST) THEN
          LINE=NL//                                                     !seb MADE THE FOLLOWING ERROR STATEMENTS CLEANER
...
C11-----Check for correct parameter type.
        IF(PARTYP(NP).NE.PTYPX) THEN
          LINE=NL//'ERROR: Parameter type must be: '//TRIM(PTYPX)//
...
C12-----Parameter definition must include at least one cell.
        IF (NLST.LT.0) THEN
          LINE=NL//' ERROR:  DEFINITION FOR PARAMETER "'
     +        //TRIM(PN)//'" INCLUDES NO CELLS'//NL//
     +        '   -- STOP EXECUTION (UPARLSTRP)'
          WRITE(IOUT,'(A)')TRIM(LINE)
!            WRITE(IOUT,140) PN
!          ENDIF
...
C     ------------------------------------------------------------------
      USE PARAMMODULE
      CHARACTER(MXNAMLEN):: CTMP1,CTMP2                                 !seb CHANGED FROM CHAR LEN OF 10
      CHARACTER*700 LINE
C     ------------------------------------------------------------------
C
