...
C
  ...
  121 FORMAT(1X/,1X,'PARAMETER NAME:',A,'   TYPE:',A,' UNITS:',I4)
        WRITE(IOUT,122) PV
  122 FORMAT(1X,'The parameter value from the package file is:',ES13.5)
        IF(B(NP).NE.PV) WRITE(IOUT,123) B(NP)
  123  FORMAT(1X,'This parameter value has been replaced by the',
     &  ' value from the',/1X,'Parameter Value file:',ES13.5)
C
C  Read clusters
...
C  Store layer number for LVDA or SYTP
          IF(PARTYP(NP).EQ.'LVDA') THEN
            CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,IPCLST(1,I),R,0,IN)
