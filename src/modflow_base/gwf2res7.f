      MODULE GWFRESMODULE
        ...
        !PRIVATE:: BUDGET_GROUP, PACKAGE_LISTING
        !
...
C2------RESERVOIR OPTIONS
      CALL URDCOM(IN,IOUT,LINE)   !seb allow for comments and changed to free format read.
      LLOC=1
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,NRES,R,IOUT,IN)
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,IRESCB,R,IOUT,IN)
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,NRESOP,R,IOUT,IN)
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,IRESPT,R,IOUT,IN)
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,NPTS,R,IOUT,IN)
      !
...
C5------VERIFY LOCATIONS EXIST FOR ALL RESERVOIRS
      ...
      DO 30 I=1,NROW
      DO 20 J=1,NCOL
!      IF(IBOUND(J,I,1).LE.0) IRES(J,I)=0    --BAD TO DROP CELLS CAUSE LAY 1 IS INACTIVE. RES COULD BE IN DIFFERENT LAYER OR SEARCH FOR ACTIVE LAYER
...
C
     ...
     +      NEW_LINE(' ')//TRIM(LINE)
      ...
      !
      LLOC=1
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,3,IDUM,HRESSE(1,N),IOUT,IN)
      CALL URWORD(LINE,LLOC,ISTART,ISTOP,3,IDUM,HRESSE(2,N),IOUT,IN)
      !
...
C     ------------------------------------------------------------------
      DOUBLE PRECISION:: HHNEW, RBOT, CR, HR
      CALL SGWF2RES7PNT(IGRID)
C
...
C6------IF RESERVOIR STAGE IS BELOW RESERVOIR BOTTOM, SKIP IT
 70   HR=DBLE(HRES(NR))
      IF(HR.LE.DBLE(BRES(IC,IR)))  GO TO 190
...
C8A-----GROUND-WATER HEAD > BOTTOM THEN RATE=CR*(HR-HNEW).
      IF(HHNEW.GT.RBOT)RATE=SNGL(CR*(HR-HHNEW))
C
C8B-----GROUND-WATER HEAD < BOTTOM THEN RATE=CR*(HR-RBOT)
      IF(HHNEW.LE.RBOT)RATE=SNGL(CR*(HR-RBOT))
C
