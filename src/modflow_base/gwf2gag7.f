...
C     SPECIFICATIONS:
C     ------------------------------------------------------------------
      USE CONSTANTS,    ONLY: BLNK, Z, ONE, FOUR
      USE GLOBAL,       ONLY: IOUT
      USE GWFGAGMODULE
      USE UTIL_INTERFACE, ONLY: GET_INTEGER, READ_TO_DATA, 
     +                          WARNING_MESSAGE, STOP_ERROR
C     ------------------------------------------------------------------
      CHARACTER(256):: LINE
      INTEGER:: LLOC, ISTR, ISTP
C
C1------TURN OFF GAGE PACKAGE IF ACTIVE AND 
C         STREAMS AND LAKES ARE INACTIVE.
      IF(IUNITSFR.EQ.Z .AND. IUNITLAK.EQ.Z) THEN
         CALL WARNING_MESSAGE(BLNK,INGAGE,IOUT,'GAGE PACKAGE '//
     +                'ACTIVE EVEN THOUGH SFR AND LAK PACKAGES '//
     +                'ARE INACTIVE: GAGE PACKAGE IS BEING TURNED OFF')
         CLOSE(INGAGE, IOSTAT=LLOC)
!           WRITE(IOUT,1)
...
C         IS LESS THAN OR EQUAL TO ZERO.
      !
      LLOC = ONE
      CALL READ_TO_DATA(LINE,INGAGE,IOUT,IOUT)
      CALL GET_INTEGER(LINE, LLOC, ISTR, ISTP, IOUT, INGAGE, NUMGAGE,
     +                 MSG='GAGE PACKAGE FAILED TO LOAD NUMGAGE')
      !
      IF(NUMGAGE.LE.Z) THEN
         CALL WARNING_MESSAGE(LINE,INGAGE,IOUT,
     +                      'NUMGAGE <= Z, SO GAGE IS BEING TURNED OFF')
         CLOSE(INGAGE, IOSTAT=LLOC)
         DEALLOCATE(NUMGAGE)
!        !
         INGAGE=Z
...
C         (4) OUTTYPE
      !
      ALLOCATE (IGGLST(FOUR,NUMGAGE))
      !
      IGGLST = Z 
      IF (NUMGAGE .GT. ONE) WRITE (IOUT,140) NUMGAGE
      IF (NUMGAGE .EQ. ONE) WRITE (IOUT,141) NUMGAGE
C
...
C5------READ THE FIRST RECORD OF LIST.
      DO IOB=1,NUMGAGE
         !LN,LOC,ISTART,ISTOP,IOUT,IN,VAL
         LLOC = ONE
         CALL READ_TO_DATA(LINE,INGAGE,IOUT)
         CALL GET_INTEGER(LINE,LLOC,ISTR,ISTP,IOUT,INGAGE,IGGLST(1,IOB),
     +       MSG='GAGE PACKAGE FAILED TO LOAD SEGMENT (or LAKE) NUMBER')
         !
         IF(IGGLST(1,IOB)==Z) CALL STOP_ERROR(LINE, 
     +                INGAGE, IOUT,
     +                'GAGE: SEGMENT (or LAKE) NUMBER MUST BE NON-ZERO')
         !READ(INGAGE,*) IGGLST(1,IOB)
         !BACKSPACE INGAGE
         IF (IGGLST(1,IOB).GT.Z) THEN
C
C6------STREAM GAGE.
            NSG=NSG+1
         CALL GET_INTEGER(LINE,LLOC,ISTR,ISTP,IOUT,INGAGE,IGGLST(2,IOB),
     +         MSG='GAGE PACKAGE FAILED TO LOAD REACH NUMBER (GAGERCH)')
         CALL GET_INTEGER(LINE,LLOC,ISTR,ISTP,IOUT,INGAGE,IGGLST(3,IOB),
     +         MSG='GAGE PACKAGE FAILED TO LOAD UNIT NUMBER')
         CALL GET_INTEGER(LINE,LLOC,ISTR,ISTP,IOUT,INGAGE,IGGLST(4,IOB),
     +         MSG='GAGE PACKAGE FAILED TO LOAD OUTTYPE')
...
C         THAT OUTTYPE FOR A LAKE IS READ.
         IF(IGGLST(3,IOB) < Z) 
     +   CALL GET_INTEGER(LINE,LLOC,ISTR,ISTP,IOUT,INGAGE,IGGLST(4,IOB),
     +         MSG='GAGE PACKAGE FAILED TO LOAD OUTTYPE NUMBER')
             
            !READ(INGAGE,*) IGGLST(1,IOB),IGGLST(3,IOB)
            !IF (IGGLST(3,IOB).LT.Z) THEN
            ! BACKSPACE INGAGE
...
C8------TRANSPORT IS ON.
               ELSE
                   IF(IUNITUZF.NE.0) WRITE (IOUT,296) 
C
...
C14-----PREPARE ARRAY OF HEADER NAMES FOR MULTIPLE CONSTITUENTS.
                   IF(IUNITUZF.NE.0) WRITE(IOUT,320)
                 DFLAG=0
...
C17-----RELEASE MEMORY.
      ...
      IF (IUNITLAK.NE.0) DEALLOCATE (DUMMY)
C18-----RETURN.
      RETURN
