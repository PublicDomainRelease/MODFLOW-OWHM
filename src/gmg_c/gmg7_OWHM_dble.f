...
C--------------------------------------------------------------------
      MODULE GMGMODULE
        ...
        PRIVATE:: C_PTR
        !PRIVATE::DEFAULT_KIND                                          !seb not sure why this is here, it does nothing, seems like something i would add, but not commented
        ...
        DOUBLE PRECISION ,SAVE,POINTER,DIMENSION(:,:,:)::HNEWLAST       !seb CHANGED FROM DOUBLE PRECISION TO SINGLE PRECISION TO MATCH ALL SUBROUTINES THAT USE IT.
        DOUBLE PRECISION,SAVE,POINTER :: BIGHEADCHG
        DOUBLE PRECISION,SAVE,POINTER  :: RELAXGMG
        DOUBLE PRECISION, SAVE,POINTER::HNOFLO_GMG                      !seb ADDED NEW VALUE FOR HNOFLO
      TYPE GMGTYPE
        ...
        DOUBLE PRECISION, POINTER::HNOFLO_GMG                           !seb ADDED NEW VALUE FOR HNOFLO
      END TYPE
     ...
     &                    HNEWLAST)
          ...
          USE GMG_C_INTERFACE
          IMPLICIT NONE
          DOUBLE PRECISION RHS(*),CR(*),CC(*),CV(*),HCOF(*),HNEWLAST(*)
...
C--------------------------------------------------------------------
      ...
      USE GWFBASMODULE,ONLY:HNOFLO                                      !seb ADDED HNOFLO
...
C--------------------------------------------------------------------
     ...
     1         SITER,TSITER,IUNITMHC,HNOFLO_GMG)!,GMGID  --GMGID no longer a pointer
      ALLOCATE(HCLOSEGMG,RCLOSEGMG,DAMPGMG,RELAXGMG)
      ALLOCATE(DUP,DLOW,CHGLIMIT)
      ALLOCATE(BIGHEADCHG)
      HNOFLO_GMG=DBLE(HNOFLO)                                           !seb SET HNOFLO TO DOUBLE PRECISION EQUIVALENT
C
C--------------------------------------------------------------------
C     READ INPUT FILE
C--------------------------------------------------------------------
...
C
      ...
      IUNITMHC = 0
      NDUM = -1
      CALL URWORD(LINE,ICOL,ISTART,ISTOP,2,NDUM,RDUM,0,IN)
...
C
      ...
      ELSE
        CALL STOP_ERROR(LINE,IN,IOUT,'ERROR IN GMG INPUT: '//
     1  'IADAMP MUST BE ONE OF 0, 1, OR 2 (GMG1ALG)')
      END IF
C
      IIOUT=IOUT
...
C
      IPREC=1                                                           !seb set precion to double only since most variables are double
      !IF(KIND(DAMPGMG) .EQ. KIND(1D0)) IPREC=1
C
      CALL MF2KGMG_ALLOCATE(GMGID,NCOL,NROW,NLAY,IPREC,ISM,ISC,
     &                      RELAXGMG,ISIZ,IERR)
      IF(IERR .NE. 0) THEN
        CALL STOP_ERROR(' ',IN,IIOUT,
     +                         'ALLOCATION ERROR IN SUBROUTINE GMG1ALG')
      END IF
C
...
C--------------------------------------------------------------------
      ...
      TARGET RHS, CR, CC, CV, HCOF
      DOUBLE PRECISION HNEWLAST(*)                                      !seb ORINIALLY DP, BUT ACTUALLY IS REAL
      REAL HNOFLO,RCLOSE,HCLOSE,DAMP
      TARGET HNOFLO
      REAL DUP,DLOW,CHGLIMIT
      DOUBLE PRECISION BIGHEADCHG
...
C
      ...
      IIOUT=IOUT
      IF(IOUTGMG .GT. 2) IIOUT=6
      IF(KITER .EQ. 1) DDAMP=DBLE(DAMP)                                 !seb ADDED DBLE
      DAMP0=DDAMP
C
C--------------------------------------------------------------------
C     ASSEMBLE SOLVER
C-------------------------------  -------------------------------------
!      CALL MF2KGMG_ASSEMBLE(GMGID,BIGR0,CR,CC,CV,HCOF,HNEW,RHS,
!     &                      HNOFLO_GMG,IBOUND,IERR)
      CALL MF2KGMG_ASSEMBLE(GMGID,BIGR0,C_LOC(CR),C_LOC(CC),
     &                      C_LOC(CV),C_LOC(HCOF),HNEW,C_LOC(RHS),
     &                      C_LOC(HNOFLO),IBOUND,IERR)
      IF(IERR .NE. 0) THEN
        CALL STOP_ERROR(' ',0,IOUT,
...
C--------------------------------------------------------------------
      DRCLOSE=DDAMP*DBLE(RCLOSE)+(1.0D0-DDAMP)*BIGR0                    !seb ADDED DBLE
...
C  Relative Reduced Residual Method
        ...
        DAMPA=DBLE(DAMP)
        BIGHA=BIGH
        IF (KSTP==1 .AND. KITER==1) BIGHEADCHG=1D0                      !seb Value on First iteration has no effect on solution, but if not set causes additional variables to become undefine.
...
C
  510 FORMAT(1X,'-------------------------------------',
     ...
     &       1X,'DAMPING                           : ',0P,F7.5,/,
     &       1X,'L2-NORM OF RESIDUAL               : ',ES11.4,/,
     &       1X,'MAX HEAD CHANGE                   : ',ES11.4,/,
...
C  Deallocate GMG data
     ...
     +                     HNOFLO_GMG
      ...
      !
      DEALLOCATE(GMGDAT(IGRID)%IITER     )
      DEALLOCATE(GMGDAT(IGRID)%IADAMPGMG )
      DEALLOCATE(GMGDAT(IGRID)%ISM       )
      DEALLOCATE(GMGDAT(IGRID)%ISC       )
      DEALLOCATE(GMGDAT(IGRID)%ISIZ      )
      DEALLOCATE(GMGDAT(IGRID)%IOUTGMG   )
      DEALLOCATE(GMGDAT(IGRID)%IPREC     )
      DEALLOCATE(GMGDAT(IGRID)%IIOUT     )
      DEALLOCATE(GMGDAT(IGRID)%SITER     )
      DEALLOCATE(GMGDAT(IGRID)%TSITER    )
      !DEALLOCATE(GMGDAT(IGRID)%GMGID     )
      DEALLOCATE(GMGDAT(IGRID)%HCLOSEGMG )
      DEALLOCATE(GMGDAT(IGRID)%RCLOSEGMG )
      DEALLOCATE(GMGDAT(IGRID)%DAMPGMG   )
      DEALLOCATE(GMGDAT(IGRID)%RELAXGMG  )
      DEALLOCATE(GMGDAT(IGRID)%IUNITMHC  )
      DEALLOCATE(GMGDAT(IGRID)%DUP       )
      DEALLOCATE(GMGDAT(IGRID)%DLOW      )
      DEALLOCATE(GMGDAT(IGRID)%CHGLIMIT  )
      DEALLOCATE(GMGDAT(IGRID)%HNEWLAST  )
      DEALLOCATE(GMGDAT(IGRID)%BIGHEADCHG)
      DEALLOCATE(GMGDAT(IGRID)%HNOFLO_GMG)
C
...
C Note: GMGID is a C_PTR; use = not =>
      ...
      HNOFLO_GMG=>GMGDAT(IGRID)%HNOFLO_GMG
C
      !
      END SUBROUTINE
C
...
C Note: GMGID is a C_PTR; use = not =>
      ...
      GMGDAT(IGRID)%HNOFLO_GMG=>HNOFLO_GMG
C
      RETURN
      END
